...
define([
      ...
      "../util/specHelper",
      "../util/trace"
], function(dojo, lang, domAttr, query, topic, url, Topics, Views, ViewContainer, GadgetDialog, specHelper, trace) {

   /* API to export */
   var api_ = lang.getObject("com.ibm.lconn.gadget.services.viewModifications", true);
   /* ViewContainer / View implementation singleton */
   var view_container = Views.__instance__();
   /* function to get the next domId */
   var nextDomId_ = specHelper.nextDomId;
   ...
   var publish_GadgetSite_event_from_func = function(function_name, event_name) {
      ...
      osapi.container.GadgetSite.prototype[function_name] = function() {
         ...
         var siteTopic = Topics.getSiteTopic(this.getId(), Topics.GadgetWindow[event_name]);
         topic.publish(siteTopic, Array.prototype.slice.call(arguments, 0));
      };
   };
    ...
    * @param container
    *           {osapi.container.Container} Instance of common container
    */
   api_.registerService = function(container) {
       ...
       */
      cviews.createElementForGadget = lang.hitch(view_container, 'createElementForGadget');
      cviews.createElementForEmbeddedExperience = lang.hitch(view_container, 'createElementForEmbeddedExperience');
      cviews.createElementForUrl = lang.hitch(view_container, 'createElementForUrl');
      cviews.destroyElement = lang.hitch(view_container, 'destroyElement');
      ...
      container.rpcRegister("requestNavigateTo", function(rpcArgs, toView, opt_params, opt_ownerId) {
         ...
         var viewParams = {};
         if (opt_params) {
            if (lang.isObject(opt_params)) {
               lang.mixin(viewParams, opt_params);
            }
            else if (lang.isString(opt_params)) {
               lang.mixin(viewParams, url.splitQuery(opt_params));
            }
            else {
               trace.log("opt_params passed to gadgets.views.requestNavigateTo could not be processed.", opt_params);
            }
         }
   ...
   /**
    * Method will be called to create the DOM element to place the Gadget Site
    * in.
    */
    ...
    * experience in. using gadgets.selection to pass the ActivityStream Item or
    * any other information relevant to the Embedded Experience chrome such as
    * title etc. Hoping to have this passed directly to this function in future.
    */
   function d_createElementForEmbeddedExperience(rel, opt_gadgetInfo, opt_viewTarget, opt_coordinates, parentSite) {
      // console.log("function createElementForEmbeddedExperience is
      // extremely minimal!");
    ...
    * 
    * @param {object=}
    *           site The id of the site to close. maybe.
    */
    ...
    *           container Common container instance.
    * @param {boolean}
    *           modaldialog true for modal dialog.
    * @param {string}
    *           view view type.
    * @param {boolean}
    *           isGadget true for gadget, false for url.
    * @param {string}
    *           opt_gadgetMetadata gadget metadata.
    * @return {Object} The DOM element to place the gadget or url site object
    *         in.
    */
   function _openInDialog(container, modaldialog, view, isGadget, opt_gadgetMetadata, opt_pos) {
      ...
      var dialog = new GadgetDialog(dialogParams);

      // this is important, we need to make sure we close the gadget properly
      // if the user closes our dialog
      // some dialog gadgets will set a return value, closing them correctly
      // makes sure this is passed back
      dialog.closeDialog = function() {
         var iFrameId = domAttr.get(query("#" + gadgetNodeId + ">iframe")[0], "id");
