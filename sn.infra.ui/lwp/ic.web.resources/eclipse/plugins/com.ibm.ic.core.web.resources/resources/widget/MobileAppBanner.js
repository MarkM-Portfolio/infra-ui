...
define([
   ...
   "dojo/query",
   "dojo/string",
   "dojo/text!ic-core/widget/templates/MobileAppBanner.html",
   "dijit/_Templated",
   "dijit/_Widget",
   "ic-core/locale",
   "ic-core/util/PreferenceCache"
], function (dojo, declare, i18nstrings, array, lang, domClass, has, domConstruct, i18n, query, string, template, _Templated, _Widget, locale, cache) {
    ...
    * @class ic-core.widget.MobileAppBanner
    */
    ...
    */
   function isAndroid() {
      var uAgent = lang.isFunction(lang.getObject('getUserAgent')) ? getUserAgent() : navigator.userAgent.toLowerCase();
    ...
    */
   function getAppId() {
      var el = query("meta[name=" + (has("ios") ? IOS_APP_ATTR_NAME : ANDROID_APP_ATTR_NAME) + "]")[0];
      if (el) {
         var attr = el.getAttribute('content', 0).split(',');
         array.map(attr, function(val) {
            val = lang.trim(val).split('=');
            if (val[0] === 'app-id')
               app_id = val[1];
         });
         if (app_id) {
      ...
      return has("ios") ? IOS_APP_ID : ANDROID_APP_ID;
   }
    ...
    */
   function buildUrl() {
      var urlTemplate = has("ios") ? IOS_URL_TEMPLATE : ANDROID_URL_TEMPLATE;
      return string.substitute(urlTemplate, {
         lang: locale.getCountry() || locale.getLanguage(),
         app_id: getAppId()
      });
   }
   ...
   function buildIconUrl() {
      return has("ios") ? IOS_ICON_URL_TEMPLATE : ANDROID_ICON_URL_TEMPLATE;
   }
   ...
   function getSrcNodeRef() {
      var lotusFrame = query(".lotusFrame")[0];
      if (!lotusFrame)
         throw "This widget requires a lotusFrame element on the page";
      return domConstruct.create('div', {}, lotusFrame, "before");
   }

   var nls = i18nstrings;

   var MobileAppBanner = declare("lconn.core.widget.MobileAppBanner", [ _Widget, _Templated ], /** @lends ic-core.widget.MobileAppBanner.prototype */ {
      templateString: template,
      postMixInProperties: function() {
         this.strings = this.strings || {
            onStore : has("ios") ? nls.rs_mobileBanner_inAppStore : nls.rs_mobileBanner_inGooglePlay,
         ...
         this.cssClass = has("ios") ? "ios" : "android";
         this.url = buildUrl();
    ...
    * @function isEnabled()
    * @memberof ic-core.widget.MobileAppBanner
    * @returns True if the banner is enabled
    */
   MobileAppBanner.isEnabled = isEnabled;
    ...
    */
   MobileAppBanner.isHidden = isHidden;
    ...
    * @memberof ic-core.widget.MobileAppBanner
    */
   MobileAppBanner.unhide = function() {
