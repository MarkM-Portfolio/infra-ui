...
(function() {
    ...
    * </pre>
    *
    * @mixin lconn.core.app._DeclarativeLayoutMixin
    * @author Claudio Procida <procidac@ie.ibm.com>
    */
   dojo.provide("lconn.core.app._DeclarativeLayoutMixin");

   function add(scene, prop, node) {
      if (dojo.isArray(node))
         return dojo.forEach(node, dojo.partial(add, scene, prop));
      // Return immediately if caller passes no argument
      if (!scene || !scene[prop] || !node) return;
      scene[prop].appendChild(node.domNode || node);
   }

   dojo.declare("lconn.core.app._DeclarativeLayoutMixin", null, /** @lends lconn.core.app._DeclarativeLayoutMixin.prototype */ {
      /**
            ...
            page: {
               columns: {
                  left: {
                      ...
                      */
                     add: dojo.partial(add, scene, 'colLeft')
                  },
                  right: {
                     /**
                      ...
                      */
                     add: dojo.partial(add, scene, 'colRight')
                  }
               },
               content: {
                  /**
                   ...
                   */
                  add: dojo.partial(add, scene, 'content')
               }
            }
         });
      }
