...
var loader = dojo.provide("jazz.core.loader");

var SERVICE_BASE_URL = net.jazz.ajax.contextRoot() + "/web/",
...
function LoadJob(synchronous, modules) {
		...
		this.start = function() {
			...
			if (net.jazz.ajax._isGadgetAdapter)
				...
				request._proxyURL = net.jazz.ajax.contextRoot();
		...
		function onError(error) {
			try {
				error.log = false;
				console.error(dojo.fromJson(error.responseText).error, "");
			} catch (e) {
				console.error(error);
			}
			finish();
...
loader._waitForCSS = function(styleSheet, lastRuleCount){
	try {
		...
		if (cssRules.length > lastRuleCount) {
			setTimeout(function() {
				jazz.core.loader._waitForCSS(styleSheet, cssRules.length);
			}, 200);
		}
	}
	catch (exc) {
		setTimeout(function() {
			jazz.core.loader._waitForCSS(styleSheet, -1);
		}, 200);
	}
}
