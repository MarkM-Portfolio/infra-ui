...
define([
      "dojo",
      "dojo/dom-style",
      "dojo/text!ic-core/templates/FilteringCheckbox.html",
      "dojo/regexp",
      "dojo/query",
      "dojo/dom",
      "dojo/_base/declare",
   ...
   function(dojo, domStyle, template, regexp, query, dom, declare, domClass, domConstruct, lang, has, string, topic, _Templated, _Widget, HTMLUtil, Res, bidiUtilModule) {

      /**
       * Legacy widget to filter and select a list of items used in Activities.
       * This widget allows single or multiple selection based on the value of
       * the <code>isSingleSelect</code> property. Each time an item is
       * selected, the <code>personSelected()</code> event is fired, with two
       * arguments: the item selected, and a flag set to <code>true</code> if
       * the element is selected, <code>false</code> if deselected.
       * <p>
       * The widget is hidden by default <em>(what were they thinking?)</em>
       * and must be displayed by calling its method <code>getList()</code>.
       * Callers must set a <code>feedFunction</code> member function that
       * accepts a page number as first argument, and a callback as second
       * argument. The callback accepts a JSON object with a <code>names</code>
       * member, consisting of an array of objects with a <code>name</code>,
       * <code>userid</code> and an optional <code>disabled</code>
       * properties.
       * <p>
       * If the property <code>groupOptions</code> is set to an array of
       * objects with a <code>name</code> and <code>value</code> properties,
       * it shows a dropdown. Callers can connect to the
       * <code>groupChanged()</code> event to be notified of the change and
       * receive the selected group as argument. Note that the widget itself
       * won't do anything when the user chooses a different group. It's up to
       * the caller to do something &mdash; typically set a different
       * <code>feedFunction</code>.
       * 
       * @class ic-core.FilteringCheckbox
       * @extends dijit._Widget
       * @extends dijit._Templated
       * @extends ic-core.Res
       */

      var bidiUtil = bidiUtilModule;

      // Widget declaration object
      var w = /** @lends ic-core.FilteringCheckbox.prototype */
      {
      ...
      w.controlsLegend = "";
      /**
       * function that takes the current page number and a callback that will
       * give a feed of members to select from
       */
      w.feedFunction = null;
      ...
      /**
       * Array of objects that will be used for the group selector. If null,
       * group selector will be hidden. Object form: name - group name to
       * display value - value to associate with the displayed name
       */
      w.groupOptions = null;
      ...
      w.focusPrevItem = function focusPrevItem() {
         ...
         if (this.index == 0) {
            ...
            var current = dom.byId(this.indexingList[this.index]);
            this.index--;

            // Get what should be the next item
            var newItem = dom.byId(this.indexingList[this.index]);
      ...
      w.getCurrentSelected = function getCurrentSelected() {
         var listOfChecks = query("input[type='checkbox']", this.notifyList_AP);
      ...
      w.getList = function getList() {
         if (!this.gotList) {
            ...
            this.feedFunction(this.currPage, lang.hitch(this, "parsePeople"));
         }
      ...
      w.setDisabled = function setDisabled(disabled) {
         ...
         if (disabled) {
            domClass.add(this.notifyList_AP, "disabled");
            if (buttons)
               domClass.add(buttons, "lotusHidden");
         }
         else {
            domClass.remove(this.notifyList_AP, "disabled");
            if (buttons)
               domClass.remove(buttons, "lotusHidden");
         }
      ...
      w.reset = function reset() {
         ...
         domClass.add(this.filterTextbox_AP, "lotusInactive");
         this.oldTextValue = "";
         if (this.filterTimer) {
         ...
         if (this.groupOptions) {
            ...
            query(".lotusSearch", this.domNode).addClass("hasGroup");
         }
      ...
      w.selectItem = function selectItem(itemObj, selected) {
         var item = dom.byId(this.id + "notifyCheckbox" + itemObj.uuid);
         if (item) {
      ...
      w.readySearch = function readySearch(event) {
         if (domClass.contains(this.filterTextbox_AP, "lotusInactive")) {
      ...
      w.timedFilter = function timedFilter(event) {
         domClass.remove(this.filterTextbox_AP, "lotusInactive");
         if (this.filterTimer) {
            // reset the timer when the user types so we don't search too often
            window.clearTimeout(this.filterTimer);
            this.filterTimer = null;
         }
         this.filterTimer = window.setTimeout(lang.hitch(this, 'filterNames'), this.autoFiltertime);
      };
      ...
      w.showList = function showList(feed) {
         ...
         if (feed) {
            ...
            if (searchString) {
               ...
               searchString = regexp.escapeString(searchString);
            ...
            if (feed.hasNext || feed.hasPrev) {
               ...
               if (feed.startIndex && feed.totalResults) {
                  ...
                  pagingText = string.substitute(this.rs_pagepos, [
                        pageStart,
                        pageEnd,
                        feed.totalResults
                  ]);
               ...
               link.onclick = lang.hitch(this, "notifyNext");
               ...
               link.href = "javascript:void(0)";
               link.onclick = lang.hitch(this, "notifyPrev");
               // RTC 63768 Added role button
               link.setAttribute("role", "button");
               ...
               if (feed.hasPrev) {
                  ...
                  domClass.add(prev, "lotusFirst");
               }
               if (feed.hasNext) {
                  buttons.appendChild(next);
                  if (!feed.hasPrev) {
                     domClass.add(next, "lotusFirst");
                  }
               }
               this.notifyPaging_AP.appendChild(buttons);
               this.notifyPaging_AP.appendChild(pages);
               domClass.remove(this.notifyPaging_AP, "lotusHidden");
            }
            else {
               domClass.add(this.notifyPaging_AP, "lotusHidden");
            }

            // update accessibility text
            this.access_AP.innerHTML = string.substitute(this.resBundle.rs_numResults, [
                  numResults,
                  totalResults
            ]);
         }
      ...
      w.createOption = function createOption(person) {
         ...
         var node, opt, label;
         var id = userid;
         // this class has a bug where it takes in a userid, but gives a uuid.
         // Fix this madness by sending both.
         person.uuid = userid;
         ...
         label.innerHTML = bidiUtil.enforceTextDirection(HTMLUtil.escapeText(username));
         label.htmlFor = opt.id;
         label.id = opt.id + "Label";
         label.className = "lotusCheckbox";
         domStyle.set(label, "position", "static");
         ...
         if (this.peopleSelected[userid]) {
            if (person.disabled) {
               ...
               // SPR #DJOS7NWRZ2: for IE checked has to be set after adding to
               // dom
               opt.checked = true;
            }
         }
      ...
      w.setValue = function setValue(event) {
         ...
         if (this._currPeople[event.target.value]) {
            ...
            // this should not happen - somehow a person was selected that we
            // don't have an entry for
            person = {
      ...
      var FilteringCheckbox = declare("lconn.core.FilteringCheckbox", [
            _Widget,
            _Templated,
            Res
      ], w);
