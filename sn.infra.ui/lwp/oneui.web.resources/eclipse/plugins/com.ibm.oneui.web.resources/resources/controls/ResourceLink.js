 ...
 *******************************************************************************/
(function(){
dojo.provide("com.ibm.oneui.controls.ResourceLink");
...
dojo.require("com.ibm.oneui.util.html");
dojo.require("com.ibm.oneui.util.ui");

dojo.require("dojo.i18n");
dojo.requireLocalization("com.ibm.oneui.controls", "ResourceLink");
...
var messages = dojo.i18n.getLocalization("com.ibm.oneui.controls", "ResourceLink");

/* For requests originating in the same domain, provide an imitation postMessage behavior. */
if (!window.postMessage) {
	window.onmessage = function() {}; // if we can't access the parent, this will throw an exception
	window.postMessage = function(message, options, source) {
		dojo.byId("output").innerHTML = "postMessage: "+message+" source.location="+source.location;
		window.onmessage({data:message, source: source});
	}
}

/**
 * @class com.ibm.oneui.controls.ResourceLink
 * @author Clayton Coleman <claycole@us.ibm.com>
 */
var com_ibm_oneui_controls_ResourceLink = dojo.declare("com.ibm.oneui.controls.ResourceLink", dijit._Widget, /** @lends com.ibm.oneui.controls.ResourceLink.prototype */ {
	...
	//		(optional) useAbbreviation: (boolean) If set to true, the text of the
	...
	//				false.
	//		(optional) retainLinkText: (boolean) If set to false, the text in the 
	...
	//		(optional) iconNode: (DOM Node reference or id) The DOM node in the page
	...
	//				node.
	//		(optional) lazyFetch: (boolean) If set to true, the Resource Link will
	...
	// methods:
	//
	//		refresh: if the resource that the Resource Link is viewing has changed,
	...
	/**
	 * Called when the compact rendering document has loaded. Will be called once per successful
	 * load in the event of calls to refresh.
	 */
	onLoad: function(){},
	
	/**
	 * Returns the title of the resource from the compact rendering document or null if no title
	 * is defined or the document has not been loaded yet.
	 */
	getTitle: function() {
	...
	buildRendering: function(){
		...
		if (!this.uri.match(/http(?:s)?\:\/\//)) {
			var baseUri = document.baseURI || document.URL; //document.URL is used to acquire the base URI in IE
			this.uri = new dojo._Url(baseUri, this.uri).toString();
		}
		this.domNode.href = this.presentationUri || this.uri;
		if (this.label)
			this.domNode.innerHTML = com.ibm.oneui.util.html.removeScript(this.label);
		this._styleLink();
	},
	...
	postCreate: function() {
		...
		this.popup = new com.ibm.oneui.controls._RichHover({
			...
			onDisable: dojo.hitch(this,this._disable),
			onLoad: this.onLoad
		});
	},
	...
	_styleLink: function () {
		...
		dojo.addClass(domNode, CSS_RL);
		var child = domNode.firstChild;
		if (child != domNode.lastChild || (child.nodeName != "IMG" && child.nodeName != "img")) {
			this.arrowNode = dojo.create("img", {
				className: CSS_ARROW, 
				src: dojo.moduleUrl("com.ibm.oneui", "controls/graphics/rh-twistie.gif")
			}, domNode);
	...
	_disable: function () {
		...
		dojo.removeClass(this.domNode, CSS_RL);
		delete this.popup;
	},
...
com_ibm_oneui_controls_ResourceLink.setLazyFetch = function(bool) {
	defaultLazyFetch = bool;
};

com_ibm_oneui_controls_ResourceLink.register = function(provider) {
	_providers.push(provider);
}

com_ibm_oneui_controls_ResourceLink.getCompactRepresentation = function(uri, link) {
...
dojo.declare("com.ibm.oneui.controls._RichHover", com.ibm.oneui.controls.HoverDialog, {
	programmatic: false,
	...
	createContents: function() {
		...
		if (!oslcCompact)
			oslcCompact = com_ibm_oneui_controls_ResourceLink.getCompactRepresentation(this.uri, this.params.around);

		if (!oslcCompact) {
			if (this.lazyFetch) {
				this._fetchCompactRendering();
			}
			return dojo.create("div", {innerHTML: messages.loading});
		} else {
	...
	onOpen: function() {
		//ADDED: if the control is repeatedly resized, open should revert us back to the original size
	...
	updateToggleLink: function(showMore) {
		if (this._presentation) {
			dojo.addClass(this._presentation.toggleLink, showMore ? "showMore" : "showLess");
			dojo.removeClass(this._presentation.toggleLink, showMore ? "showLess" : "showMore");
	...
	displaySmallSummary: function(){
		...
		if (this.oslcCompact.smallPreview) {
			// update the iframe dimensions early to give an accurate size for the HoverPopup.position function
			this._updateIFrame();
	...
	_renderResult: function() {
		if (this.oauthRequired) {
			...
			var loginLink = com.ibm.oneui.util.internal.login.insertLoginLink(this.content, this.oauthToken);
			this.connect(loginLink, "onclick", dojo.hitch(this,this._fetchCompactRendering,true));*/
		} 
	...
	_updatePresentation: function() {
		// this function happens to work in both legacy mode (compact rendering) and in oslc ui preview mode
		...
		if (pres) {
			...
			if (cr.icon) {
				...
				dojo.removeClass(pres.iconLink, "lotusHidden");
				dojo.addClass(pres.titleWrapper, "titleIndent");
			} else {
				pres.iconNode.src = "";
				dojo.addClass(pres.iconLink, "lotusHidden");
				dojo.removeClass(pres.titleWrapper, "titleIndent");
			}
			if (cr.showExpand) {
				dojo.removeClass(pres.bottomSection, "lotusHidden");
			} else {
				dojo.addClass(pres.bottomSection, "lotusHidden");
			}
		}
	},
	...
	_createPresentation: function(node) {
		...
		if (!this.oauthRequired && !this._presentation) {
			var presentation = this._presentation = new com.ibm.oneui.controls._RichHoverPresentation({
				toggleSize: dojo.hitch(this,this.toggleSize)
			});
			this._updatePresentation();
	...
	_fetchCompactRendering: function(allowOauthInterceptor, e) {
		...
		dfd.addCallback(dojo.hitch(this,function(dfd,response) {
			var doc;
			if (dfd.ioArgs.xhr.getResponseHeader("Content-Type").substring(0,CT_OSLC_COMPACT.length) === CT_OSLC_COMPACT) {
				doc = {oslcCompact: parseOslcCompactXML(dojox.xml.DomParser.parse(response))};
			}
			// if this is not an oauth request, it is a lazy fetch, handle as lazy
		...
		dfd.addErrback(dojo.hitch(this, function (err) {
			// if the deferred was cancelled, the user cancelled login, no action to take
			if (!err || err.dojoType !== "cancel") {
				/*if (err && err.status === 401) {
					// this is only for lazy fetch mode
					// when we are in regular mode and are using this path for auth we will either have authed or will hit err.dojoType === "cancel"
					var token = new dojo._Url(this.uri).authority;
					this.setOauthRequired(token);
					this.aggregatorId = com.ibm.oneui.controls.ResourceLinkAggregator().registerOauth(token, this.uri, dojo.hitch(this,this._handleLazyFetchJson), dojo.hitch(this,this.disable,messages.infoNotAvailable));
				} else*/ {
					this.disable(messages.infoNotAvailable);
				}
			}
		...
		if (allowOauthInterceptor) {
			// After we authenticate with OAuth, we poke the ResourceLinkAggregator to resend
		 	// an xhr request for all ResourceLinks that access the same server.
			dfd.addBoth(dojo.hitch(this, function(response) {
				// if the deferred was cancelled, the user did not login, do not poke the aggregator
				if (response && response.dojoType === "cancel")
					return response;
				/*var aggregator = com.ibm.oneui.controls.ResourceLinkAggregator();
				if (this.aggregatorId) {
					// exclude ourself from the aggregator call since we already have the result
					var obj = aggregator.unregisterOauth(this.oauthToken, this.uri, this.aggregatorId);
					// react to the result
					if (response && (response.compactRendering || response.oslcCompact)) {
	...
	_updateIFrame: function() {
		...
		// HoverPopup will treat a width greater than viewport - 60 as viewport - 60
		// update the effective widths so the iframe is sized according to the max
		...
		if (oslc.smallPreview) {
			...
			if (dojo.isIE < 8 || this.effectiveSmallWidth + 5 > 500)
		...
		if (oslc.largePreview) {
			...
			if (dojo.isIE < 8 || this.effectiveLargeWidth + 5 > 500 && (this.effectiveLargeWidth > this.effectiveSmallWidth || !this.effectiveSmallWidth))
		...
		if (w) {
			...
			if (dojo.isIE) {
				// IE needs explicit width on the divider and width 100% breaks it out the side
		...
		// HoverPopup will treat a height greater than viewport - 30 as viewport - 30
		// update the effective widths so the iframe is sized according to the max
		// account for the 45 px (+ titleBar height) vertical padding/content around the iframe using heightOffset
		...
		if (oslc.largePreview) {
			var largeHeight = this.dynamicLargeHeight || (oslc.largePreview.initialHeight && this.dynamicSmallHeight) || parseLength(oslc.largePreview.hintHeight);
...
/*	_onKeyPress: function(e) {
	...
	_onBodyClick: function(e) {
		...
		if (target && !dojo.isDescendant(target, this._getMasterPopup().domNode))
			this.close();
	},
	...
	_onMessage: function(e) {
		var data = e.data;
		// changed from !== on window to != window, IE6 recognizes the two are equal
		if (!data || !this.oslcCompact || !this._presentation || e.source != this._presentation.hoverIframe.contentWindow ||
				(!resizablePreviews &&
						((this._isFullDisplay && typeof (this.oslcCompact.largePreview.initialHeight) === "undefined") ||
						(!this._isFullDisplay && (typeof (this.oslcCompact.smallPreview.initialHeight) === "undefined")))))
			return;
...
dojo.declare("com.ibm.oneui.controls._RichHoverPresentation", [dijit._Widget, dijit._Templated], {
	templatePath: dojo.moduleUrl("com.ibm.oneui", "controls/templates/ResourceLink.html"),
	messages: messages,
	
	postCreate: function() {
		if (dojo.isIE){
			dojo.addClass(this.loadingImage, "lotusHidden");
			dojo.removeClass(this.hoverIframe, "loadingIframe");
		}
	},
	
	_contentLoaded: function(e){
		//Not checking for IE becaues there is a bug in IE where iframe contents are not shown
		//when "visibility: hidden" is removed from the iframe's CSS attribute
		if ( e.type === "load"){
			dojo.addClass(this.loadingImage, "lotusHidden");
			dojo.removeClass(this.hoverIframe, "loadingIframe");
		}
...
var processCompactRendering = function(results) {
	...
	if (results.title)
		results.title = com.ibm.oneui.util.html.removeScript(results.title);
...
var parseOslcCompactXML = function(oslcXml) {
	var oslcObj = {}, nodes,
		titleNodes = oslcXml.getElementsByTagNameNS("title", XMLNS_DC_TERMS),
		shortTitleNodes = oslcXml.getElementsByTagNameNS("shortTitle", XMLNS_OSLC),
		iconNodes = oslcXml.getElementsByTagNameNS("icon", XMLNS_OSLC),
		smallPreviewNodes = oslcXml.getElementsByTagNameNS("smallPreview", XMLNS_OSLC),
		largePreviewNodes = oslcXml.getElementsByTagNameNS("largePreview", XMLNS_OSLC);
...
var parseOslcPreviewElement = function(node, result, elementName) {
	...
	if (nodes.length > 0 && nodes[0].childNodes.length > 0)
		result[elementName] = dojo.trim(nodes[0].childNodes[0].nodeValue);
};
...
var parseLength = function(length) {
	...
	if (em > 0) return Math.ceil(length * getEmSize());
	if (ex > 0) return Math.ceil(length * getExSize());
...
var getEmSize = function() {
	if (!_emSize) {
		_emSize = com.ibm.oneui.util.ui.measureEm(getTestNode());
...
var getExSize = function() {
	if (!_exSize) {
		_exSize = com.ibm.oneui.util.ui.measureEx(getTestNode());
		// clear the cache when finished rendering
		setTimeout(function() {
			_exSize = null;
		},0);
...
var getTestNode = function() {
	if (!_testNode) {
		_testNode = dojo.create("div",{style: {visibility: "hidden", fontSize: "133.3333333%"}},dojo.body());
	}
	return _testNode;
};
