...
{
	var exports_ = {},
		location_ = dojo.getObject("location", dojo.global).toString(),
	...
	var ensurePlacementDomId_ = function (widgetSpec) {
		...
		if (placement && !dojo.isString(placement)) {
			if (!placement.id) {
		...
		if (isGadget_(widgetSpec)) {
			...
			var userPrefs = renderParams.userPrefs || (renderParams.userPrefs = {});
			 ...
			 if (lconn.core.globalization.api) {
				 userPrefs['lconn.core.globalization.api.calendar'] = lconn.core.globalization.api.getCalendar();
				 userPrefs['lconn.core.globalization.api.textDirection'] = lconn.core.globalization.api.getTextDirection();
				 userPrefs['lconn.core.globalization.api.bidiEnabled'] = lconn.core.globalization.api.isBidiEnabled();
			 }
