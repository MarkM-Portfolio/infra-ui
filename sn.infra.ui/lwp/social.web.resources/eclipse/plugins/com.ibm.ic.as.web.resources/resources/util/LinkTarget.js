...
define([
...
], function(dojo, declare, lang, domAttr, query, enablement, ConfigManager, ActivityStreamSearchHashtag) {
	...
	var LinkTarget = declare("com.ibm.social.as.util.LinkTarget", null, {
		...
		modifyContentLinkTargets: function(newsFeedFragment) {
			query(".asLinkContainer", newsFeedFragment)
				.query('a').forEach(lang.hitch(this, function(node, index) {
					// Do not set a target:
					...
					if (!href || (href && href.indexOf("javascript:") != 0 && !href.match(filesRegexp))) {
						domAttr.set(node, "target", this.getLinkTarget());
						domAttr.set(node, "rel", "nofollow noopener noreferrer");
					}
					href = null;
					node = null;
				}));
			...
			//only run if enabled
			if (enablement.checkEnablement(enablement.AS_HASHTAG_SEARCH)) {
		...
		modifyHashtagSearchLinks: function(domNode) {
			var hashtagLinks = query("a.hashtagSearchLink", domNode).forEach(lang.hitch(this, function(node, index) {
				if (node) {
		...
		handleHashtagSearch: function(e) {
			var target = e.target;
			var tagName = domAttr.get(target, "tag");
			ActivityStreamSearchHashtag.getInstance().addSearchTag(tagName);
		},
		...
		getLinkTarget: function() {		
			return ConfigManager ? ConfigManager.getLinkTarget() : this.linkTarget;
		}
	});
