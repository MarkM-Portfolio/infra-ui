...
define([
      ...
      "ic-ui/layout/people", // FIXME: core must not depend on coreui
      "ic-core/config/services"
],
   function(cache, number, string, dojoConfig, array, lang, has, template, people, services) {
      ...
      /**
       * @namespace ic-core.util.text
       */
      return /** @lends ic-core.util.text */ {
         CODE_MAP : [
               [
                     0x0080,
                     1
         ...
         /**
          * Trims a string to a string returnValue whose byte length is not
          * greater than length, and returns it
          * 
          * @param {String}
         ...
         trimToByteLength : function(str, length) {
            ...
            var codeMap = this.CODE_MAP;
            if (length <= 0) {
               return "";
            }
            var i = 0, lastValid = 0, len = str.length;
            var realLen = 0, code, b;
            while (i < len) {
               if (realLen >= length) {
         ...
         trimToLength : function(s, length, wordLength) {
            ...
            var ellipsis = "...";
            if (wordLength > 0) {
               var i, l, words = s.split(/[\s\u3000]/);
               for (i = 0; i < words.length; i++) {
                  if (words[i].length > wordLength) {
                     l = s.indexOf(words[i]) + wordLength;
                     if (l > length) {
                        break;
                     }
         ...
         /**
          * Formats a size in bytes. This method can accept one or two
          * arguments. If only one argument is provided, it will be used as size
          * to format. If two arguments are provided, the first will be used as
          * a string bundle for sizes, and the second will be used as the size
          * to format.
          * <p>
          ...
          * <dd>nls.TB</dd>
          * <dd>String to format size in TB</dd>
          * </dl>
          * 
          * @param {Object|Number}
          *           nls A string bundle for templates. If the second argument
          *           is omitted, it will be interpreted as the size to format.
          * @param {Number}
          *           [size] The size to format.
          * @returns {String} A formatted size
          */
         formatSize : function(nls, size) {
            ...
            if (size > 10 * ONE_TB && nls.TB) {
               return string.substitute(nls.TB, [ number.format(Math.floor(size * 10 / ONE_TB) / 10)
               ]);
            }
            if (size >= ONE_TB && nls.TB) {
               return string.substitute(nls.TB, [ number.format(Math.floor(size * 100 / ONE_TB) / 100)
               ]);
            }
            if (size > 10 * ONE_GB) {
               return string.substitute(nls.GB, [ number.format(Math.floor(size * 10 / ONE_GB) / 10)
               ]);
            }
            if (size >= ONE_GB) {
               return string.substitute(nls.GB, [ number.format(Math.floor(size * 100 / ONE_GB) / 100)
               ]);
            }
            if (size > 100 * ONE_MB) {
               return string.substitute(nls.MB, [ number.format(Math.floor(size / ONE_MB))
               ]);
            }
            if (size > 10 * ONE_MB) {
               return string.substitute(nls.MB, [ number.format(Math.floor(size * 10 / ONE_MB) / 10)
               ]);
            }
            if (size >= ONE_MB) {
               return string.substitute(nls.MB, [ number.format(Math.floor(size * 100 / ONE_MB) / 100)
               ]);
            }
            if (size > 1024) {
               return string.substitute(nls.KB, [ number.format(Math.floor(size / 1024))
               ]);
            }
            return string.substitute(nls.KB, [ number.format(size > 0 ? 1 : 0)
            ]);
         },

         /**
          * Return the length of a string, the javascript self-contained
          * attribute "string.length" seems have bug on some special UTF-8
          * characters.
          * 
          * @param {String}
         ...
         length : function(s) {
            ...
            while (true) {
               ...
               if (idx2 == -1) {
                  ...
                  topByte = parseInt(t.substr(idx2 + 1, 2), 16);
                  if (topByte < 0xC0) {
                     ret += 1;
                     idx1 = idx2 + 3;
                  }
         ...
         lengthUtf8 : function(s) {
            ...
            for (i = 0; i < length; i++) {
               ...
               if (c <= 127) {
                  ...
                  bytes += 3;
                  // surrogate pairs in a character input stream are between
                  // D800 and DFFF -
         ...
         /**
          * Return the first index in the string that exceeds the maximum number
          * of UTF-8 bytes into the string. Used to trim a string to below a
          * certain length in UTF-8 bytes.
          * <p>
          ...
          * @param {boolean}
          *           replaceSpaces replace groups of spaces by non-breaking
          *           spaces (except the last space)
            ...
            array.forEach([
               ...
               while (match = regex.exec(str)) {
                  ...
                  if (regex == TEXT_MENTION_REGEX) {
                     ...
                     displayName = match[2];
                     name = shouldNotify ? '@' + displayName : displayName;
                     userid = match[1];
                  }
                  // HTML_MENTION_REGEX
                  else {
                  ...
                  var href = people.getProfileUrl({
                     name : displayName,
                     userid : userid
                  });
                  var l = string.substitute(template, {
                     url : href,
                     name : name,
                     userid : userid,
                  ...
                  // if debug=true bizCard HTML comments were causing blank
                  // space.
                  // use regex and trim to remove
                  if (dojoConfig.isDebug) {
                     l = lang.trim(l.replace(/<!--[\s\S]*?-->/g, ""));
                  }
