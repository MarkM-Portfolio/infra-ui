...
define([
	...
	"ic-core/locale"
], function (dojo, kernel, localeModule, i18n, i18ngregorian, i18nstrings, string, Res, BidiDateUtil, coreLocale) {

	/**
	 * @namespace ic-core.DateUtil
	 */
	// Documentation:
	// https://w3.webahead.ibm.com/w3ki/display/conndev/Common+date+printing+code
	var w = {}, bdu = BidiDateUtil;
	 ...
	 * resource strings.
	 * 
	 * The locale used is determined by checking kernel.locale. If this isn't set,
	...
	w.toStringForTimeinMs = function(dateInMS, dontshowtime, showFullYear) {
	   if (this.Days == null)
	      this.Days = i18ngregorian['days-format-wide'];
	   if (this.Month == null)
	      this.Month = i18ngregorian['months-format-abbr'];
	   //bidi support
	   if(!bdu.isGregorian())
	   ...
	   if (dayDelta <= 2 && dayDelta >= -2) {
	      ...
	      var lcl = coreLocale, lang = lcl.getLanguage(), country = lcl.getCountry();
	...
	w.getLocalizedTime = function(date) {
	   return localeModule.format(date, {
	      selector : 'time',
	      formatLength : 'short',
	...
	w.getLocalizedDate = function(date) {
	     //bidi support
	   if(bdu.isGregorian())
	      return localeModule.format(date, {selector : 'date', formatLength : 'medium', locale : kernel.locale});
	   else
	      return bdu.formatBidiDate(date, {selector: "date", formatLength: "medium", locale: kernel.locale});
	};
	...
	w.getLocaleString = function(date) {
	   ...
	   if(bdu.isGregorian())
	      ...
	      return bdu.formatBidiDate(serverDate, {selector: "date", formatLength: "medium", locale: kernel.locale});
	};
	 ...
	 */
	w.atomDateToJsDate = function atomDateToJsDate(dateString) {
	   var parsedDate = null;
	   var s = dateString;
	 ...
	 * @memberof ic-core.DateUtil
	 * 
	 * @see core.DateUtil.toString
	 * 
	 ...
	 *         parsed
	 */
	w.AtomDateToString = function AtomDateToString(dateString,
	      dontShowTime) {
		var date = w.atomDateToJsDate(dateString);
		//bidi support
		var iDate = new Date(dateString);
		date = iDate.getDay() && !date ? iDate : date;
	   return date ? w.toString(date, dontShowTime) : dateString;
	};
