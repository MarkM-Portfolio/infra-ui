...
], function(declare, lang, request, AutocompleteDataStore) {

   function quoteRegExp(str) {
      return str.replace(/([.?*+^$[\]\\(){}-])/g, "\\$1");
   }

   var PeopleDataStore = declare("lconn.core.widget.autocomplete.PeopleDataStore", AutocompleteDataStore, {

      constructor : function() {
         var people = lang.getObject("com.ibm.social.layout.people");
      ...
      // 4) If this is a paged result, the start member of the array is set to
      // the index of the first item in the
      // overall listing
      // 6) Each item is an object which obeys the constraints defined on
      // lconn.core.widget.autocomplete.person
      // This member may be set to null to imply that no transformation must be
      ...
      convert : function(kwArgs, response) {
         ...
         // Directory searches do not have stable sort orders, so we load a
         // large page of results
      ...
      getConnectionsPhotoUrl : function(item, defaultValue) {
         ...
         return com.ibm.social.layout.people.getImageUrl({
            userid : userid
         }, 32);
      },
      ...
      getCacheKey : function(kwArgs) {
         var s = kwArgs.queryOptions.directory ? ("d_") : ("f_" + (kwArgs.start || 0) + "_" + kwArgs.count + "_");
         s += kwArgs.query.toLowerCase();
         return s;
      }

   /*
    * getUrl: function(kwArgs) { return url; }
    */
   });
