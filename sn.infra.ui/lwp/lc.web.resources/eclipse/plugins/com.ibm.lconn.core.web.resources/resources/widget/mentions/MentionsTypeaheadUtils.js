 ...
 * 
 * @mixin lconn.core.widget.mentions.MentionsTypeaheadUtils
 * @author Piyush K Agarwal <pagarwal@us.ibm.com>
 */
dojo.declare("lconn.core.widget.mentions.MentionsTypeaheadUtils", null, /** @lends lconn.core.widget.mentions.MentionsTypeaheadUtils.prototype */
{
   ...
   registerActivatorType : function(mentionsType) {
      ...
      var isRegistered = dojo.some(this._registeredTypes, function(item) {
   ...
   unregisterActivatorType : function(type) {
      ...
      dojo.forEach(this._registeredTypes, function(item, i) {
         if (item._type == type) {
   ...
   isTrackedKeyCode : function(e) {
      ...
      if (!this.disableMentions && this._registeredTypes) {
         dojo.some(this._registeredTypes, function(item) {
   ...
   handleTypeaheadKeyInput : function() {
      ...
      if (pw) {
         ...
         if (highLightedOpt) {
            ...
            if (item) {
               if (dojo.indexOf([ activeFieldID + "_popup_searchDir", activeFieldID + "_popup_resultsNode" ], highLightedOpt.id) === -1) {
