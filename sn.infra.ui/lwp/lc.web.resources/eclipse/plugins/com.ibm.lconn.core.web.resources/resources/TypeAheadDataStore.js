...
dojo.declare("lconn.core.TypeAheadDataStore", null, {
   constructor : function(/* Object */keywordParameters, node) {
      this.queryParam = (keywordParameters.queryParam ? keywordParameters.queryParam
            : node.getAttribute("queryParam"));

      this.url = (keywordParameters.url ? keywordParameters.url
   ...
   fetch : function(keywordArgs) {
      ...
      this.searchDirectory = (keywordArgs.queryOptions.searchDirectory ? true
            : false);

      // If we don't test typeof here, then cache["sort"] will
      // return true because sort() is a function on arrays!
      // #DJOS7S8PNY
      if (typeof this.cache[keywordArgs.query.toLowerCase()] == "object") {
         // Todo: In this case, we run onComplete before returning,
         // unlike the other case.
         // Is this a problem?
         keywordArgs.onComplete(this.cache[keywordArgs.query
               .toLowerCase()], keywordArgs);
         return keywordArgs;
      }

      if (this.queryParam) {
         if (this.getUTF8Length(keywordArgs.query) > this.maxQueryLength) {
            var queryKeyWord = this
                  .getUTF8Substring(keywordArgs.query);
