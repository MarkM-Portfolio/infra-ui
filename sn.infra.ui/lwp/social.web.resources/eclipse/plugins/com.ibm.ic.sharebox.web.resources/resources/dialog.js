...
define([
...
], function (lang, topic, ShareboxDialog) {
	...
	(function(){
	   ...
	   sbd._getInstance = function(ctx) {
		      ...
		      if(!sbd._isGlobal(ctx)) {
		         ...
		         if(!inst) {
		            inst = sbd._ctxInstance = new ShareboxDialog(ctx);
		    		topic.publish(sbd.INIT_CTX_COMPLETE);
		         }
		      }
		      else {
		         inst = sbd._instance;
		         if(!inst) {
		    	    inst = sbd._instance = new ShareboxDialog(ctx);
		    	    topic.publish(sbd.INIT_GLOBAL_COMPLETE);
		         }
		      }
		      return inst;
	   };
	   ...
	   sbd._openDialog = function(ctx) {
	       topic.publish("com/ibm/social/sharebox/dialog/opening");
	      ...
	      if(!sbd._showInProgress) {
	         ...
	         if(!sbd._instanceOpen()) {
	        	if(!sbd._initInProgress) {
	        		...
	        		topic.subscribe(sbd._isGlobal(ctx) ?
	        				sbd.INIT_GLOBAL_COMPLETE : sbd.INIT_CTX_COMPLETE, lang.hitch(null, function(sbd, ctx) {
	        					sbd._openDialog(ctx);
	        					sbd._showInProgress = false;
	        				}, sbd, ctx));
	        	}
	...
	return com.ibm.social.sharebox.dialog;
});
