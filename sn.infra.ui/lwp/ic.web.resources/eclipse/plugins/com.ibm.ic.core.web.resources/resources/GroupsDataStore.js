...
define([
      ...
      "dojo/request"
], function(dojo, declare, lang, sorter, request) {

   /**
    * Data store for groups
    * 
    * @class ic-core.GroupsDataStore
    */

   var GroupsDataStore = declare("lconn.core.GroupsDataStore", null, /** @lends ic-core.GroupsDataStore.prototype */
   {
      ...
      // Groups parameters (these are in addition to the parameters used by
      // PeopleDataStore
      ...
      fetch : function(keywordArgs) {
         ...
         // this.searchDirectory = (keywordArgs.queryOptions.searchDirectory ?
         // true : false);
         ...
         if (typeof cache[keywordArgs.query.toLowerCase()] == "object") {
            ...
            // var endIndex = (keywordArgs.count && (keywordArgs.count !==
            // Infinity)) ? (startIndex + keywordArgs.count) : items.length;
            // keywordArgs.onComplete(items.slice(startIndex, endIndex),
            // keywordArgs);
            // this was needed to allow previous/next actions in addgroups.js to
            // function right otherwise subsequent searches messed up
         ...
         request(this.url, {
            ...
            if (keywordArgs.sort) {
               data.items.sort(sorter.createSortFunction(keywordArgs.sort, this));
            }
            else {
               var defaultSort = [ {
                  attribute : "name",
               ...
               data.items.sort(sorter.createSortFunction(defaultSort, this));
            }
            ...
            // we were asked to trim what we show for group name to 80 chars in
            // type-ahead
            ...
            if (keywordArgs.onComplete) {
               if (this.groupTypeahead) {
                  ...
                  // var endIndex = (keywordArgs.count && (keywordArgs.count !==
                  // Infinity)) ? (startIndex + keywordArgs.count) :
                  // keywordArgs.totalItemCount;
                  // keywordArgs.onComplete(data.items.slice(startIndex,
                  // endIndex), keywordArgs);
               }
               else {
                  keywordArgs.totalItemCount = data.items.length;
