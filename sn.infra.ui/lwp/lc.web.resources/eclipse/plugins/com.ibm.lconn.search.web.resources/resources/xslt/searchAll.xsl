...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="FilePreviewOnly"/>
	...
	<xsl:template name="resultSummary">
		...
		<div class="lotusClear">
			...
			<xsl:choose>
				<xsl:when test="string-length(atom:summary)!=0">
					...
					<span class="lotusMeta lconnSearchHighlight bidiAware">
						<xsl:if test="$resultType = 'files'">
							<xsl:attribute name="class">lconnHideOnSelected lotusMeta lconnSearchHighlight bidiAware</xsl:attribute>
						</xsl:if>
				...
				<xsl:when test="$resultType != 'status_update'">
					<em class="lotusMeta">
						<xsl:if test="$resultType = 'files'">
							<xsl:attribute name="class">lconnHideOnSelected lotusMeta</xsl:attribute>
						</xsl:if>
			...
			<xsl:if test="$resultType = 'files'">
				<span class="lconnShowOnSelected lotusMeta">
					<xsl:value-of select="$FilePreviewOnly"/>
				</span>
			</xsl:if>
		</div>
