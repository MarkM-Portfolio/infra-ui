...
define([
      ...
      "./peopleFinder/directory/PeopleFinderService",
      "./quickResults/QuickResultsService"
],
   function(dojo, string, declare, i18n, lang, domClass, domAttr, i18nSearchBar, template, topic, Menu, MenuItem, _Templated, TextBox, config, servicesModule, TypeAheadManager, PeopleFinderService, QuickResultsService) {

      /**
       * Connections Search Bar widget
       * 
       * @class ic-core.SearchBar
       * @extends dijit._Templated
       * @extends ic-core.peopleFinder.TypeAheadManager
       * @author Ryan Silva <rsilva@us.ibm.com>
       */

      var SearchBar = declare("lconn.core.SearchBar", [
            TypeAheadManager,
            _Templated
      ], /** @lends ic-core.SearchBar.prototype */
      {

         templateString : template,

         // NOTE:
         // Should you introduce a customizable name for the field that contains
         // the query term in SearchBar.js
         // (right now, its name is the immutable "query") it will be "param",
         // and will be written to Javascript
         // by ExternalSearchTag. This a requirement in order for task 8404 to
         // work correctly for Wikis/Files

         // Parameters to be passed in

         // localOptions: array
         // localOptions is an array of objects which will be used to initialize
         // several
         // menu items in the scope drop down of the search bar. Shown are
         // several supported
         // options, however note that the entire object will be passed in as
         // the parameter
         ...
         // method: "GET" (or "POST")
         //
         // //Boolean, specify true for one of your options to make it the
         // default one
         // // If no default option is true, "all connections" will be the
         // default.
         // defaultOption: true
         // } ]
         localOptions : [],
         ...
         // thirdPartySearchEngines is an array of objects which will be used to
         // initialize several
         // menu items in the scope drop down of the search bar. Shown are
         // several supported
         // options, however note that the entire object will be passed in as
         ...
         // globalOptions is a private internal array. It contains objects which
         // describe both the
         // label and feature keyword for each application which is available to
         // search.
         // There is no need to specify it as constructor argument.
         globalOptions : [],

         // showAllFeatures: boolean
         // This should be set to true if you want to have the global options
         // array populated
         // with all features that are installed in Connections.
         showAllFeatures : false,

         // localAction: string
         // The URL to which this form should post if one of the localOptions
         // are selected
         // Posts two parameters: query and scope (scope is set by
         // selectedOption.scope)
         localAction : '',

         // searchContextPath: string
         // The context path of the common search app. For example, "/search".
         // You should get this value from lotusConnections-config.xml, by
         // calling
         // (in java) something like this:
         // ConnConfig.INSTANCE.getServerURL("search", "");
         searchContextPath : '',

         // STRINGS
         _strings : i18nSearchBar,
         ...
         // globalAction: string
         // You don't need to pass this in. It will be computed from search
         // context path.
         // The URL to which this form should post if one of the global options
         // are selected
         // Posts two parameters: query and component (component is set by
         ...
         // You don't need to pass this in. It will be computed from search
         // context path.
         // The URL of the advanced search page. The browser will forward here
         // if the advanced
         ...
         SEARCH_ADVANCED_PATH : '/web/jsp/advancedSearch.jsp', // appended to
         // searchContextPath

         // Return focus to menu when an option is selected rather than jumping
         // to the textbox
         ...
         postCreate : function() {
            ...
            // Unless the globalOptions array is empty, only contains a single
            // entry or showAllFeatures is false, populate with all services.
            if (this.globalOptions.length <= 1 && !this.showAllFeatures) {
               this.globalOptions = [];
            }
            else if (this.globalOptions.length < 1 || this.showAllFeatures) {
               var services = servicesModule;
                  ...
                  if (this.featureIcons.hasOwnProperty(featureName)) {
                     ...
                     if (featureInstalled) {
                        this.globalOptions.push({
                           label : this._strings["LABEL_" + featureName.toUpperCase()],
                           feature : featureName
                        });
                     }
                  }
         ...
         getScopeMenu : function() {
            if (!this.scopeMenu) {
               this.scopeMenu = new Menu({
                  "class" : "lotusNavMenu lconnSearchScope",
                  onItemClick : lang.hitch(this, "selectOption")
               });
               ...
               for ( var i in options) {
                  ...
                  this.menuItems.push(new MenuItem(options[i]));
               }

               if (options && options.length && this.globalOptions.length > 0) {
                  this.menuItems.push(new dijit.MenuSeparator({
               ...
               if ((thirdPartyOptions != null) && (thirdPartyOptions.length > 0)) {
                  ...
                  for (var i = 0; i < thirdPartyOptions.length; i++) {
                     ...
                     this.menuItems.push(new MenuItem(thirdPartyOptions[i]));
                  }
               }
         ...
         openMenu : function(evt) {
            ...
            dijit.popup.open({
               ...
               // Specify how the popup should align with the button that was
               // clicked
               ...
               onCancel : lang.hitch(this, '_cancelMenu')
            });

            // Must focus here or else clicking off won't blur and won't cause
            // it to close
            menu.focus();
         ...
         selectOption : function(item, focusTextbox) {
            ...
            if (item.iconClass) {
               domClass.add(this.currScopeIconNode, item.iconClass);
            }
               ...
               if (this.onSubmit()) {
                  if (this.isThirdPartySearchEngineSelected()) {
                     ...
                     if (endsWith(this.formNode.action, this.SEARCH_POST_PATH)) {
                        // Search API uses scope as the name of the parameter to
                        // filter by feature
                        domAttr.set(this.featureInputNode, "name", "scope");
                        if (!this.scopeInputNode.value) {
                           // Disable inputs not used by Search API
                           domAttr.set(this.scopeInputNode, "disabled", true);
                        }
                        if (this.textBox.textbox) {
                           domAttr.set(this.textBox.textbox, "disabled", true);
                        }
                     }
