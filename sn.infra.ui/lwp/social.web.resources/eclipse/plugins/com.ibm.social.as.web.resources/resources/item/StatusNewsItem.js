...
dojo.provide("com.ibm.social.as.item.StatusNewsItem");
...
com.ibm.social.as.item.NewsItem,
{
	...
	postCreate: function(){		
		...
		// CNXSERV-11362
		// Deprecated code. Used to add a link to the timestamp added in an AS news item.
		// This only does it for Status Updates though, which is more confusing than helpful
		//
		// Set appropriate timeago element if Profiles is enabled
		// if (lconn.core.config.services.profiles || lconn.core.config.services.scprofiles) {
		//	nodeType = "a";
		...
		// 		dojo.string.substitute(this.strings.linkToAriaLabel,[this.newsData.getGeneratorDisplayName(), this.appLocalizedDisplayName]): "";
				
		// 	target = this.getLinkTarget();
		// } 
		
		newTimeNode = dojo.create(nodeType, {
			"href":href,
			"class":"timeago",
			"aria-label":title, 
			title: dojo.attr(this.timeagoNode, "title"),
			"target":target
			});
		
		dojo.place(newTimeNode, this.timeagoNode.parentNode, "first");
		
		dojo.place(this.timeagoNode, newTimeNode);
		...
		if (urlAttachments){
			...
			if (this.urlAttachments.connections && this.urlAttachments.connections.video){
	 		...
	 		provider_url = com.ibm.social.incontext.util.url.removeRelativePathFromHost(videoUrl);
			
			this._urlPreview = new lconn.core.widget.urlPreview.URLPreviewNode({
