/* Copyright IBM Corp. 2012, 2015  All Rights Reserved.              */
	...
	define([
	...
	], function (array, declare, lang, events, Registrar) {
		...
		var DynamicLoadController = declare("com.ibm.social.as.controller.DynamicLoadController", 
		Registrar,
		{
			...
			constructor: function(){
				...
				this.register(events.ADDACTIVITYENTRY, 
								lang.hitch(this, "onLoadRequest"));
			},
			...
			addToQueue: function(objectId, timestamp) {
				...
				var alreadyInQ = array.some(this.queue, function(item) {
			...
			removeFromQueue: function(objectId) {
				var newQueue = array.filter(this.queue, function(item) {
			...
			*/
			...
			onFeedLoad: function(destroyNewsFeed, insertAtTop, url, clickEvent, scrollTop, response, request){
				...
				if(!insertAtTop){
					...
					// finally give up and publish dynamic update failure event 
						...
						topic.publish(this.dynamicUpdateFailure, this.objectId, this.currentLoadUpdatedSince);
						this.resetLoadRequestTimeout();
						this.nextFromQueue();
			...
			makeTimeoutRequest: function(url, timeout, clickEvent){
				...
				this.loadRequestTimeout = window.setTimeout(lang.hitch(this, function(){
