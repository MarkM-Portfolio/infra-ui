...
dojo.declare("com.ibm.social.incontext.util.DateFormat", null, {
   ...
   if (bdu.isGregorian())  
      ...
      return bdu.formatBidiDate(this.dt, {selector:'date',formatLength: this.USE_MED_DATE_FOR_SHORT[dojo.locale] ? 'medium' : 'short', locale: dojo.locale });
   },
   time: function() {
...
com.ibm.social.incontext.util.date = {
   REPLACE: /\$\{([^\s\:\}]+)(?:\:([^\s\:\}]+))?\}/g,
   MILLS_IN_DAY: 86400000,
   toTimestamp: function(date, nls) {
      return com.ibm.social.incontext.util.date.format(date,nls.FULL,nls);
   },
   ...
   delta: function(date, nls) {
      return com.ibm.social.incontext.util.DateFormat.delta(date, new Date());
   },
   getDays: function() {
      var qud = com.ibm.social.incontext.util.date;
      if (!qud._days)
   ...
   EEee: function(date,nls,offset) {
      var delta = com.ibm.social.incontext.util.date.delta(date) + (offset ? offset : 0);
      return (delta === 0) ? nls.TODAY_U : (delta == 1) ? nls.YESTERDAY_U : com.ibm.social.incontext.util.date.EEEE(date,nls,offset);
   },
   eeEE: function(date,nls,offset) {
      var delta = com.ibm.social.incontext.util.date.delta(date) + (offset ? offset : 0);
      return (delta === 0) ? nls.TODAY : (delta == 1) ? nls.YESTERDAY : com.ibm.social.incontext.util.date.EEEE(date,nls,offset);
   },
      
   MMMM: function(date, nls) {
   //bidi support
   if (bdu.isGregorian())  
      return com.ibm.social.incontext.util.date.getMonthsWide()[date.getMonth()];
   else
      return com.ibm.social.incontext.util.date.getMonthsWide()[bdu.fromGregorian(date).getMonth()];
   },
   MMM: function(date, nls) {
   //bidi support
   if (bdu.isGregorian())  
      return com.ibm.social.incontext.util.date.getMonths()[date.getMonth()];
   else
      return com.ibm.social.incontext.util.date.getMonths()[bdu.fromGregorian(date).getMonth()];
   },
   ...
   hh: function(date, nls){
      var hours = com.ibm.social.incontext.util.date.h(date, nls);
      return (hours > 9) ? hours : ("0"+hours);
   },
   mm: function(date, nls) {
   ...
   //bidi support
   if (bdu.isGregorian())  
      return dojo.date.locale.format(date, {selector:'date',formatLength: com.ibm.social.incontext.util.DateFormat.prototype.USE_MED_DATE_FOR_SHORT[djConfig.locale] ? 'medium' : 'short', locale: djConfig.locale });
   else
      return bdu.formatBidiDate(date, {selector:'date',formatLength: com.ibm.social.incontext.util.DateFormat.prototype.USE_MED_DATE_FOR_SHORT[djConfig.locale] ? 'medium' : 'short', locale: djConfig.locale });   
   },
...
com.ibm.social.incontext.util.DateFormat.delta = function(from,to) {
