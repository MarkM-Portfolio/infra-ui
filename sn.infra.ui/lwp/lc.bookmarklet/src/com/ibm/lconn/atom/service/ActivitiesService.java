...
import com.ibm.lconn.atom.model.ActivityObject;
...
public class ActivitiesService extends ConnectionsService {
	private final static String COPYRIGHT = com.ibm.lconn.bookmarklet.Copyright.SHORT;

	private String ACTIVITIES_URL = ServiceConfiguration
			.getString("ActivitiesService.url"); //$NON-NLS-1$

	private Log _logger = LogFactory.getLog(ActivitiesService.class);
	...
	public String getAllObjectsUrl(ServiceData data) {
		if (_logger.isDebugEnabled()) {
			_logger.debug(new StringBuilder("entry: getAllObjectsUrl, data:")
					.append(data.toString()));
		}
		String url = ACTIVITIES_URL;// + data.getUserName();
		if (_logger.isDebugEnabled()) {
			_logger.debug(new StringBuilder("getAllObjectsUrl: url is")
					.append(url));
		}
		_logger.debug("exit: getAllObjectsUrl");
		return url;
	}
	...
	public ArrayList<ModelObject> parseEntries(Document doc) throws IRISyntaxException{
		...
		while (it.hasNext()) {
			Entry entry = it.next();
			if (entry.getLinkResolvedHref("edit") != null) {
				ActivityObject obj = new ActivityObject(entry);
				rstList.add(obj);
			}
		}
		return rstList;
