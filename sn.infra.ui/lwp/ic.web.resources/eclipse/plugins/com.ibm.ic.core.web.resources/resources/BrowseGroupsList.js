...
define([
	...
	"ic-core/Res"
], function (dojo, string, array, declare, lang, domAttr, domClass, domConstruct, domStyle, queryModule, topic, _ComboBoxMenu, Res) {

/*
	 * List widget used by {@link lconn.core.BrowseGroups}
	 *
	 * @class ic-core.BrowseGroupsList
	 * @extends dijit.form._ComboBoxMenu
	 * @extends ic-core.Res
	 */
	var BrowseGroupsList = declare("lconn.core.BrowseGroupsList", [_ComboBoxMenu, Res], /** @lends ic-core.BrowseGroupsList.prototype */ {
	   pageSize: 25,
	   item: null,
	   items: null,
	   query: '',
	   ...
	   _selOption: function(idx) {
	      var allNodes = queryModule("div[role='option']", this.mainListNode);
	      if (idx < 0) idx = allNodes.length - 1;
	      if (idx > allNodes.length - 1) idx = 0;
	      var pw = this.parentWidget;
	      array.forEach(allNodes, function(node, i, nodes) {
	         if (i == idx) {
	   ...
	   _focusOptionNode: function( /*DomNode*/ node) {
	      ...
	      if (this._highlighted_option != node) {
	         ...
	         if (description && (type == 1)) {
	            ...
	            lang.hitch(this, "showTooltip", this.tooltipId, node, description), this.tooltipDelay);
	         }
	      }
	   },
	   ...
	   _createOption: function( /*Object*/ item, labelFunc, /*menuNumber*/ i) {
	      ...
	      if (i >= 0) domAttr.set(menuitem, "item", i);
	      if (item.userid) domAttr.set(menuitem, "exid", item.userid);
	
	      if (item.type) domAttr.set(menuitem, "persontype", item.type);
	
	      if (item.description != "null") {
	         domAttr.set(menuitem, "description", item.description);
	      }
	
	      domAttr.set(menuitem, "tabindex", "-1");
	      //else
	      //{
	   ...
	   createOptions: function(results, dataObject, findGroupSearch, labelFunc) {
	      ...
	      if (!dataObject.start || dataObject.start == 0) {
	         domStyle.set(this.previousButton, "display", "none");
	         domStyle.set(this.previousButton_ro, "display", "inline");
	      } else {
	         domStyle.set(this.previousButton, "display", "inline");
	         domStyle.set(this.previousButton_ro, "display", "none");
	      }
	
	      //diplay the next button
	      if ((dataObject.start + dataObject.count) < dataObject.totalItemCount) {
	         domStyle.set(this.nextButton, "display", "inline");
	         domStyle.set(this.nextButton_ro, "display", "none");
	      } else {
	         domStyle.set(this.nextButton, "display", "none");
	         domStyle.set(this.nextButton_ro, "display", "inline");
	      }
	      ...
	      if (pw._navFromPaging != false) {
	         try {
	            ...
	            if (!el || domStyle.get(el, "display") == "none") {
	               el = this[pw._navFromPaging + "_ro"];
	            } else {
	               el = queryModule("a", el)[0]; //get the first link to set the focus
	            }
	
	            if (el && domStyle.get(el, "display") != "none") el.focus();
	      ...
	      if (results.length == 0) {
	         ...
	         if (findGroupSearch && this.NoResultsMessage) {
	            resultsMsg = string.substitute(this.NoResultsMessage, [dataObject.query]);
	            dojo.removeAttr(this.mainListNode, "aria-describedby");
	         } else if (!findGroupSearch && this.NoChildGroupsMessage) {
	            resultsMsg = string.substitute(this.NoChildGroupsMessage, [dataObject.query]);
	            domAttr.set(this.mainListNode, "aria-describedby", "browseGroupsBreadcrumbs");
	         }
	         ...
	         el.item = resultsMsg;
	         domAttr.set(el, "aria-disabled", "true");
	         ...
	         var msg = string.substitute(this.pageCountMessage, [start, end, total]);
	         var msgalt = string.substitute(this.pageCountMessageAlt, [start, end, total]);
	
	         domConstruct.place(document.createTextNode(msg), this.mainPagingNode, "only");
	         this.mainPagingNode.title = msgalt;
	         domAttr.set(this.mainPagingNode, "aria-label", msgalt);
	         this.mainPagingNode.style.display = "";
	         domAttr.set(this.mainListNode, "aria-describedby", "browseGroupsWidgetLabel");
	   ...
	   clearResultList: function(bClearButtons) {
	      ...
	      domStyle.set(this.mainPagingNode, "display", "none");
