 ...
 */
dojo.provide("com.ibm.social.as.item.comment.CommentInput");
...
dojo.require("com.ibm.social.incontext.util.checkLoginMixin");
...
dojo.declare("com.ibm.social.as.item.comment.CommentInput", 
		[dijit._Widget, dijit._Templated, com.ibm.social.incontext.util.checkLoginMixin], {

	templatePath: dojo.moduleUrl("com.ibm.social.as", "item/comment/templates/commentInput.html"),
	...
	asCtx: lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial).uri + "/rest/activitystreams",
	asPrefix: "urn:lsid:ibm.com:activitystreams:",

	itemDeselectedEvent:  com.ibm.social.as.constants.events.ITEMDESELECTED,
	...
	postMixInProperties: function() {
		...
		this.mentionsStrings = dojo.i18n.getLocalization("com.ibm.social.ublog", "Mentions");
	...
	hideActions: function() {
		dojo.addClass(this.commentActionsAP, "lotusHidden");
	},
	...
	showActions: function() {
		if (!this.hasError) {
			dojo.removeClass(this.commentActionsAP, "lotusHidden");
		}
	},
	...
	disablePostAction: function() {
		dojo.addClass(this.commentPostActoin, "lotusPersonInactive");
		dojo.attr(this.commentPostActoin,"aria-disabled","true");
		this.postDisabled = true;
	},
	
	disableCancelBtn: function() {
		dojo.addClass(this.commentCancelBtn, "lotusPersonInactive");
		dojo.attr(this.commentCancelBtn,"aria-disabled","true");
	...
	enableAction: function() {
		dojo.removeClass(this.commentPostActoin, "lotusPersonInactive");
		dojo.attr(this.commentPostActoin,"aria-disabled", "false");
		this.postDisabled = false;
	},
	
	enableCancelBtn: function() {
		dojo.removeClass(this.commentCancelBtn, "lotusPersonInactive");
		dojo.attr(this.commentCancelBtn,"aria-disabled","false");
	...
	expand: function() {
		...
		dojo.publish(this.itemSelectedEvent, [this.newsItem]);
		this._focusTextArea();
	},
	/**
	...
	expandTextArea: function() {
        ...
        dojo.attr(this.commentTextAP, "placeholder", this.strings.writeCommentText);
	},
	...
	setIsCommenting: function(isCommenting) {
		...
		if (isCommenting) {
			...
			dojo.publish(com.ibm.social.as.constants.events.STARTCOMMENT, [this]);
			this.dirty = true;
			dojo.publish(com.ibm.social.as.constants.events.PAGEDIRTY, ["asComment"]);
		} else {
			com.ibm.social.as.comment.commentInputManager.commentingNewsItem = null;
			dojo.publish(com.ibm.social.as.constants.events.STOPCOMMENT);
	...
	reset: function() {
		...
		dojo.publish(this.itemDeselectedEvent, [this.newsItem]);
	},
	...
	post: function() {
		...
		if (this.postDisabled) {
			this._setTextForTextArea(dojo.string.trim(this._getTextFromTextArea()));
			return;
		}

		// Check that the comment isn't too long in either length or byte count.
		var commentToPost = dojo.string.trim(this._getTextFromTextArea());
		var commentBytes = lconn.core.util.text.lengthUtf8(commentToPost);
	...
	checkPermission: function() {
		...
		if (this.isCommentingPermitted === false) {
			...
			var args = {
				...
				handleAs: "xml",
				headers: {},
				handle: dojo.hitch(this, function(doc) {
	...
	*/
	_preventLoginRedirect: function(args){
		if (dojo.getObject("com.ibm.ajax.auth")) {
			args = com.ibm.ajax.auth.prepareSecure(args);
			this._defaultAuthHandler = com.ibm.ajax.auth.authenticationHandler;

			com.ibm.ajax.auth.setAuthenticationHandler(dojo.hitch(this, function (data, ioArgs, onauthenticated) {
				this.checkLoginOrPerm = false;
				var timeout_str = dojo.string.substitute(this.strings.commentSessionTimedOut, ["/homepage"]);
	...
	_resetDefaultHandler: function () {
		if (dojo.getObject("com.ibm.ajax.auth")) {
			com.ibm.ajax.auth.setAuthenticationHandler(this._defaultAuthHandler);
		}
	},
	...
	generateCommunityIsCommentPermittedMsg: function(asc) {
		...
		if(asc.containerInfo && asc.containerInfo.resourceId && asc.containerInfo.resourceId === communityId) {
			...
			try {										
				...
				dojo.create("a", {innerHTML: communityName, 
				...
				this.commentNotPermittedReason = 
					dojo.string.substitute(this.mentionsStrings.JOIN_COMMUNITY_NON_SPECIFIC, [tempDiv.innerHTML]);																															
			} 
			catch(e) {
				if(communityName){
					this.commentNotPermittedReason = 
						dojo.string.substitute(this.mentionsStrings.JOIN_COMMUNITY_NON_SPECIFIC, [communityName]);
				} else {
					this.commentNotPermittedReason = this.mentionsStrings.JOIN_COMMUNITY;
				}						
			}
	...
	addInlineComment: function(entry, fileDetails) {
		...
		if (this.newsItem.inlineComments) { // has comment already
			...
			this.newsItem.inlineComments = new com.ibm.social.as.item.comment.InlineComments({
		...
		dojo.publish(com.ibm.social.as.constants.events.RESIZEHEIGHT, ["comment" , comment.id]);
	},
	...
	_updateRemainingChars: function() {
		...
		var commentLen = lconn.core.util.text.length(dojo.string.trim(this._getTextFromTextArea()));
    ...
    _getTextFromTextArea: function() {
        return this.commentTextAP.value;
    },
    
    _setTextForTextArea: function(text) {
        this.commentTextAP.value = text;
    }
