...
simple_stock_no_dojo = function() {
	this.handlingFn = null;
}

simple_stock_no_dojo.prototype = {
	onLoad: function () {
	...
	onview:function(){
		...
		changeButton.onclick = function() {
			...
			if (symbol) {
				_this._updateChart(symbol);
				_this._updateCompanyName(symbol);
			}
		};
		/*
		dojo.connect(changeButton, "onclick", this, function() {
			var symbol = input.value;
			if (symbol) {
				this._updateChart(symbol);
		...
		changeTitleButton.onclick = function() {
			var title = titleInput.value;
			if (title) {
				iDescriptor.setItemValue("title",title);
				_this.iContext.getElementById("title").innerHTML = _this.iContext.getiDescriptor().getItemValue("title");
			}
		};
		/*
		dojo.connect(changeTitleButton, "onclick", this, function() {
			var title = titleInput.value;
			if (title) {
				iDescriptor.setItemValue("title",title);
		...
		changeAuthorButton.onclick = function() {
			var author = authorInput.value;
			if (author) {
				iDescriptor.setItemValue("author",author);
				_this.iContext.getElementById("author").innerHTML = _this.iContext.getiDescriptor().getItemValue("author");
			}
		};
		/*
		dojo.connect(changeAuthorButton, "onclick", this, function() {
			var author = authorInput.value;
			if (author) {
				iDescriptor.setItemValue("author",author);
