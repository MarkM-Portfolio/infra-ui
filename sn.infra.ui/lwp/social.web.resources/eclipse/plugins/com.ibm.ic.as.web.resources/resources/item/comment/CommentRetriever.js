/* Copyright IBM Corp. 2013, 2015  All Rights Reserved.              */
	...
	], function (dojo, array, declare, lang, dojoDate, stamp, LCDeferred) {
	
		/**
		 */
		var CommentRetriever = declare("com.ibm.social.as.item.comment.CommentRetriever", null,
		{
			...
			constructor: function(/* string */ suId, /* object */ comments) {
				...
				if ( comments ) {
					...
					array.forEach(comments.items, function(comment) {
						if ( comment ) {
							this.comments.push(lang.clone(comment));
						}
					}, this);
				}
			},
			...
			retrievePreviousComments: function(/* int */ displayedCount, /* int */step) {
				...
				return this.loadMoreComments(retrieveCount)
						.then(lang.hitch(this, "storeNewComments", retrieveCount))
			...
			storeNewComments: function(/* int */ retrieveCount, /* Object */ resp) {
				...
				curFirstTime = curFirstTime ? stamp.fromISOString(curFirstTime) : new Date();
				
				// comments returned in descending date, we want ascending
				resp.list.reverse();
				
				var newComments = [];
				array.some(resp.list, function(comment) {
					var cTime = stamp.fromISOString(comment.published);
					if ( dojoDate.compare(curFirstTime, cTime) > 0 ) { // is comment older?
						newComments.push(comment);
					} else if ( dojoDate.compare(curFirstTime, cTime) == 0 ) { // same time
				...
				array.forEach(newComments, function(comment) {
			...
			loadMoreComments: function(/* int */ retrieveCount) {
				...
				activityStreamAbstractHelper.xhrGet({
					url: url,
					handleAs: "json",
					load: lang.hitch(this, function(resp) { 
						this.lastPageRetrieved++;
