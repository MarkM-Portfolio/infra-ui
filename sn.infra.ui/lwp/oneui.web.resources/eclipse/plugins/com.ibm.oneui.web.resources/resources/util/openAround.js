...
/**
 * Opens a dijit pointed by menuId (typically a dijit.Menu) in a popup placed around the node in evt.target
 * typically called on a onclick event in the page, es:
 * <p>
 * &lt;a onclick=&quot;com.ibm.oneui.util.openAround('id of your menu', this, null, event)&quot;&gt;Click me&lt;/a&gt;
 * <p>
 * You can also use dojo.connect to bind it programmatically to a DOM node.
 * 
 * @function openAround
 * @memberof com.ibm.oneui.util
 * @param {String|Ref} menuId DOM id or dijit.Menu object
 * @param {Node} [openedBy] Explicit DOM node to orient the menu around. If it is null or undefined, the event target will be used instead
 * @param {Object} [opt] Map of other options controlling display. May be null or undefined.
 * @param {Event} evt
 * @param {Object} {x, y} coordinates to orient the menu around.
 */
com.ibm.oneui.util.openAround = function (menuId, openedBy, opt, evt, coords){  
   ...
   if (coords && coords.x && coords.y) {
      ...
      dijit.popup.open({
         ...
         x: coords.x,
         y: coords.y,
         orient: (opt ? opt.orient : null) || (dojo._isBodyLtr() ?
               {'BL':'TL', 'BR':'TR', 'TL':'BL', 'TR':'BR'} : {'BR':'TR', 'BL':'TL', 'TR':'BR','TL':'BL'}),
