...
define([
	...
	"../url",
	"../config/services"
], function (dojo, Deferred, declare, lang, _Widget, auth, url, services) {
	
	/**
	 * This class will manage all requests to a Search API
	 * 
	 * @class ic-core.typeahead.DataReader
	 * @author Andrea Paolucci <andreapa@ie.ibm.com>
	 */
	var DataReader = declare(
		"lconn.core.typeahead.DataReader",
		_Widget, /** @lends ic-core.typeahead.DataReader.prototype */
	{
		/**
		...
		startup: function() {
			...
			var searchUrl = url.getServiceUrl(services.search);
			var authenticationType = (auth.isAuthenticated() ? this.basicAuthentication : this.anonymousAuthentication);
		...
		executeQuery: function(query) {
			...
			var xhrArgs = {
				...
				content: "",
				handleAs: "json",
				load: lang.hitch(this, "_querySuccess"), 
				error: lang.hitch(this, "_queryError"),
				failOk: true
			};
			if(this.timeout > 0) {
