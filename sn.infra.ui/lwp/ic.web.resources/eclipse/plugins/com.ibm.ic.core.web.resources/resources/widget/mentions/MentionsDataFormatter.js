define([
	...
	"dojo/dom-construct",
	"dojox/html/entities"
], function (array, declare, lang, domConstruct, entities) {
	 ...
	 * 
	 * @class ic-core.widget.mentions.MentionsDataFormatter
	 * @see {@link lconn.news.microblogging.sharebox.data.MentionsDataFormatter}
	 * @author Qi Wei Zhang <zhangqiw@cn.ibm.com>
	 */
	var MentionsDataFormatter = declare("lconn.core.widget.mentions.MentionsDataFormatter", null, /** @lends ic-core.widget.mentions.MentionsDataFormatter */ {
	   ...
	   formatData: function(data) {
	      ...
	      array.forEach(textBreakdown, lang.hitch(this, function(item) {
	   ...
	   _createMentionsContainer: function() {
	      ...
	      var vcard = domConstruct.create("span", {
	         className : "vcard"
	      }, this.container);
	
	      this.fnContainer = domConstruct.create("span", {
	         className : "fn"
	      }, vcard);
	
	      this.idContainer = domConstruct.create("span", {
	   ...
	   _createMentionsMarkup: function(user) {
	      ...
	      var displayName = user.displayName ? entities.encode(user.displayName) : "";
	      
	      this.fnContainer.innerHTML = (user.hasSymbol) ? "@".concat(displayName) : displayName;
	      
	      this.idContainer.innerHTML = user.userId || "";
	
	      return entities.decode(this.container.innerHTML);
	   }
	});
	
	return MentionsDataFormatter;
});
