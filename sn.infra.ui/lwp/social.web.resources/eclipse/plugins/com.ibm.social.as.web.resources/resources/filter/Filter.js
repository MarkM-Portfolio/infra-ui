...
dojo.declare("com.ibm.social.as.filter.Filter",
 ...
 com.ibm.social.as.util.Localizer],
{
	...
	// Utility object
	util: com.ibm.social.as.util.ConfigObjectUtil,

	// Name of the filter change event topic.
	// This will be used to 'publish' an update to the feed.
	onFilterChangeEventName: com.ibm.social.as.constants.events.UPDATESTATE,
	...
	filterClicked : function(newFilterId){
		...
		dojo.publish(this.onFilterChangeEventName, [stateArr, this._focused]);
	...
	updateFilterOptions: function(filterOptions){
		...
		var selectedPrefix = this.getLocalizedString("selectedLabel");

		for(var v in this.filterOptions){
			var filterOption = this.filterOptions[v];
			var isSelected = v==this.filter.selectedItem;
			var label = filterOption.label;
			var title = filterOption.description;

			if (isSelected){
				title = selectedPrefix + " - " + title;
			}

			fragment.appendChild(this.createFilterItemNode(v, label, isSelected, title));
		}
		...
		if(filters){
			...
			window.setTimeout(dojo.hitch(this, "createSubFilter", filters), 0);
		...
		if(this.isFocused){
			window.setTimeout(dojo.hitch(this, "focusMainFilter", filters), 0);
		}
	},
