...
define([
	...
	"dojo/date/stamp",
	"dojo/has"
], function (dojo, aspect, stamp, has) {
	...
	window.lconnTestStarted = function(groupName, fixture) {
	   if (!lconnResults.testsuites[groupName].timestamp )
	      lconnResults.testsuites[groupName].timestamp = stamp.toISOString(new Date(), {milliseconds: true});
	};
	...
	window.lconnAllTestsFinished = function() {
	      ...
	      if (has("ie")) {
	         params.browser = "Internet Explorer (" + has("ie") + ")";
	      }
	      else if (has("ff")) {
	         params.browser = "Firefox (" + has("ff") + ")";
	      }
	      else if (has("safari")) {
	         ...
	         params.browser = "Opera (" + has("opera") + ")";
	      }
	      else {
	...
	aspect.after(doh, "_report", window.lconnAllTestsFinished, true);
	
	return com.ibm.social.test.scripts.results.ResultCollector;
});
