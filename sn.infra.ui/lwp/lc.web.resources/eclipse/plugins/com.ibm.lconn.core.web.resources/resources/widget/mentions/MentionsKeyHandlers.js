 ...
 *
 * @mixin lconn.core.widget.mentions.MentionsKeyHandlers
 */
dojo.declare("lconn.core.widget.mentions.MentionsKeyHandlers", null, /** @lends lconn.core.widget.mentions.MentionsKeyHandlers.prototype */ {
   ...
   moveToPrevious : function(range, node) {
      ...
      if (node != this.textAreaNode) {
         if (this.isAnyMentionsDomNode(range.startContainer)) {
            ...
            // we only have problems with IE so introducing a new condition for IE only 
      ...
      if (node != this.textAreaNode) {
         ...
         if (nextSibling) {
            if (nextSibling.nodeType == 3) {
               ...
               if (dojo.isWebKit && parent && parent.parentNode && this.isAnyMentionsDomNode(parent.parentNode)) {
                  isMentionNode = true;
               }
               var pos = (nextSibling.data.charAt(0) == '\u200B' || (dojo.isWebKit && !isMentionNode)) ? 1 : 0;
...
//            } else {
   ...
   handlePrePaste : function(e) {
        if (dojo.isIE) { //RTC 96636 & 110143 - Fixes Ctrl + V pasting issue in IE
      var emptyBeforePaste = (this.textAreaNode.innerText == "");
      this.preventDefault(e);
      this.tempTextArea.value = clipboardData.getData("Text");
      if (emptyBeforePaste) {
         this.setText("");
         if (dojo.isIE == 8) this._rangySelection = null;
      }
     }
   ...
   handleUpDownArrow : function(e) {
      if (this._isTracking && e) {
         ...
         if (this.getActiveType_IsShowing()) {
            ...
            highlighted && dojo.attr(this.textAreaNode, "aria-activedescendant", highlighted.id);
         }
      } else if (dojo.isIE) {
         setTimeout(dojo.hitch(this, function() {
            var curSel = this.getSelection();
