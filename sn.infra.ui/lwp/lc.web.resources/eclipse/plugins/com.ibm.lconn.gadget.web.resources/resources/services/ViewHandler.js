 ...
 domStore_) 
{
	 ...
	 */
	dojo.declare('com.ibm.lconn.gadget.services.ViewHandler', null, {
		 ...
		 * @function
		 * @param {String=} view_target 
		 * 	The view target to invoke create.  If this is <code>null</code>, 
		 *  this is treated as the 'default' or 'fallback' handler.
		 * @param {Object[]=} args 
		 ...
		 * 	The common container instance that the call is bound to.
		 * @param {String=} view_target 
		 * 	The view target to invoke create 
		 * @param {Object[]=} args 
		 * 	Array of arguments to pass to the handler method.
		 * @return {Object=} 
		 * 	Returns the created element
		...
		invokeCreate : function(container, view_target, args) {
			...
			var registry = this._registry,
				handler_pair = registry[view_target] || registry[CONST_NULL_];
				element = null;
			
			if (handler_pair) {
				var element = handler_pair.createHandler.apply(container, args);
				domStore_(element).set(ATTR_DESTROY_HANDLER_, handler_pair.destroyHandler);
				return element;
			} else {
	 ...
	 * Add static methods
	 */
	dojo.mixin(com.ibm.lconn.gadget.services.ViewHandler, {
		
		/**
		 * Invokes the 'destroy' method for this site
		 * @memberOf com.ibm.lconn.gadget.services.ViewHandler
		 * @function
		 * @param {String=} view_target The view target to invoke create 
		 * @param {Object[]=} 
		 *   Array of arguments to pass to the handler method.
		 */
		invokeDestroy : function(container, site, args) {
			// TODO, find element
			var element = getElementForSite_(site),
				handler = domStore_(element).get(ATTR_DESTROY_HANDLER_);
