define([
...
], function (lang, dom, domClass, has, on, query, BidiComplex, locale, api, config) {

	/* Copyright IBM Corp. 2008, 2019  All Rights Reserved.              */
	
	/**
	 * This package provides methods setting text direction for various HTML control
	 * (dynamic input fields, static labels ...)
	 * @namespace ic-core.globalization.bidiUtil
	...
	/** 
	 * This function enforces the text direction for all the HTML elements having the class bidiAware
	 * It also handles Structured Text (STT) such as URL, FILE PATH ...
	 * @memberof ic-core.globalization.bidiUtil
	 * @function enforceTextDirectionOnPage
	 * @param {Node} [node] Root node for HTML elements. If not passed, the entire document will be scanned.
	 */
	w.enforceTextDirectionOnPage = function(node) {
		...
		if (config.areSettingsEnabled() && api.isBidiEnabled()) {
			var textDirection = api.getTextDirection();
			var str = "";var text = "";var ch = "";
			// Handles Text Direction
			var t = node ? query(".bidiAware", node) : query(".bidiAware");
			for(var i = 0; i < t.length; i++) {
				var element = t[i];
				if(element.tagName == 'INPUT' || element.tagName == 'TEXTAREA' || (element.tagName == 'DIV' && element.contentEditable == 'true')) {
					on(element, "keyup", lang.hitch(element, function(){
					...
					this.inputRTLProcessing(element, textDirection);
                                        ...
                                        || element.tagName == 'B' || element.tagName == 'P') {
					element.innerHTML = this.enforceTextDirection(element.innerHTML, textDirection);
				}
				else if(element.tagName == 'SELECT') {
					this.enforceTextDirectionForSelect(element, textDirection);
				}
				else {
					element.dir = element.style.direction = this.getTextDirection(element.value || element.text || element.outerText || element.innerHTML, textDirection);
			...
			// Note: In the future HTML5 adds new field types: URL, email so we could rely on these types instead of bidiSTT_URL and bidiSTT_EMAIL
			var urls = node ? query(".bidiSTT_URL", node) : query(".bidiSTT_URL");
			for(i = 0; i < urls.length; i++) {
				if (urls[i].tagName == 'INPUT' || urls[i].tagName == 'TEXTAREA') {
					BidiComplex.attachInput(urls[i], "URL");
				}
				else {
			...
			urls = node ? query(".bidiSTT_FILE_PATH_SIZE", node) : query(".bidiSTT_FILE_PATH_SIZE");
			for(i = 0; i < urls.length; i++) {
				if (urls[i].tagName == 'INPUT' || urls[i].tagName == 'TEXTAREA') {
					BidiComplex.attachInput(urls[i], "URL");
				}
			...
			var emails = node ? query(".bidiSTT_EMAIL", node) : query(".bidiSTT_EMAIL");
			for(i = 0; i < emails.length; i++) {
				if (emails[i].tagName == 'INPUT' || emails[i].tagName == 'TEXTAREA') {
					BidiComplex.attachInput(emails[i], "EMAIL");
				}
				else {
					emails[i].innerHTML = BidiComplex.createDisplayString(emails[i].innerHTML, "EMAIL");
				}			
			}
			...
			if (myForm) {
				on(myForm, "submit", lang.hitch(myForm, function(){
					w.stripSpecialCharacters(node);
				}));
			}
			
			// Handles Input field of type file
			var f = node ? query(".bidiFileInput", node) : query(".bidiFileInput");
			for(i = 0; i < f.length; i++) {
				var element = f[i];		
				on(element, "keydown", lang.hitch(element, function(){
					w.onFileKeyDown(this);
				}));
				on(element, "change", lang.hitch(element, function(){
					w.processChange(this);
				}));
				on(element, "focus", lang.hitch(element, function(){
					w.fileOnFocus(this);
				}));
				on(element, "mousedown", lang.hitch(element, function(){
					w.onFileMouseDown(this);
				}));
				var fakeElement = dom.byId("bidi_faked_" + element.id);
				if (fakeElement) {
					on(fakeElement, "focus", lang.hitch(fakeElement, function(){
						w.onfocus(this);
					}));
					on(fakeElement, "keydown", lang.hitch(fakeElement, function(){
						w.onFakeKeyDown(event, this);
					}));
					on(fakeElement, "copy", lang.hitch(fakeElement, function(){
						w.processCopy(this);
					}));
					on(fakeElement, "paste", lang.hitch(fakeElement, function(){
						return false;
					}));
					on(fakeElement, "cut", lang.hitch(fakeElement, function(){
						return false;
					}));
				}
			}
	 ...
	 * If user input is a right to left language like(Arabic and Hebrew), then change the input style to right to left.
	 * @memberof ic-core.globalization.bidiUtil
	 * @function inputRTLProcessing
	 * @param {String} [requiredElement] The DOM node to inspect.
	 * @param {String} [txtDir] A string representing the direction of text.
	 */
	w.inputRTLProcessing = function(requiredElement, txtDir){	
		if (config.areSettingsEnabled() && api.isBidiEnabled() && requiredElement != null && (requiredElement.value != "" || requiredElement.innerHTML != "")){
			txtDir = this.getTextDirectionPref(txtDir); 
			if(requiredElement.tagName == 'INPUT' || requiredElement.tagName == 'TEXTAREA' || (requiredElement.tagName == 'DIV' && requiredElement.contentEditable == 'true') && (requiredElement.value || requiredElement.innerHTML)){
				if (txtDir == config.TEXT_DIRECTION.CONTEXTUAL){
				...
				} else if (txtDir == config.TEXT_DIRECTION.LEFT_TO_RIGHT)
					requiredElement.dir = requiredElement.style.direction = "ltr"; 
				else if (txtDir == config.TEXT_DIRECTION.RIGHT_TO_LEFT)
					requiredElement.dir = requiredElement.style.direction = "rtl";
				if (domClass.contains(requiredElement, "lotusLeft") && requiredElement.style.direction === "rtl"){
					requiredElement.style.textAlign = "right";
				} else if (domClass.contains(requiredElement, "lotusRight") && requiredElement.style.direction === "ltr"){
	...
	/** 
	 * This function enforces the text direction for a &lt;select&gt; HTML element. Actually it enforces the text direction for
	 * all its options.
	 * @memberof ic-core.globalization.bidiUtil
	 * @function enforceTextDirectionForSelect
	 * @param {String} [element] The DOM node used as a root, normally a &lt;select&gt; element.
	 * @param {String} [txtDir] A string representing the direction of text.
	*/
	 ...
	 * @param {String} [stringValue] The string to process.
	 * @param {String} [txtDir] A string representing the direction of text.
	 */
	 ...
	 * @param {String} [stringValue] The string to analyze.
	 * @param {String} [txtDir] A string representing the direction of text.
	 */
	 ...
	 * @param {String} [txtDir] A string representing the direction of text. If not provided, it will be obtained through the Globalization Preferences JS API.
	 */
	w.getTextDirectionPref = function (txtDir) {
		...
		if (typeof(txtDir) != "undefined") {
			...
			if (config.areSettingsEnabled() && api.isBidiEnabled()) {
				textDirPref = api.getTextDirection();
			}
		}
		return textDirPref;
	};
	...
	/** 
	 * This function uses regular expressions to detect the language of user input
	 * @memberof ic-core.globalization.bidiUtil
	 * @function isRTLValue
	 * @param {String} stringValue The string to analyze.
	 */
	 ...
	 * @memberof ic-core.globalization.bidiUtil
	 * @function isBidiChar
	 * @param {String} c The character to analyze.
	 */
	 ...
	 * @param {Node} obj The input field.
	 */
	...
	w.fileOnFocus = function (obj) {
	  this.setRelatedElements(obj);
	  obj.relatedElement.value = BidiComplex.createDisplayString(obj.value, "FILE_PATH");
	};
	...
	/** 
	 * This Function fills the attribute relatedElement with reference to the real (or fake) input field
	 * @memberof ic-core.globalization.bidiUtil
	 ...
	 * @param {Node} obj The target node.
	 */
	w.processCopy = function (obj){
		...
		try{
			...
			var textToClipboard = BidiComplex.stripSpecialCharacters(text);		
	 ...
	 * @param {Node} node The input field.
	 */
	w.stripSpecialCharacters = function (node) {
		if (typeof node === "string")
			return BidiComplex.stripSpecialCharacters(node);

		var urls = node ? query(".bidiSTT_URL", node) : query(".bidiSTT_URL");
		for(var i = 0; i < urls.length; i++) {
			if (urls[i].tagName == 'INPUT' || urls[i].tagName == 'TEXTAREA') {
				urls[i].value = BidiComplex.stripSpecialCharacters(urls[i].value);
			}
		}
		var emails = node ? query(".bidiSTT_EMAIL", node) : query(".bidiSTT_EMAIL");
		for(var i = 0; i < emails.length; i++) {
			if (emails[i].tagName == 'INPUT' || emails[i].tagName == 'TEXTAREA') {
				emails[i].value = BidiComplex.stripSpecialCharacters(emails[i].value);
			}
		}
	};
