 ...
 * 
 * 
 * @class lconn.core.typeahead.TypeAheadManager
 * @author Andrea Paolucci <andreapa@ie.ibm.com>
 */
dojo.declare(
	"lconn.core.typeahead.TypeAheadManager",
	[dijit._Widget], /** @lends lconn.core.typeahead.TypeAheadManager.prototype */
{
	/**
	...
	/**
	 * Prototype of a lconn.core.typeahead.ResultDropDown implementation.
	 * Use this if you want to override ResultDropDown class.
	 * 
	 * @default [lconn.core.typeahead.ResutltDropDown]
	 * @type {Function}
	 */
	ResultDropDown: lconn.core.typeahead.ResultDropDown,
	...
	postCreate: function() {
		...
		timeout = parseInt(lconn.core.config.properties[this.SERVICE_TIMEOUT_PROPERTY]);
		if(timeout) {
			this._serciveTimeout = timeout;
		}
		var maxRes = parseInt(lconn.core.config.properties[this.MAX_RESULTS_PROPERTY]);
		if(maxRes) {
		...
		for(var i=0; i < this.TAservicesList.length; i++) {
			...
			if(!lconn.core.typeahead.Service.prototype.isPrototypeOf(service) && dojo.config.isDebug) {
	...
	runQuery: function() {
		...
		this._timeoutID = setTimeout(dojo.hitch(this, function() {
			...
			for(var i=0; i < this.TAservicesList.length; i++) {
				...
				promise.then(dojo.hitch(this, "setTypeaheadResults", service.id));
				promisesList.push(promise);
			}
			
			var deferredList = new dojo.DeferredList(promisesList);
			deferredList.then(dojo.hitch(this, "showTypeaheadResults"));
		}), this._queryTimeout);
	},
