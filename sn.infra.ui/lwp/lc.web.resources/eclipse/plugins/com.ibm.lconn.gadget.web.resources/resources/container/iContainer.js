...
dojo.provide("com.ibm.lconn.gadget.container.iContainer");
dojo.provide("com.ibm.lconn.gadget.container.Handle");
dojo.provide("com.ibm.lconn.gadget.container.EEHandle");
 ...
 * Interface object
 */
dojo.declare("com.ibm.lconn.gadget.container.Handle_old", null, {
	/**
...
dojo.declare("com.ibm.lconn.gadget.container.EEHandle", [com.ibm.lconn.gadget.container.Handle_old], {
...
com.ibm.lconn.gadget.container.iContainer = 
	...
	(function init_lconn_core_cwgc_container(){
		...
		var _global = dojo.global,
			...
			_lconn_core_url = lconn.core.url,
			_creSvcConfig = lconn.core.config.services['opensocial'],
			_creResUrl = (function() {
			      var _svcUrl = _lconn_core_url.getServiceUrl(_creSvcConfig);
					var path = _svcUrl ? com.ibm.oneui.util.proxy(_svcUrl.toString()) : "";
		...
		*/
		...
		var _iWidgetHandle = dojo.declare([com.ibm.lconn.gadget.container.Handle], {
			...
			open : function() {
				...
				//
				 ...
				 prom.then(dojo.hitch(this, "_loaded"), dojo.hitch(this, "_fail"));
			},
			close : function() {
				if (this.isOpen()) {
			...
			_fail : function(err) { // Failed loading widget.
				this._loaded = false;
				console.log("iContainer - iWidget failed to load. " + dojo.toJson(arguments));
			}
		});
		 ...
		 */ 
		var _GadgetHandle = dojo.declare([com.ibm.lconn.gadget.container.Handle], {
			_gadgetSite : null,
			_definitionUrl : null,
			_placement : null,
			_inline: false,
			...
			constructor : function(widgetObject) {
				this._placement = dojo.byId(widgetObject.placement);
		...
		var _EEGadgetHandle = dojo.declare([_GadgetHandle, com.ibm.lconn.gadget.container.EEHandle], {
			_eeContext: null,
			...
			open : function() {
				...
				this._gadgetSite = _os_container.ee.navigate(
							...
							renderParams,
							dojo.hitch(this, function(site, metaData) {
								if(dojo.config.isDebug) {
		...
		var _fn_loadJS_sync = function (_config, js_files) {
			if (js_files) {
				...
				while (index--) {
					resUrl = dojo.replace(js_files[index], replacement);
					console.log("Load: " + resUrl);
					dojo.xhrGet({url: resUrl, handleAs: 'javascript', sync: true});
				}
			}
		};
		...
		var _fn_initOSContainer = function (_config) {
			...
			if (typeof (cre$) !== "undefined" && _debug) {
				if(dojo.config.isDebug) {
					console.debug("CRE already loaded, skipping load of: " + dojo.toJson(_os_res));	
				}
			} else {
		...
		var _fn_newPlacementSite = function(widgetObject) {
			var placement_dom = dojo.byId(widgetObject.placement),
