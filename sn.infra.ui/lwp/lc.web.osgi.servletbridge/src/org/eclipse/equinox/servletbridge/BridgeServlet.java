...
public class BridgeServlet extends HttpServlet {
    ...
    private static final Logger log = Logger.getLogger(BridgeServlet.class.getName());
	...
	static private long lastRestart = 0;
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (log.isLoggable(Level.FINEST))
            log.logp(Level.FINEST, BridgeServlet.class.getName(), "service", "Processing path {0}", pathInfo);
		...
		ClassLoader frameworkClassLoader = framework.getFrameworkContextClassLoader();
		HttpServlet servletReference = acquireDelegateReference();
		if (servletReference == null) {
	        if (log.isLoggable(Level.FINE))
	            log.logp(Level.FINE, BridgeServlet.class.getName(), "service", "No delegate available for {0}", pathInfo);
			resp.sendError(HttpServletResponse.SC_NOT_FOUND, "BridgeServlet, no delegate: " + req.getRequestURI()); //$NON-NLS-1$
			return;
		}
		try {
			...
			// kludge for Connections PMR: 28512,L6Q,000
			// Catch non-recoverable NPE that originated in ZipBundleEntry and
			// restart AjaxFramework

			StackTraceElement[] elements = npe.getStackTrace();
			if (elements != null && elements[0] != null) {
				String firstElement = elements[0].getClassName();

				if (firstElement != null && firstElement.startsWith("org.eclipse.osgi.baseadaptor.bundlefile.ZipBundleEntry$ZipBundleEntryInputStream")) {
					restart = true;
				}
			} 
			if (restart != true) {
			...
			if (restart) {
				if (lock.tryAcquire()) {
					try {
						// only restart if it's been more then 60 seconds since we last started
						if (System.currentTimeMillis() - lastRestart > 60000) {
							// handle test case, remove file that is used to simulate for testing the NPE
							try {
								File f = new File("/tmp/simulate-npe");
								if (f.exists()) {
									f.delete();
								}
							} catch (Throwable t) {}

							log.severe("** Trying to recover from ZipBundleEntry NullPointerException.  Restarting AjaxFramework.");
							framework.stop();
							log.severe("** Framework stopped");
							// Undeploy is hanging, let's hope just a stop/start does it.
							// framework.undeploy();
							// log.severe("** undeployed");
							// framework.deploy();
							// log.severe("** deployed");
							framework.start();
							log.severe("** Framwork started");
							lastRestart = System.currentTimeMillis();
						} else {
							System.out.println("** someone beat me to AjaxFramework restart (last restarted " + (System.currentTimeMillis() - lastRestart)/1000 + " seconds ago).");
						}
					} finally {
						lock.release();
					}
				}
				resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "ZipBundleEntry error - please retry");
			}
		}
	}
	...
	public static synchronized void unregisterServletDelegate(HttpServlet servletDelegate) {
		...
		synchronized (instance) {
			...
			while (instance.delegateReferenceCount != 0) {
				try {
					instance.wait(); //FIXME: unbounded wait here somewhat dangerous
