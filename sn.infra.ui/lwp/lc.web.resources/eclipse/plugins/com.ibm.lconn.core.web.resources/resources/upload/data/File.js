...
/**
 * @class lconn.core.upload.data.File
 */
dojo.declare("lconn.core.upload.data.File", null, /** @lends lconn.core.upload.data.File.prototype */ {
      ...
      TRUNCATE: {
         execute: function(file) {
            if (file.getOwningList().isRenameAllowed()) {
               ...
               var name = lconn.core.util.text.trimExtension(file.getName());

               name = file.shortenFilename(name, ext, file.getOwningList().getMaxFilenameLength());

               var newName = name.name;
               if (ext && dojo.string.trim(ext).length > 0)
                  newName += "." + ext;
   ...
   constructor : function(fileObject, name, fileList) {
      ...
      this._originalName = lconn.core.util.text.trimExtension(name) + (extension ? "." + extension : "");
   ...
   /**
    * Callback to be notified when SetBytesComplete is called.
    */
   onSetBytesComplete: function(file, oldVal, newVal) {},
   ...
   setBytesComplete: function(bc) {
      ...
      this.onSetBytesComplete(this, oldVal, this._bytesComplete);
   },
      ...
      if (this._fileList && (oldVal != this._hasInvalidNameLength)) {
         ...
         if (this._hasInvalidNameLength) {
            var status = {
               ...
               level: this.StatusLevels.ERROR,
               message: dojo.string.substitute(this.nls.STATUS_NAME_TOO_LONG, [this._fileList.getMaxFilenameLength()]),
               preventUpload: true,
      ...
      if (this._fileList && (oldName != editedName)) {
         ...
         if (editedName != this.getOriginalFileName()) {
            var status = {
               ...
               message: dojo.string.substitute(this.nls.STATUS_RENAMED, [lconn.core.util.html.formatFilename(this._shortenDisplayName(this.getOriginalFileName()))]),
   ...
   getExtension : function() {
      if (typeof this._extension == 'undefined') {
         ...
         this._extension = lconn.core.util.text.getExtension(name);
      }
   ...
   canUpload: function() {
      ...
      var disableSameNameCheck = this._fileList ? !!this._fileList.disableSameNameCheck : false;
      if (_canUpload && !disableSameNameCheck) {
         _canUpload = typeof this._isRemoteDuplicate != "undefined";
      }
    ...
    */
   isValid: function() {
      var _canUpload = dojo.every(this._statuses, function(status) { return !status.preventUpload; });
      return _canUpload;
   },
   ...
   validateSize: function() {
      ...
      if (size >= 0 && max > 0) {
         if (size > max) {
            var fmtSize = lconn.share.util.text.formatSize(this.nls, size);
            var fmtMax = lconn.share.util.text.formatSize(this.nls, max);
            if (dojo.getObject("lconn.share.config.services.multiPhaseFileUpload") && this._fileList && typeof(Worker) == "undefined") {
               error = {
                  message: dojo.string.substitute(this.nls.ERROR_TOO_BIG_FOR_BROWSER, [ this.getName() ])
               };
            }else {
               error = {
                  message: dojo.string.substitute(this.nls.ERROR_TOO_BIG, [ this.getName(), fmtSize, fmtMax ])
               };
            }
         }
      }
   ...
   validateExtension: function() {
      ...
      if (extObj) {
         ...
         if (extObj.allowed) {
            if (dojo.indexOf(extObj.allowed, ext) == -1) {
               error = { message : dojo.string.substitute(this.nls.ERROR_BAD_EXT_WHITELIST, [this.getName(), this.getExtension()])};
            }
         } else if (extObj.prohibited) {
            if (dojo.indexOf(extObj.prohibited, ext) != -1) {
               error = { message : dojo.string.substitute(this.nls.ERROR_BAD_EXT_BLACKLIST, [this.getName(), this.getExtension()])};
            }
         }
      }
   ...
   validateName: function() {
      ...
      var ext = this.getExtension().toLowerCase();
      var disableSameNameCheck = this._fileList ? !!this._fileList.disableSameNameCheck : false;
      var doDupeCheck = !disableSameNameCheck;
      ...
      if (dojo.isFunction(invalidCharsRE.test) && invalidCharsRE.test(name)) {
      ...
      var len = lconn.core.util.text.getByteLength(name);
   ...
   validateRemoteDuplicate: function() {
      ...
      if (dojo.isFunction(this.getOwningList().remoteFileExists)) {
         ...
         deferred.addBoth(dojo.hitch(this, function() {
            var idx = dojo.indexOf(checks, deferred);
            if (idx != -1)
               checks.splice(idx, 1);
         ...
         // in case of dojo1.5 the 404 response does not trigger callback,
         // so errback has to be registered for this case;
         // the solution is provided currently for all dojo versions < 1.6
         if ((dojo.version.major == 1 && dojo.version.minor < 6) || dojo.version.major < 1) {
            deferred.addErrback(dojo.hitch(this, function(error) {
   ...
   _shortenDisplayName: function(name) {
      if (!dojo.isString(name) || name.length <= 38)
         return name;
   ...
   _setRemoteDuplicateStatus: function() {
      ...
      if (this._isRemoteDuplicate && typeof this._isRemoteDuplicate == "object" && this._isRemoteDuplicate.nestedFolder) {
         ...
         if (this._isRemoteDuplicate.context && this._isRemoteDuplicate.context == "collection") {
            msgPre = this._isRemoteDuplicate.isMultiRemoteDuplicate ? this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_COLLECTION : this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_COLLECTION_1;
         }
         else if(this._isRemoteDuplicate.context && this._isRemoteDuplicate.context == "mydrive") {
            msgPre = this._isRemoteDuplicate.isMultiRemoteDuplicate ? this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_MYDRIVE : this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_MYDRIVE_1;
         }
         else {
            msgPre = this._isRemoteDuplicate.isMultiRemoteDuplicate ? this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_GLOBAL : this.nls.STATUS_REMOTE_DUPLICATE_RENAME_CONTEXT_GLOBAL_1;
         }
   ...
   shortenFilename: function(name, ext, maxLength) {
      ...
      var bytesName = lconn.core.util.text.getByteLength(name);
      var bytesExt = lconn.core.util.text.getByteLength(ext);
      ...
      if (bytesForName <= 0) {
         ...
         ext = lconn.core.util.text.trimToByteLength(ext, bytesForExt);
      }
      else
         name = lconn.core.util.text.trimToByteLength(name, bytesForName);
