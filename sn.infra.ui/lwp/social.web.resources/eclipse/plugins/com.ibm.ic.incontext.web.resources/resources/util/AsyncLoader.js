...
define([
	...
	var AsyncLoader = declare("com.ibm.social.incontext.util.AsyncLoader", null, {
	   ...
	   cancel: function() {
	      array.forEach(this._requests, function(request) { if (request && request.dojoRequest) request.dojoRequest.cancel(); });
	      this._requests = [];
	      this._plannedRequests = [];
	      this._runningRequests = 0;
	   },
	   ...
	   _requestSingle: function(url, handler, handleAs, timeout) {
	      ...
	      request.dojoRequest = this.net.get({
	         ...
	         url: url,
	         handle: lang.hitch(this, this.handleRequest),
	         timeout: timeout,
