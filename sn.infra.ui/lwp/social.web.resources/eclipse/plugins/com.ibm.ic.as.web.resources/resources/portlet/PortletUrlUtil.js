/* Copyright IBM Corp. 2013, 2015  All Rights Reserved.              */
	define([
		...
		"ic-core/url"
	], function (dojo, query, declare, array, lang, dom, domAttr, domClass, topic, registry, services, urlModule) {
		...
		var PortletUrlUtil = declare("com.ibm.social.as.portlet.PortletUrlUtil", null, {
		   connSvcUrls: null,
		   skipPocUpdate: false,
		   ...
		   constructor: function(portletUrlSettings) {
		      ...
		      for(var service in services) {
		          if(urlModule.getServiceUrl(services[service])) {
		              var svcString = urlModule.getServiceUrl(services[service]).toString();
		              if(lang.trim(svcString).length > 0)
		                 this.connSvcUrls.push(svcString);
		          }
		      }
		      //Set blogs url
		      this.blogsSvcUrl = services.blogs ? 
		         urlModule.getServiceUrl(services.blogs).toString() : "";
		
		      //Subscribe to like popup additions
		      topic.subscribe("com/ibm/oneui/likePopup/personAdded", lang.hitch(this, function(likePerson){
		    	 if(likePerson && likePerson.domNode)
		            this._updateUrls(likePerson.domNode);
		      }));
		   },
		   updateUrls: function(node) {
		      array.forEach(registry.findWidgets(node), lang.hitch(this, this.updateUrlsByWidget));
		   },
		   updateEEUrls: function(node, context) {
		      ...
		      if(node && context.id && checkItemUrl) {
		         ...
		         this._updateUrls(node, context.id, checkItemUrl, communityId , lang.hitch(this, this._resetEEPocExclusions), context.eventType, "_parent");
		      }
		   },
		   ...
		   updateUrlsByWidget: function(widget) {
			  if(widget && widget.domNode && widget.newsData && widget.newsData.object) {
		         ...
		         this._updateUrls(widget.domNode, id, url, 
		            ...
		            lang.hitch(this, this._resetActivityStreamPocExclusions, widget.domNode), eventType);
			  }
		   },
		   _resetEEPocExclusions: function(domNode) {
			  var updateFunc = function(resetTargetUpdate, link) {
				  if(domAttr.get(link, "connectionsWebUrl") && !domClass.contains(link, "lotusPerson") && 
			        		 link.parentNode && !domClass.contains(link.parentNode, "vcard") && !domClass.contains(link, "eeActionLinks")) {
			            domAttr.set(link, "href", domAttr.get(link, "connectionsWebUrl"));
				  }
				  if(resetTargetUpdate && domAttr.get(link, "connectionsOriginalTarget")) {
		              domAttr.set(link, "target", domAttr.get(link, "connectionsOriginalTarget"));
				  }
			  };
		      ...
		      query("div.eeCommentContent a, div.blogsWrapText a, div.lotusStatus a", domNode).forEach(lang.hitch(null, updateFunc, false));
		
		      //Don't change files download/preview type of links
		      query("a[id$='_downloadLink'], a[id$='_previewLink'], a[dojoAttachPoint='downloadLink'], a[dojoAttachPoint='prevLink']", domNode).forEach(lang.hitch(null, updateFunc, true));
		   },   
		   _resetActivityStreamPocExclusions: function(domNode) {
			 //Return user generated content links back to original url (activity stream summary and comments)
		      query("div.activityStreamSummary div.lotusPostContent a, div.activityStreamSummary div.lotusPostDetails a, li.lotusCommentItem div.lotusPostContent a", domNode).forEach(function(link){
		            if(link.parentNode && !domClass.contains(link.parentNode, "vcard")) {
		               if(domAttr.get(link, "connectionsWebUrl"))
		                  domAttr.set(link, "href", domAttr.get(link, "connectionsWebUrl"));
		               if(domAttr.get(link, "connectionsOriginalTarget"))
		                  domAttr.set(link, "target", domAttr.get(link, "connectionsOriginalTarget"));
		            }
		         });
		   },
		   _updateUrls: function(domNode, objectId, objectUrl, communityId, resetUrlsFunc, eventType, sameWindowTargetValue) {
		      query("a", domNode).forEach(lang.hitch(this,function(objectId, objectUrl, communityId, link){
		       	    var url = domAttr.get(link, "href");
		            if(this.isConnectionsUrl(url)  && !domAttr.get(link, "pocRun")) {
		               domAttr.set(link, "connectionsWebUrl", url);
		               if(!this.skipPocUpdate) {
		                  domAttr.set(link, "href", this.getUrl(lang.trim(url.toString()), communityId, objectId, objectUrl, eventType));
		               }
		               ...
		               if(domAttr.get(link, "target")) {
		                  domAttr.set(link, "connectionsOriginalTarget", domAttr.get(link, "target"));
		               }
		               if(this.openLinksNewWindow) {
		                  domAttr.set(link, "target", "_blank");
		               } else {
		                  domAttr.set(link, "target", sameWindowTargetValue || "_self");
		               }
		               domAttr.set(link, "pocRun", true);
		            }
		      ...
		      query("a[connectionsWebUrl],a[connectionsOriginalTarget]", domNode).forEach(function(link){
		    	  domAttr.remove(link, "connectionsWebUrl");
		    	  domAttr.remove(link, "connectionsOriginalTarget");
		      });
		   },
		   ...
		   isConnectionsUrl: function(url) {
		      if(!url || (url === "javascript:;"))
		         return false;
		      return  (url.indexOf(ECM_URL_PREFIX) == 0)|| array.some(this.connSvcUrls, lang.hitch(null, function(url, svcUrl) {
		    	  return url.indexOf(svcUrl) == 0;
		      }, url));
		   },
		   isProfilesUrl: function(url) {
			   var profileServiceFromConfig = services.profiles ? services.profiles : services.scprofiles;
			   return url && (url.indexOf(urlModule.getServiceUrl(profileServiceFromConfig).toString()) == 0);
		   },
