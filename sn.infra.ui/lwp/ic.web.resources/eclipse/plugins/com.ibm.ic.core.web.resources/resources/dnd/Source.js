...
define([
	...
	"dojo/query",
	"dojo/topic"
], function (dojo, declare, windowModule, Source, domClass, has, query, topic) {

	var Source = declare("lconn.core.dnd.Source", Source, {
		...
		onDndStart: function(/* dojo.dnd.Source */source,/* DOM Node */ nodes,/* Boolean */ copy){
			...
			windowModule.body().onselectstart = function() {
	        return false;
	    	};
	    	windowModule.body().unselectable = "on";
			
			lconn.core.dnd.Source.superclass.onDndStart.apply(this, arguments);
			
			// init cache used by onMouseMove
			this._children = query("> .dojoDndItem", this.parent);
		...
		onMouseDown: function(/* Event */ e){
			...
			var handle = query(".ibmDndDragHandle", this.current);		
			if (handle.length){			
		...
		onMouseOver: function(/* Event */e){
			...
			if (!this.isDragging){
				...
				if (handle.length){			
					if (e.target == handle[0]){
						domClass.add(e.target, "ibmDndDragHandleOver");
					}
				}	
			}		
		},
		...
		onMouseOut: function(/* Event */ e){
			...
			if (handle.length){			
				if (e.target == handle[0]){
					domClass.remove(e.target, "ibmDndDragHandleOver");
				}
			}		
		},	
		...
		onDndDrop: function(/* dojo.dnd.Source */ source,/* DOM Node array*/ nodes,/* Boolean */ copy){	
			...
			// description: - the event "/ic-core/dnd/drop" is published 
			...
			if (this.containerState == "Over")
				topic.publish("/ic-core/dnd/drop", source.node, this.node, nodes[0]);
		...
		onMouseMove: function(/* Event */ e){		
			...
			if(!this.isDragging){
				if(this.mouseDown && this.isSource &&
					...
					if(nodes.length){
						m.startDrag(this, nodes, this.copyState(has("copykey")(e), true));
						this.flag = true;	
					}
				}			
			}		
