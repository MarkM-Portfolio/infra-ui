...
define([ "ic-mm/enabler/debug"
   ...
   function(debug) {
      ...
      com.ibm.mm.enabler.xslt.transform = function(xml, xsl, sXslMode, aXslParams, bReturnString) {
         debug.entry("xslt.transform", xml, xsl, sXslMode, aXslParams, bReturnString);
         if (typeof ActiveXObject != "undefined") {
      ...
      com.ibm.mm.enabler.xslt.transformAndUpdate = function(/* HTMLElement */nodeToUpdate, /* XMLDocument */xml, /* XMLDocument */xsl, /* String? */sXslMode, /* Map */aXslParams) {
         debug.entry("xslt.transformAndUpdate", nodeToUpdate, xml, xsl, sXslMode, aXslParams);
         if (typeof ActiveXObject != "undefined") {
            ...
            // way IE will
            // take the update.
            debug.log("xslt.transformAndUpdate", "XSLT result: " + results);
            nodeToUpdate.innerHTML += results;
         }
         else {
            results = com.ibm.mm.enabler.xslt.gecko.transform(xml, xsl, sXslMode, aXslParams, false);
            debug.log("xslt.transformAndUpdate",
               "XSLT result: " + (new XMLSerializer()).serializeToString(results),
               "com.ibm.mm.enabler.xslt.transformAndUpdate");
            ...
            if (results.documentElement.tagName == "transformiix:result") {
                ...
                * 
                * com.ibm.mm.enabler.debug.log( "xslt.transformAndUpdate",
                * "there are " + toAppend.length + " items to add." );
                * 
                * for ( var c = 0; c < length; c++ ) {
                * com.ibm.mm.enabler.debug.log( "xslt.transformAndUpdate",
                * "Appending: " + ( new XMLSerializer() ).serializeToString(
                * toAppend[c] ), "com.ibm.mm.enabler.xslt.transformAndUpdate" );
                * com.ibm.mm.enabler.debug.log( "xslt.transformAndUpdate",
                * "toAppend = " + toAppend[c] + "; c=" + c + "; length=" +
                * length ); nodeToUpdate.appendChild( toAppend[c] ); }
                */
            }
            else {
               debug.log("xslt.transformAndUpdate",
                  "Appending2: " + (new XMLSerializer()).serializeToString(toAppend),
                  "com.ibm.mm.enabler.xslt.transformAndUpdate");
               nodeToUpdate.appendChild(toAppend);
            }
         }
         debug.exit("xslt.transformAndUpdate");
      };
      ...
      return com.ibm.mm.enabler.xslt;
   });
