...
define([
      ...
      "dojo/_base/lang",
      "dojo/i18n",
      "dojo/i18n!./nls/dataTypeStrings"
], function(dojo, declare, lang, i18n, i18ndataTypeStrings) {
   ...
   var datatypes = declare("com.ibm.mm.data.datatypes", null, /** @lends com.ibm.mm.data.datatypes */
   {
      ...
      constructor : function(/* String */datatypecfg) {
         this.resourceBundle = i18ndataTypeStrings;
      },
      ...
      getAllTypes : function() {
         return lang.clone(this._alltypes);
      },

      /**
       * The method to return if two types should be considered as a match. It
       * may need more work to include the combined data type mapping mechanism.
       * 
       * @param {String}
       *           typename1 The first data type name(handle event)
       * @param {String}
       *           typename2 The second data type name.(publish event)<b>
       * @returns {Boolean}
       */
      doesTypeMatch : function(/* String */typename1, /* String */typename2) {
         ...
         // if two types are same or either type is any, then they are
         ...
         if (typename1 == typename2 || typename1.toLowerCase() == 'any' || typename2.toLowerCase() == 'any') {
            ...
            switch (typeOfName1) {
               ...
               case this._SIMPLE_DATA_TYPE:
                  ...
                  switch (typeOfName2) {
                     // Two simple data types, use the mapping to decide whether
                     // they match or not
                     case this._SIMPLE_DATA_TYPE:
                        ...
                        break;
                     // Simple data type VS combine data type, compare the main
                     // type. (url.text's main type is :url)
                     case this._COMBINED_DATA_TYPE:
                        var mainTypeOfType2 = this.getMainType(typename2);
                        if (typename1 == mainTypeOfType2 || this._mappings[typename1][mainTypeOfType2] == 1) { // url
                                                                                                               // can
                                                                                                               // handle
                                                                                                               // url.text
                           doesItMatch = true;
                  ...
                  break;
               // For comebined type and complex data type as received data
               // type, currently, consider match only when type names are
               // exactly same
      ...
      /**
       * The method to return if two types should be considered as a match with
       * a much more flex rule on combined type: only cares about the mail type,
       * which mean
       * 
       * @param {String}
       *           typename1 The first data type name(handle event)
            ...
            switch (typeOfName1) {
               ...
               case this._COMBINED_DATA_TYPE:
                  ...
                  switch (typeOfName2) {
                     ...
                     case this._COMBINED_DATA_TYPE:
                        var mainTypeOfType2 = this.getMainType(typename2);
                        if (mainTypeOfType1 == mainTypeOfType2 || this._mappings[mainTypeOfType1][mainTypeOfType2] == 1) { // url
                                                                                                                           // can
                                                                                                                           // handle
                                                                                                                           // url.text
                           doesItMatch = true;
      ...
      /**
       * The method to return the mail type of a combined data type. If it's not
       * combined data type, For example: url.text will return url
       * 
       * @param {String}
       *           typename
       */
      ...
      checkDataType : function(/* String */typename) {
         ...
         else if (types.length == 2 || types.length == 3) { // type like:
                                                            // url.text ||
                                                            // text.countrycode
                                                            // ||
                                                            // url.text.countrycode
            var prefixType = types[0];
            var suffixType = types[types.length - 1];
            // In v1.1, only support url as prefix and
            // if (this._alltypes[prefixType] == 1 && this._alltypes[suffixType]
            // == 1)
      ...
      /**
       * Get the display name of the simple or complex data type. The rule here
       * is 1. For the simple type predefined, return the display name for this
       * locale 2. For user defined or complex data type, if it's composed with
       * English characters, return the date type with first letter capitalized
