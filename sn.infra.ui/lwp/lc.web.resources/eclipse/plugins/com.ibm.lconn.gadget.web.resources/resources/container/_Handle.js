...
{	
	...
	function getDomId_(nodeOrString) {
		if (nodeOrString) {
			if (dojo.isString(nodeOrString)) {
				return nodeOrString;
			} else {
				return dojo.attr(nodeOrString, 'id');
			}
		}
	 ...
	 */
	var WidgetHandleBase_ = dojo.declare('', Handle_, {
		_accessor : null,
		...
		constructor : function(accessor, widgetSpec, _promise) {
			...
			if (_promise) {
				...
				_promise.then(dojo.hitch(this, '_loaded'), dojo.hitch(this, this._failLoad));
			}
		},
		...
		load : function() {
			...
			if (this._state === Handle_.UNLOADED) {
				...
				this._promise = this._withRuntime(dojo.hitch(this, '_loading'), dojo.hitch(this, '_failLoad'));				
			}
		},
		...
		unload : function() {
			...
			if (state === Handle_.LOADING || state === Handle_.LOADED) {
				...
				this._promise.then(dojo.hitch(this, '_unload'), dojo.hitch(this, '_failUnload'));
			}
		},
	 ...
	 * @extends com.ibm.lconn.gadget.container.Handle
	 */
	var WidgetHandle_ = dojo.declare('', [WidgetHandleBase_], {
		...
		_loading : function(iRuntime) {
			return iRuntime.loadWidgets([specHelper_.cloneSpec(this._widgetSpec)])
				.then(dojo.hitch(this, '_loaded', iRuntime), dojo.hitch(this, '_failLoad'));
		},
	 ...
	 * @extends com.ibm.lconn.gadget.container.Handle
	 */
	var CommonContainerBase_ = dojo.declare('', [WidgetHandleBase_], {
		_gadgetSite : null,
	 ...
	 * @extends com.ibm.lconn.gadget.container.Handle
	 */
	var GadgetHandle_ = dojo.declare('', [CommonContainerBase_], {
		...
		_loading : function(cc) {
			...
			var gadgetSite = this._gadgetSite || (this._gadgetSite = cc.newGadgetSite(dojo.byId(widgetSpec.placement)));
			...
			container.navigateGadget(
					...
					viewParams, 
					renderParams,
					dojo.hitch(this, '_loaded', cc, loadedPromise));
	 ...
	 * @extends com.ibm.lconn.gadget.container.Handle
	 */
	var EEHandle_ = dojo.declare('', [CommonContainerBase_], {
		_loading : function(cc) {
			var widgetSpec = this._widgetSpec,
				instanceData = widgetSpec.instanceData || {},
				eeRenderParams = {},
			...
			cc.ee.navigate(
				dojo.byId(widgetSpec.placement), eeDataModel, eeRenderParams, 
				dojo.hitch(this, '_loaded', cc, loadedPromise)
			);
