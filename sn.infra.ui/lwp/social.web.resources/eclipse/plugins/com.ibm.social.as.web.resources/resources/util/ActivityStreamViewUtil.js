...
dojo.provide("com.ibm.social.as.util.ActivityStreamViewUtil");
...
dojo.declare("com.ibm.social.as.util.ActivityStreamViewUtil", null,
{
	/**
	...
	getLastNewsItem: function(){
		...
		var lastChildren = dojo.query("> li.lastNewsItem:last-child", this.newsFeedNode);
		...
		if(lastChildren && lastChildren.length > 0){
			...
			if(lastChild && lastChild.id){
				...
				return dijit.byId(lastChild.id);
			}
		}
	...
	getFirstNewsItem: function(){
		...
		if(firstChild && firstChild.id){
			// Return the dijit with this id
			return dijit.byId(firstChild.id);
		}
		return null;
	},
	...
	containsNewsItemWithId : function(id) {
		var dij;
		return dojo.some(this.newsFeedNode.childNodes, function(node){
			if(node && (dij = dijit.byId(node.id)) && dij.newsData && dij.newsData.getId().indexOf(id)) {
				return true;
			}
		});
	}
