...
dojo.provide("com.ibm.social.as.item.data.NewsDataAccessor");
dojo.require("com.ibm.social.as.item.data.interfaces.INewsDataAccessor");
...
dojo.declare("com.ibm.social.as.item.data.NewsDataAccessor", 
			  com.ibm.social.as.item.data.interfaces.INewsDataAccessor,
{
	activity: null,
	...
	getActivitySummaryValidated: function() {
		...
		if ( srcSummary ) {
			var tmpDiv = dojo.create("div", { innerHTML: srcSummary});
			var txtContent = dojo.isIE ? tmpDiv.innerText: tmpDiv.textContent;
			txtContent = txtContent ? dojo.trim(txtContent) : null;
			if ( txtContent ) {	// is there any actual text
				ret = srcSummary;
			} else if ( dojo.query("img", tmpDiv).length > 0 ) { // or any images
				ret = srcSummary;
			}
		}
		return ret;
		...
		if (targetType === "person") {
			...
			if (targetType === "note") {
				...
				if (targetUrl) {
					...
					boardUserId = dojo.queryToObject(params).userid;
				}
			}
		}
	...
	isActivityExternal : function(){
		...
		if(!com.ibm.social.as.config.enablement.checkEnablement(com.ibm.social.as.config.enablement.AS_SHARED_EXTERNALLY) || this.isSharedExternallyDisabledOnActivityStream()) {
            return false;
        }
