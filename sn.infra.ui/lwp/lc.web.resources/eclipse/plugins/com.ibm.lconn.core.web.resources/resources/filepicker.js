...
(function(picker) {
    ...
    *
    *   Opens a dialog with a list of files stored in Connections Files, allowing consumers
    *  to select files and interact with them via a consumer-defined callback function.
    *  
    *  This interface uses (__link lconn.files.picker.files#open}
    *    
    ...
    *        - onSave: the callback function. Accepts an array of (__link lconn.share.bean.File} objects.
    ...
    *        - sourceSet: An array that decides which sources will be shown in the picker. The item value in the array could be "myfiles", "mycomputer".
    *                    By default the value will be ["myfiles", "mycomputer"].
    ...
    *       - hideCheckbox: Boolean. If <code>true</code>, checkbox/radio button is not displayed before file or folder.
    *       - hideBreadcrumbWithRootOnly: Boolean. Speicfy whether to hide breadcrumb if it only has root node. 
    *       - breadcrumbPrefix: Specify a prefix to decorate breadcrumb in picker
    *       - sourceSwitchStyle: only relevant, if <code>useCompact = true</code>; specifies styling of source switching area 
    *         possible values: <code>"radio"</code> or <code>"tab"</code>
    *         <code>"radio"</code>: Sources are switched via radio buttons
    *         <code>"tab"</code>: Sources are switched via tabs with icons
    *         If no value is provided then a default value is chosen depending on gatekeeper feature flag files-picker-dnd
    *             files-picker-dnd ? "tab" : "radio"
    *       - disabledCss: An array that defines the css files been prohibited from loading in Picker widget. Currently there is only one option: <code>sprites</code>.
    *       If <code>true</code>, the picker will NOT load Connections css files. Default to <code>false</code>.
	 *       - theme:  Using this parameter to load specified theme css, support values: gen4,hikari,blue,green,red,gold,onyx,orange,purple,silver.
    ...
    *        - uploadToCommunity: If <code>true</code>, the upload file will check community library, and will eventually upload selected file to community library. <code>community<code> parameter must be set to make this parameter take effect. 
   ...
   /**
    * lconn.core.filepicker.openInline
    *
    * Opens a Embedded widget with a list of files stored in Connections Files, allowing consumers
    *  to select files or upload local files, and interact with them via a consumer-defined callback function.
    *  
    *  This interface uses (__link lconn.files.picker.files#openInline}
    *    
    *  @param args A hash object with the following properties:
    *    - Required:
    *       - filesService: the config object representing the files, retrieved via
    ...
    *       - hideBreadcrumbWithRootOnly: Boolean. Speicfy whether to hide breadcrumb if it only has root node. 
    *       - breadcrumbPrefix: Specify a prefix to decorate breadcrumb in picker
    *       - sourceSwitchStyle: specifies styling of source switching area 
    *         possible values: <code>"radio"</code> or <code>"tab"</code>
    *         <code>"radio"</code>: Sources are switched via radio buttons
    *         <code>"tab"</code>: Sources are switched via tabs with icons
    *         If no value is provided then a default value is chosen depending on gatekeeper feature flag files-picker-dnd
