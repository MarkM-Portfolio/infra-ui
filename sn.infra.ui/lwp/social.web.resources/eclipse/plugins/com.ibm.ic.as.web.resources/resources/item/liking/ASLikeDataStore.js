/* Copyright IBM Corp. 2012, 2015  All Rights Reserved.              */
	...
	define([
		...
		"dojo/i18n!ic-ublog/nls/InlineLiking",
		"dojo/string",
		"ic-ui/layout/people",
		"ic-as/constants/events",
		"ic-core/util/LCDeferred"
	], function (dojo, declare, lang, domConstruct, i18nactivitystream, i18nInlineLiking, string, people, events, LCDeferred) {
		...
		var ASLikeDataStore = declare("com.ibm.social.as.item.liking.ASLikeDataStore", [],
		{
			_items: null,
			_newsItem: null,
			_firstFetch: true,
			...
			constructor: function(newsItem) {
				...
				this._likeStatusPromise = new LCDeferred();
				...
				this._ublogStrings = i18nInlineLiking;
				this._asStrings = i18nactivitystream;
			...
			_getItems: function() {
				...
				if ( this._getItemsPromise == null ) {
					this._getItemsPromise = new LCDeferred();
		
					// need to fetch the items
					// we need to request the feed of likes
					this.getLikes().then(lang.hitch(this, function(items) {
						this._items = items;
			...
			fetch: function(request) {
				...
				if ( this._firstFetch ) {
					...
					this._getItems().then(lang.hitch(this, function(items) {
			...
			fetchItemByIdentity: function (request) {
				...
				if ( this._curUserId == request.identity ) {
					...
					if ( request.onItem ) {
						var scope = request.scope ? request.scope : window;
			...
			save: function(saveArgs) {
				...
				if ( this._dirtyItem ) {
					...
					if ( item.isNew ) { // new item == news item was liked
						...
						this.like(item).then(lang.hitch(this, function() {
						...
						this.unlike(item).then(lang.hitch(this, function() {
				...
				if ( error.status === 403 ) {
					...
					if ( d && d.getGeneratorId() === "communities" ) {
						...
						/*if ( communityId && communityName ) {	
							...
							msg = string.substitute(this._joinCommunitySpecificStr, [tempDiv.innerHTML]);
		
		                    msg = domConstruct.create("div", {innerHTML: msg});
						} */
					}
				} else {
			...
			getUserProfileUrl: function(id) {
				if(people.getProfileUrl)
					return people.getProfileUrl({id: id});
				else
					return id;
			},
