...
lconn.share.util.atom.getErrorCode = function(response,ioArgs) {
   var error = lconn.share.util.atom.getError(response,ioArgs);
   return (error ? error.code : null);
}

lconn.share.util.atom._CMIS_ERROR_CODES = { 
...
lconn.share.util.atom.getError = function(response,ioArgs,opts) {
   ...
   if (isError) {
      ...
      if (!error.code && statusCode == 412 && xhr && xhr.responseXML && xhr.responseXML.documentElement && lconn.share.util.dom.isNamedNS(xhr.responseXML.documentElement, "entry", lconn.share.util.dom.NAMESPACES.ATOM)) {
      ...
      if (!error.code && xhr && xhr.responseXML && xhr.responseXML.documentElement) {
         ...
         var text = lconn.share.util.dom.getChildElementTextContentNS(el, "errorCode", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE);
         
         // Check CMIS error structure
         if (!text) {
            text = lconn.share.util.dom.getChildElementTextContentNS(el, "code", lconn.share.util.dom.LCMIS_NAMESPACE);
            if(text)
               text = lconn.share.util.atom._CMIS_ERROR_CODES[text] || "unknown";
         }
         ...
         if (code.length > 0) {
            ...
            error.el = el;
            error.message = lconn.share.util.dom.getChildElementTextContentNS(el, "errorMessage", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE);
            
            // Check CMIS error structure
            if (!error.message)
               error.message = lconn.share.util.dom.getChildElementTextContentNS(el, "message", lconn.share.util.dom.LCMIS_NAMESPACE);
            
            if (code == "ConstraintViolation") {
               error.violationType = lconn.share.util.dom.getChildElementTextContentNS(el, "violationType", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE);
            }
            
            if (code == "QuotaViolation") {
               error.libraryQuota = parseInt(lconn.share.util.dom.getChildElementTextContentNS(el, "libraryQuota", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE));
               error.librarySize = parseInt(lconn.share.util.dom.getChildElementTextContentNS(el, "librarySize", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE));
               error.uploadSize = parseInt(lconn.share.util.dom.getChildElementTextContentNS(el, "uploadSize", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE));               
            }
            else if ( code == "ExtensionViolation") {
               error.extension = lconn.share.util.dom.getChildElementTextContentNS(el, "extension", lconn.share.util.dom.DOCUMENTS_ATOM_NAMESPACE);
            }
         }
      }
      ...
      if (!error.code && (statusCode == 502 || statusCode == 504))
      	error.message = error.code = "ProxyTimeout";
...
lconn.share.util.atom.getJsonResponseFromHtml = function(d) {
   ...
   try {
      if (d && d.body) {
         ...
         try {
            var text = lconn.share.util.atom._htmlText(d.body);
...
lconn.share.util.atom.getJsonError = function(r) {
   ...
   if (r.status == 412) {
      ...
      error.code = lconn.share.util.atom._CMIS_ERROR_CODES[r.contents.code] || "unknown";
      for (var key in r.contents)
...
lconn.share.util.atom.detectLoginForMultipartResponse = function(response,ioArgs) {
   var login = false;
