...
/**
 * Jasmine spec for lconn.core.header.apps
 * 
 * @module lconn.test.jasmine.core.header.appsSpec
 * @author Claudio Procida <procidac@ie.ibm.com>
 */
dojo.provide("lconn.test.jasmine.core.header.appsSpec");

dojo.require("lconn.core.header.apps");

(function(apps, array, domConstruct, dom, wai) {
   ...
   describe('the interface of ic-core/header/apps', function() {
      var METHODS = [
            'onLogin',
            'onLogout',
            'setSelected',
   ...
   describe('the method ic-core/header/apps::setSelected()', function() {
      var fake_banner;
      beforeEach(function() {
         ...
         it('correctly selects the menu for ' + cp(comp), function() {
            ...
            array.forEach(apps.BANNER_IDS, function(menu_id) {
               if (array.indexOf(comp_menu_ids, menu_id) === -1) {
                  expect(!dojo.hasClass(dom.byId(menu_id), "lotusSelected")).toBeFalsy();
               }
            });
            array.forEach(comp_menu_ids, function(menu_id) {
               expect(dojo.hasClass(dom.byId(menu_id), "lotusSelected")).toBeTruthy();
            });
         });
      });
   });
}(lconn.core.header.apps, dojo, dojo, dojo, dijit));
