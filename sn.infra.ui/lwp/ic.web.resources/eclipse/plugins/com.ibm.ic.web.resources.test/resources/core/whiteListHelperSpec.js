...
define([
   ...
   describe("the constructor of whiteListHelper", function() {
      it("throws when the first argument is null", function() {
         expect(function(){ return new Helper(null); }).toThrow();
         expect(function(){ return new Helper(null, PROXY_URL); }).toThrow();
      });
      it("throws when the first argument is undefined", function() {
         ...
         expect(function(){ return new Helper(); }).toThrow();
         expect(function(){ return new Helper(undefined, PROXY_URL); }).toThrow();
      });
      it("doesn't throw when the first argument is not null", function() {
         expect(function(){ return new Helper(SERVICE_JSON); }).not.toThrow();
         expect(function(){ return new Helper(SERVICE_JSON, PROXY_URL); }).not.toThrow();
      });
   });
   ...
   describe("the initializer of whiteListHelper", function() {
      it("stores the expected URL patterns without port numbers", function() {
         ...
         expect(array.indexOf(helper._list, 'www.example.com/blogs')).not.toBe(-1);
         expect(array.indexOf(helper._list, 'www.example.com/activities')).not.toBe(-1);
      });
      it("stores the expected URL patterns with port numbers", function() {
         ...
         expect(array.indexOf(helper._list, 'www.example.com:9080/blogs')).not.toBe(-1);
         expect(array.indexOf(helper._list, 'www.example.com:9443/blogs')).not.toBe(-1);
         expect(array.indexOf(helper._list, 'www.example.com:9080/activities')).not.toBe(-1);
         expect(array.indexOf(helper._list, 'www.example.com:9443/activities')).not.toBe(-1);
      });
   });
   
   describe("the method whiteListHelper.isWhiteListedURL", function() {
      var helper, helper_with_ports, helper_with_proxy, helper_with_ports_and_proxy;
      beforeEach(function() {
