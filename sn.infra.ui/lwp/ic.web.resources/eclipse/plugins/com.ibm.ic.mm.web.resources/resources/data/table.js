...
define([
      ...
      "dojo/_base/lang",
      "dojox/data/CsvStore"
], function(declare, lang, CsvStore) {
    ...
    */
   var table = declare("com.ibm.mm.data.table", CsvStore,
   /** @lends com.ibm.mm.data.table */
   {
      ...
      constructor : function(/* Object */keywordParameters) {
         ...
         // keywordParameters: {label: String} The column label for the column
         // to use for the label returned by getLabel.
         ...
         // below code copied from dojox.data.CsvStore as super class's
         ...
         this._storeProp = "_csvStore"; // Property name for the store
                                          // reference on every item.
         this._idProp = "_csvId"; // Property name for the Item Id on every
                                    // item.
         this._features = {
         ...
         this._loadInProgress = false; // Got to track the initial load to
                                       // prevent duelling loads of the dataset.
         this._queuedFetches = [];
      },

      _isCSVLike : function(csvlike) {

         var data = lang.clone(csvlike);

         if (lang.isString(data) && data != null && data.length > 0) {
            data = data.replace(/\n/g, "");
            var rows = data.split('|');
            ...
            for (var i = 1; i < rows.length; i++) {
               ...
               if (!rowList || rowList.length != columnLength)
                  return false;
            }
      ...
      _CSVLike2CSV : function(/* String */csvlike) {
         /*
          * There are two differences between CSV data and CSV like data 1. add
          * CSV like data's type information to CSV column row 2. replace row
          * separator '|' with '\n'
          */

         var typeArray = [ 'string'
         ];/* add possible CSV like data's type term here */

         function _inArray(item, arrayData) {
            for (var i = 0; i < arrayData.length; i++) {
               if (item.toUpperCase() == arrayData[i].toUpperCase())
                  return true;
            }
            return false;
         }
         ...
         if (lang.isString(data) && data != null && data.length > 0) {
            ...
            if (rows.length >= 2) {
               ...
               if (isTypeRow) {
                  ...
                  columnList = _addTypeInfo(columnList, null/*
                                                             * add default text
                                                             * type
                                                             */);
                  csv.push(columnList.join(","));
       ...
       * this is to make sure that the the Csv store handles the data types
       * correctly.<b> will also add this._attributeTypes for a given
       * attribute.
       */
      ...
      /**
       * summary: Returns data type of the given *item*
       * 
       * @param {Object}
      ...
      /**
       * This method will serialize the data object to a format that can be
       * persisted. If this table has not been loaded, that is, if that the
       * fetch method has not been called, then the serialize method may not
       * serialize the entire data set. We probaby have to escape the double
       * quotes.
       * 
       * @returns {Object} a serialized data object. For most of the data this
