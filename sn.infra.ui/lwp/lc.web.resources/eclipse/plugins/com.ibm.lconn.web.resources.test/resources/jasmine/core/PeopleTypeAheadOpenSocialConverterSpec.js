...
dojo.provide("lconn.test.jasmine.core.PeopleTypeAheadOpenSocialConverterSpec");
dojo.require("lconn.core.PeopleTypeAheadOpenSocialConverter");

(function(PeopleTypeAheadOpenSocialConverter) {
   describe('the lconn.core.PeopleTypeAheadOpenSocialConverter module', function() {
      var osCoverterSpec;
      beforeEach(function() {
         osCoverterSpec = new PeopleTypeAheadOpenSocialConverter();
      });

      describe('the interface', function() {
         it('implements the expected methods', function() {
            expect(osCoverterSpec.convertOpenSocialPeopleTypeAheadFeed).toBeDefined();
            expect(osCoverterSpec._createWrapper).toBeDefined();
            expect(osCoverterSpec._convertItem).toBeDefined();
            expect(osCoverterSpec._getEmail).toBeDefined();
         });
      });
      ...
      describe('the method convertOpenSocialPeopleTypeAheadFeed', function() {
         ...
         it('converts the OpenSocial feed into a feed the PeopleTypeAhead can consume', function() {
            expect(osCoverterSpec.convertOpenSocialPeopleTypeAheadFeed(osFeed)).toEqual(expectedRes);
         });
      });

      describe('the method _createWrapper', function() {
         it('creates a wrapper for the people items', function() {
            expect(osCoverterSpec._createWrapper()).toEqual({
               identifier : "member",
               label : "name",
               items : []
            });
      ...
      describe('the method _convertItem', function() {
         ...
         it('creates an individual people item consumable by the PersonTypeAhead', function() {
            ...
            expect(osCoverterSpec._convertItem(osItem)).toEqual(expectedResult);
         });
      });
      ...
      describe('the method _getEmail', function() {
         ...
         it('returns the primary email', function() {
            ...
            expect(osCoverterSpec._getEmail(emails)).toBe('primary@ibm.com');
         });
         it('returns the first email in the array when no primary email found', function() {
            ...
            expect(osCoverterSpec._getEmail(emails)).toBe('work@ibm.com');
         });
      });
   });
}(lconn.core.PeopleTypeAheadOpenSocialConverter));
