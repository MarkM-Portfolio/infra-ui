 ...
 undef) 
{
	...
	var creResUrl_ = lconn_core_config_services.opensocial,
		creResUrlObj_ = lconn_core_url.parse(
			dojo.getObject("location", dojo.global).toString(), 
			creResUrl_ ? 
	...
	function ensureReauthTokenFromProxy_(userData) {
		...
		if (!checkSameUserForAllVals_(userData, cookieVals)) {
			...
			dojo.cookie(COOKIE_CONTAINER_TOKEN_CHECK_, cookieValue , {path: '/'});
		}
	}
	...
	api_ = dojo.declare('', [], {
		...
		getContainerToken : function(loadCREPromise) {
			...
			if (typeof(window['__isgadget']) == 'undefined') { // only get a container token when not running within a gadget
				p.then(function() {
					...
					u = (u ? u : (lconn_core_auth ? lconn_core_auth.getUser() : null));
					if (u) {
					   sha1.update(u);
					   u = sha1.digestString();
					...
					dojo.xhrGet({
						...
						headers : headers,
						sync: false,
						load: dojo.hitch(self, '_creTokenDataHandlerLoad', tokenLoadPromise),
						error: dojo.hitch(self, '_creTokenDataHandlerError', tokenLoadPromise)		
					});	//xhrGet
				}); // p.then
			}
		 ...
		 * @private
		 */
		_creTokenDataHandlerLoad : function(tokenLoadPromise, data, ioargs) {
		...
		logoutHandler : function() {
			...
			dojo.cookie(COOKIE_CONTAINER_TOKEN_CHECK_, undef, {expires: -1, domain: securitySettings_.ssoDomain, path: '/'});
			dojo.cookie(COOKIE_CONTAINER_TOKEN_CHECK_, undef, {expires: -1, path: '/'});
		}
	});	
