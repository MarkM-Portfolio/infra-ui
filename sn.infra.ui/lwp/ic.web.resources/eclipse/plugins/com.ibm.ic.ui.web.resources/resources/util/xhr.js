...
define([
    ...
    * 
    * @param {Function}
    *           interceptor the request inteceptor
    */
...
//   }
    ...
    * the request (i.e. the request is pending), the request manager will remove
    * the request from the pending queue.
    * 
    * @param {Object}
    *           request the request to cancel
    */
   function cancelRequest(request) {
      if (request.remoteDeferred) {
         ...
         var i;
         for (i = 0; i < pendingRequestQueue.length; i++) {
    ...
    * returned to the calling client. Using this method we can hold references
    * to Requests across asyncronous events and resend requests (e.g. in the
    * case of an authentication challenge).
    * 
    * @param {Object}
    *           method the HTTP method
    * @param {Object}
    *           args the dojo.__xhrArgs object
    * @param {Object}
    *           hasBody whether or not the request has the body
    * @param {Object}
    *           context the calling RequestManager
    */
   function Request(method, args, hasBody, context) {
      ...
      // return request && lang.isFunction(request.getID) && request.getID() ===
      // self.id;
      // }
...
//   }
    ...
    *           args the dojo.__xhrArgs object
    * @param {Object}
    *           hasBody whether or not the request has a body
    */
    ...
    * 
    * @param {Object}
    *           xhrObj the XHR object associated with a remote request
    */
   ...
   var processRequest = function(request) {
      var i;
      for (i = 0; i < requestInterceptors.length; i++) {
         requestInterceptors[i](request);
      }

      request.args.failOk = true;

      // if present, temporarily strip out 'load', 'error', and 'handle'
      // callback
      ...
      var argFunctionNames = [
                              "load",
                              "error",
                              "handle"
      ];
      array.forEach(argFunctionNames, function(cbName) {
      ...
      request.remoteDeferred.addBoth(function(dojoResponse) {
         ...
         var i, cancel = false;
         for (i = 0; i < responseInterceptors.length; i++) {
    ...
    * @return {dojo.Deferred} a dojo.Deferred object that will be called when
    *         the remote request returns
    */
...
//   }
    ...
    * requests results in a pending state, this method will halt sending remote
    * requests until it is called again.
    */
...
//            request = pendingRequestQueue.shift();
...
//   }
   ...
   /**
    * Calls any request interceptors, calls dojo.xhr, and then calls any
    ...
    * @since 0.7
    */
   var f = function xhr(/* String */method, /* dojo.__XhrArgs */args, /* Boolean? */hasBody) {
      /*
       ...
       * //console.warn("Adding cache-busting request param for IE " + dojo.isIE + "
       * GET request"); args.preventCache = true; }
       */
   ...
   /**
    * Adds a response interceptor that will be executed after responses return
    * from the server but before any callbacks are called. If more than one
    * interceptor is specified, they are called in the order in which they were
    * added.
    * 
    * @param {Function}
    *           Function interceptor
