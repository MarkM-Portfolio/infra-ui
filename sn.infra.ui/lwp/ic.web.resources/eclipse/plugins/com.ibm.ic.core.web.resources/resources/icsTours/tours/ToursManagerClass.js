...
define(['dojo/request/xhr', 'dojo/Deferred', './toursRegistry', './StorageAdaptor', './hopscotch'],
		function (xhr, Deferred, toursRegistry, StorageAdaptor, hopscotch) {
  ...
  ToursManagerClass.prototype = {
    ...
    startTour: function (tourId, skipDecisionTree) {
      ...
      var that = this;
      if (!skipDecisionTree) {
        this._shouldShowTour(tour).then(function(show){
        	if (show) {
        		that._dispatchShowTourEvent(tour);
			    if (that.hopscotch.getCurrTour()) {
			    	that.hopscotch.endTour();
			    }
			    that.hopscotch.startTour(tour);
			}
		});
      }
    },
    ...
    _addEscListener: function() {
        ...
        if (elements.length > 0) {
          ...
          if (tour.type === this.toursRegistry.GUIDED_TOUR || tour.id === 'whats-new' || tour.type === this.toursRegistry.JIT_TOUR) {
            elements[0].focus();
          }
        }
      },
    ...
    _shouldShowJitTour: function(tour) {
      ...
      var openSocialUri = lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial).uri;
      
      if (!tourSeen) {
		if(tour.id == "jit-homepage-mypage"){
			dataTour = {"appData":{"userSettings":{"homeTours":tour.id}}};
			dataTour = JSON.stringify(dataTour);
			promise = xhr(openSocialUri+'/rest/people/@me/@self?fields=userSettings.homeTours',{
				method: "GET",
				handleAs: "json"
			}).then(function(data){
				if(data.entry.appData.userSettings.homeTours == tour.id){
					that.storageAdaptor.addSeenTourId(tour);
					return false;
				}
				else{
					xhr(openSocialUri+'/basic/rest/people/@me/@self',{
						method: "PUT",
		...
		else if(tour.id == "jit-communities"){
			dataTour = {"appData":{"userSettings":{"commTours":tour.id}}};
			dataTour = JSON.stringify(dataTour);
			promise = xhr(openSocialUri+'/rest/people/@me/@self?fields=userSettings.commTours',{
				method: "GET",
				handleAs: "json"
			}).then(function(data){
				if(data.entry.appData.userSettings.commTours == tour.id){
					that.storageAdaptor.addSeenTourId(tour);
					return false;
				}
				else{
		...
		else if(tour.id == "jit-files-nov18"){
			dataTour = {"appData":{"userSettings":{"filesTours":tour.id}}};
			dataTour = JSON.stringify(dataTour);
			promise = xhr(openSocialUri+'/rest/people/@me/@self?fields=userSettings.filesTours',{
				method: "GET",
				handleAs: "json"
			}).then(function(data){
				if(data.entry.appData.userSettings.filesTours == tour.id){
					that.storageAdaptor.addSeenTourId(tour);
					return false;
				}
				else{
			...
			function(err){},
			function(evt){});
		}else if(tour.id == "jit-metrics"){
			dataTour = {"appData":{"userSettings":{"metricsTours":tour.id}}};
			dataTour = JSON.stringify(dataTour);
			promise = xhr(openSocialUri+'/rest/people/@me/@self?fields=userSettings.metricsTours',{
				method: "GET",
				handleAs: "json"
			}).then(function(data){
				if(data.entry.appData.userSettings.metricsTours == tour.id){
					that.storageAdaptor.addSeenTourId(tour);
					return false;
				}
				else{
