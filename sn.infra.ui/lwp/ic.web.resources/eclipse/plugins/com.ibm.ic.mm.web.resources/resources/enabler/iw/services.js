...
define([
      ...
      "jazz/inverted!../utilities",
      "../xpath"
],
   function(query, declare, lang, dom, domClass, domStyle, JSON, string, topic, Uuid, generateTimeBasedUuid, debug, eventModule, parser, widgetImpl, ConfigService, utilities, xpath) {

      declare("com.ibm.mm.enabler.iw.services.iWidgetFragmentService", null, {

         createItem : function(itemName, defaultValue, defaultLang, ns) {
            if (typeof ns == "undefined" || ns == null)
               ns = "mm_";
            var item = document.createElement("a");
            domClass.add(item, ns + iwConstants.CSSCLASS_INSTANCE.iwItem);
            domStyle.set(item, "visibility", "hidden");
         ...
         createItemSet : function(itemSetName, ns) {
            ...
            domClass.add(param, ns + iwConstants.CSSCLASS_INSTANCE.iwItemSet);
            param.setAttribute("title", itemSetName);
            domStyle.set(param, "visibility", "hidden");
         ...
         createWidgetDefRef : function(widgetDefUrl, ns) {
            ...
            domClass.add(aTag, ns + iwConstants.CSSCLASS_INSTANCE.iwDefinition);
            aTag.setAttribute("href", widgetDefUrl);
            domStyle.set(aTag, "visibility", "hidden");
            domStyle.set(aTag, "display", "none");
            return aTag;
         },
         getItemSet : function(iwidgetId, name, ns) {
            if (typeof ns == "undefined" || ns == null)
               ns = "mm_";
            var rc = null;
            var widgetSpan = dom.byId(iwidgetId);
            var itemsets = query("." + ns + iwConstants.CSSCLASS_INSTANCE.iwItemSet, widgetSpan);
         ...
         getItem : function(parent, name, ns) {
            if (typeof ns == "undefined" || ns == null)
               ns = "mm_";
            var rc = null;
            var items = query("." + ns + iwConstants.CSSCLASS_INSTANCE.iwItem, parent);
         ...
         getWidgetDefRef : function(iwidgetId, ns) {
            ...
            var rc = null;
            var widgetSpan = dom.byId(iwidgetId);
            var def = query("." + ns + iwConstants.CSSCLASS_INSTANCE.iwDefinition, widgetSpan)[0];
...
//      });
//
//      declare("com.ibm.mm.enabler.iw.services.eventService", null, {
//         constructor : function() {
//            this.wires = {};
//         },
//         subscribeWire : function(/* String */sourceWidget,/* String */sourceEvent,/* String */targetWidget,/* String */targetEvent) {
//            debug.entry("eventService.subscribeWire", "source:" + sourceWidget + " sourceEvent:" + sourceEvent + " targetWidget:" + targetWidget
...
//            }
//            else {
//               if (utilities.inStringArray(targetEvent, subscribingEvents)) {
...
//            debug.exit("eventService.subscribeWire", "wire is added " + this.wires[sourceWidget][sourceEvent][targetWidget].length);
//            return rc;
//         },
...
//            debug.entry("eventService.publishWire", "source:" + sourceWidget + " sourceEvent:" + sourceEvent + " payload:" + payload + " payloadType:"
//                  + payloadType);
//
//            if (typeof sourceWidget == "undefined" || sourceWidget === null) {
//               return false;
...
//               debug.log("eventService.publishWire targetWidget:" + targetWidget);
...
//            // remove all the Wires that's subscribing this Widget's publish
//            // events
...
//
//            debug.entry("eventService.addWire", "source:" + sourceWidget + " sourceEvent:" + sourceEvent + " targetWidget:" + targetWidget + " targetEvent:"
//                  + targetEvent);
//            if (typeof sourceWidget == "undefined" || sourceWidget === null) {
//               return false;
//            }
...
//
//            debug.entry("eventService.removeWire", "source:" + sourceWidget + " sourceEvent:" + sourceEvent + " targetWidget:" + targetWidget + " targetEvent:"
//                  + targetEvent);
//            if (typeof sourceWidget == "undefined" || sourceWidget === null) {
//               return false;
//            }
...
//
//            debug.entry("eventService.fireEvent", "sourceWidget:" + sourceWidget + " targetWidget:" + targetWidget + " targetEvent:" + targetEvent
...
//               debug.log("eventService.fireEvent", "targetWidget is loaded");
//               // dojo.publish("/enabler/eventService/"+targetWidget,data);
//               topic.publish("/enabler/eventService/" + targetWidget, targetEvent, aEvent);
//            }
//            else {
//               debug.log("eventService.fireEvent", "targetWidget is not loaded");
//               var eventHolder = new com.ibm.mm.enabler.iw.eventHolder(targetWidget, data);
//               eventHolder.handler = topic.subscribe("/enabler/widgetLoaded/" + targetWidget, lang.hitch(eventHolder, "handleLoaded"));
//            }
//
//         },
//         // support pagechanged event and widget deleted event
//         publishEvent : function(sourceEvent, payload) {
//            debug.entry("eventService.publishEvent", " sourceEvent:" + sourceEvent);
//            if (typeof sourceEvent == "undefined" || sourceEvent === null) {
//               return;
//            }
//            if (typeof payload == "undefined" || payload === null) {
//               topic.publish(sourceEvent);
//            }
//
//            var args = [];
//            if (lang.isArray(payload)) {
//               args = payload;
//            }
//            else {
//               args.push(payload);
...
//            topic.publish(sourceEvent, payload);
//         },
//         subscribeEvent : function(event, object, handlerFn) {
//            topic.subscribe(event, lang.hitch(object, handlerFn));
//         }
//      });
//
//      declare("com.ibm.mm.enabler.iw.services.queryService", null, {
//
...
//            debug.entry("queryService.getWidgetById", "id:", id, "obj:", obj, "cb", cb);
//
//            var widget = iWidgetContainer.getWidgetById(id);
//            if (typeof widget == "undefined" || widget === null) {
//               return null;
//            }
//            if (widget.loaded) {
//               debug.log("queryService.getWidgetById", "widget is loaded");
...
//            else {
//
//               debug.log("queryService.getWidgetById", "widget is  not loaded");
//               if (typeof obj != "undefined" && typeof cb != "undefined") {
//                  var eventHolder = {};
...
//                  };
//                  topic.subscribe("/enabler/widgetLoaded/" + id, lang.hitch(eventHolder, "handleLoaded"));
//               }
//               else {
//                  // return new com.ibm.mm.enabler.iw.WidgetStub(widget);
//                  return null;
...
//            debug.entry("queryService.getWidgetsByDefUrl", "url:", url);
//            var arr = [];
...
//            }
//
//            debug.exit("queryService.getWidgetsByDefUrl", arr);
//            return arr;
//         }
//      });
//
//      declare("com.ibm.mm.enabler.iw.services.ServiceManager", null, {
//         constructor : function() {
//            this.serviceEntries = {};
//            this.services = {};
//            var aUrl;
...
//            if (typeof ibmConfig != "undefined" && ibmConfig != null && typeof ibmConfig.loadServices != "undefined" && ibmConfig.loadServices != null
//                  && ibmConfig.loadServices == true) {
...
//                     debug.error("com.ibm.mm.enabler.iw.services.ServiceManager", "error loading data", data);
//                  }
//               };
...
//            }
//            catch (err) {
//               debug.log("services.getService", " failed to create service error detail: " + err.description);
//            }
//            return service;
//         }
//      });
...
//
//      return com.ibm.mm.enabler.iw.services;
   });
