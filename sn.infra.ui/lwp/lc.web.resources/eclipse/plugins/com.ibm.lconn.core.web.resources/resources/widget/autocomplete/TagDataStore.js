...
dojo.provide("lconn.core.widget.autocomplete.TagDataStore");
dojo.require("com.ibm.oneui.controls.AutocompleteDataStore");

dojo.declare("lconn.core.widget.autocomplete.TagDataStore", com.ibm.oneui.controls.AutocompleteDataStore, {
	...
	// url: string
	//		A templated URL that can be substituted to retrieve the suggested tags from the
	//		server.  Replaces the tokens:
	//			1) {query} - with the value of kwArgs.query
	//			2) {start} - with the value of kwArgs.start if data store supports a start index
	//			3) {count} - with the value of kwArgs.count if data store supports an end index
	url: null,
	...
	//				b) getValue(item, "count") returns the number of results found for that tag or "undefined"
	//		This value may be set to null to imply that no transformation must be done of the response.
