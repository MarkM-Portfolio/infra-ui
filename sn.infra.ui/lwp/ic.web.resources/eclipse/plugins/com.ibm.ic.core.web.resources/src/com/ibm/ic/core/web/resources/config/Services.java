...
public class Services extends ResourceProvider
{
  ...
  private static class ConnectionsConfigurationResource extends ConfigurationMapResource
  {
    ...
    private static final String INSECURE_URL = "url"; //$NON-NLS-1$

    private static final String SECURE_URL = "secureUrl"; //$NON-NLS-1$

    private static final String SECURE_ENABLED = "secureEnabled"; //$NON-NLS-1$

    private static final String IS_CONNECT_CLIENT = "isConnectClient"; //$NON-NLS-1$
      ...
      try
      {
        ...
        for (String serviceName : serviceNames)
        {
          ...
          if (provider.isSecureServiceEnabled(serviceName))
            service.put(SECURE_ENABLED, Boolean.TRUE);
          URL url = provider.getServiceURL(serviceName);
          if (url != null)
            service.put(INSECURE_URL, url.toString());
          url = provider.getSecureServiceURL(serviceName);
          if (url != null)
            service.put(SECURE_URL, url.toString());
    ...
    private boolean getServiceAttribute(String serviceName, String attribute)
    {
      ...
      attribute = "[@" + attribute + "]"; //$NON-NLS-1$//$NON-NLS-2$
