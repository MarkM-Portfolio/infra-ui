...
define([
	...
	"dojo/on",
	"dojo/request",
	"ic-test/unit/as/EntryPost/PrettyJSON"
], function (dojo, domClass, declare, lang, stamp, dom, domConstruct, JSON, on, request, PrettyJSON) {
	 ...
	 * @author Tao Shang
	 */
	var EntryPostView = declare("com.ibm.social.test.unit.as.EntryPost.EntryPostView", null, {
		...
		onLoad : function(){
			...
			com.ibm.social.test.unit.as.EntryPost.prettyJSON = new PrettyJSON();
			
			dom.byId("loadingImg").src = this.getServiceUrl(lconn.core.config.services.webresources) +  "/web/com.ibm.lconn.core.styles.oneui3/images/loading.gif";
			
			dom.byId("user").innerHTML = lconn.homepage.userName + ", " + user + ", " + userid;
			
			// populate generator selector
			var select = dom.byId("entryset");
			
			for (var key in window.testASEntries){
				select.options[select.length] = new Option(key, key);
			}
			
			this.sampleEntries = JSON.stringify(window.testASEntries);
			this.markASEntry = {"id":"","actor":{"id":""},"verb":"","object":{"id":""},"connections":{"saved":"true","actionable":"true"}};
			this.entryIds = [];
			
			this._show();
			this.changeEntry();
			dom.byId("mbMessage").value = JSON.stringify(window.mbStatusTemplate);
			on(dom.byId('entryset'), "change", lang.hitch(this, "changeEntry"));
			on(dom.byId('postBtn'), "click", lang.hitch(this, "onPostBtnClick"));
			on(dom.byId('postTypeAS'), "click", lang.hitch(this, "onAPITypeChanged"));
			on(dom.byId('postTypeMB'), "click", lang.hitch(this, "onAPITypeChanged"));
			
			on(this.editor, "Click", lang.hitch(this, "_connectionclick"));
					
		},
		
		onAPITypeChanged : function(){
			
			if(dom.byId('postTypeMB').checked){
				domClass.add(dom.byId('asDiv'),"lotusHidden");
				domClass.remove(dom.byId('mbDiv'),"lotusHidden");
			} else {
				domClass.add(dom.byId('mbDiv'),"lotusHidden");
				domClass.remove(dom.byId('asDiv'),"lotusHidden");
			}
		...
		onPostBtnClick : function() {
			...
			domConstruct.empty("message");
			domClass.remove("loderbody","lotusHidden");
			dom.byId("postBtn").disabled = true;
	
			this.totalTimes = dom.byId("postTimes").value;
			this.postedTimes = 0;
			this.postFailedTimes = 0;
			if(dom.byId("toSave").checked) {
			...
			if(dom.byId("toAction").checked) {
				this.markASEntry.connections.actionable = "true";
	
			} else {	
				delete this.markASEntry.connections.actionable;
			}
			dom.byId("toSave").disabled = true;
			dom.byId("toAction").disabled = true;
			if(this.postType == "AS") {
				this.currentEntry = JSON.parse(this.editor.get('value'));
				this.postUrl = this.asUrl;
				
			} else {
				this.currentEntry = JSON.parse(dom.byId("mbMessage").value);
			 ...
			 */
			if (dom.byId("entryset").value === "Custom Content") {
				this.currentEntry.content = dom.byId("customHtml").value;
			}	
		 ...
		 */
		postData  : function() {
			request(this.postUrl, {method: "POST", headers: {"Content-Type": "application/json"}, postData: JSON.stringify(this.currentEntry)}).then(d: lang.hitch(this,function(data) {
					this.postedTimes++;
		...
		postMarkData  : function() {
			request(this.asUrl + "/" + this.entryIds.shift() + "?X-HTTP-Method-Override=PUT", {method: "POST", headers: {"Content-Type": "application/json"}, postData: JSON.stringify(this.markASEntry)}).then(d: lang.hitch(this,function(data) {
			...
			, r: lang.hitch(this,function(err) {
		...
		postMarkDataForMB  : function(storyID) {
			request(this.asUrl + "/" +storyID + "?X-HTTP-Method-Override=PUT", {method: "POST", headers: {"Content-Type": "application/json"}, postData: JSON.stringify(this.markASEntry)}).then(d: lang.hitch(this,function(data) {
		...
		retrieveStoryId : function(){
			...
			request(this.asUrl + "?rollup=true&filterBy=object&filterOp=equals&filterValue=" + this.entryIds[0], {method: "GET", handleAs: "json"}).then(lang.hitch(this,function(data) {
		...
		showResult : function(){
			...
			domClass.add("loderbody","lotusHidden");
			...
			if(this.postFailedTimes < this.totalTimes){
				domConstruct.create("a", {href: this.hpUrl+ "/web/updates/#imFollowing/all", innerHTML: " Visit ActivityStream", target: "_blank"}, messageNode);
			}	
	
			dom.byId("postBtn").disabled = false;
			dom.byId("toSave").disabled = false;
			dom.byId("toAction").disabled = false;
			dom.byId("postTimes").value = 1;
			dom.byId("toSave").checked = false;
			dom.byId("toAction").checked = false;
			if(this.postType == "AS"){
		...
		changeEntry : function(){
			...
			var data = JSON.parse(this.sampleEntries)[dom.byId("entryset").value];
			
			this.isoDate = stamp.toISOString(new Date());
			...
			if(dom.byId('entryset').value == 'Entry with Link' ) {
			...
			if (dom.byId("entryset").value === "Custom Content") {
				domClass.remove(dom.byId("customHtmlField"),"lotusHidden");
			} else {
				domClass.add(dom.byId("customHtmlField"),"lotusHidden");
			}
		...
		_connectionclick : function(){
			...
			setTimeout(lang.hitch(this, function(){
				...
				this.editor.wrapperWidget.saveButton.set("disabled", false);
