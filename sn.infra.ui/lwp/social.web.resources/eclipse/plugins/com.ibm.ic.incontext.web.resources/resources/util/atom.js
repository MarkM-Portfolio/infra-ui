...
define([
...
], function (dojo, array, lang, domClass, has, JSON, domModule) {
	...
	(function() {
	   ...
	   atom.getError = function(response,ioArgs,opts) {
	      ...
	      isError = isError || (has("ie") && response.number == -2147024891);
	      ...
	      if(opts && opts.requireData) {
	         ...
	         isError = isError || (array.indexOf(["json","json-comment-filtered"],ioArgs.handleAs) != -1 && (!originalResponse || typeof originalResponse != "object"));
	      }
	      ...
	      if (isError) {
	         ...
	         if (!error.code && array.indexOf(["cancel","timeout","unauthenticated"],response.dojoType) != -1)
	            error.message = error.code = error.dojoType = response.dojoType;
	
	         if (!error.code && has("ie") && response.number == -2147024891)
	         ...
	         if(!error.code && (statusCode == 401 || statusCode == 403 || has("ie") && statusCode == 0)) {
	   ...
	   atom.getJsonResponseFromHtml = function(d) {
	      ...
	      try {
	         if (d && d.body) {
	            var meta = array.filter(d.getElementsByTagName("meta"), function(el) {return el.name == "status";})[0];
	   ...
	   atom.detectLoginForMultipartResponse = function(response,ioArgs) {
	      ...
	      if (response && !(response instanceof Error)) {
	         ...
	         if (body && !domClass.contains(body, "X-LConn-API-Response")) {
	            login = true;
	   ...
	   atom.createOperationId = function() {
	   	  var s = Array.prototype.slice.call(arguments, 0).join("_");
	      if (s.length > 256)
	         s = s.substring(0,256);
	      return lang.trim(s);
	   };
	})();
	return com.ibm.social.incontext.util.atom;
});
