...
dojo.provide("lconn.core.action._base");

/**
 * Emitted when an action completes
 *
 * @event lconn/core/action/completed
 * @type {List}
 ...
 * @namespace lconn.core.action
 */
dojo.declare("lconn.core.action._base", null, /** @lends lconn.core.action._base.prototype */ {
   nls: null,
   /**
    ...
    * Usually, an action's {@link lconn.core.action._base#execute|execute} method would be connected with a button's
    ...
    * All core actions are expected to emit the evebt {@link event:lconn/core/action/completed} when successful.
    * @param {Object} item Target of the action
    * @param {Object} opt Options
    * @param {Event} e Event
    * @fires lconn/core/action/completed
    */
   execute: function(item, opt, e) {
      if (e)
         dojo.stopEvent(e);
      // Implementors must issue this dojo topic 
      dojo.publish("lconn/core/action/completed");
   }
});
