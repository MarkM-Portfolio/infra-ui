...
define([
	...
	"dojo/has",
	"dojo/io/iframe"
], function (dojo, array, lang, has, iframe) {

	/**
	 * com.ibm.social.incontext.util.auth provides Dojo applications with a central authentication and detection framework.  Applications may use this class to allow
	 ...
	 *     
	 *         com.ibm.social.incontext.util.auth.interceptDojoXhr();
	 *         com.ibm.social.incontext.util.auth.isUrlSecure = function(url) {
	 ...
	 *     Check if the class "com.ibm.social.incontext.util.auth" was available in the current javascript runtime.  Call prepareSecure on the arguments 
	 ...
	 *         };
	 *         if (dojo.getObject("com.ibm.social.incontext.util.auth"))
	 *            com.ibm.social.incontext.util.auth.prepareSecure(args);
	 *         dojo.xhrGet(args);
	 *
	 ...
	 * 
	 */
	com.ibm.social.incontext.util.auth = {
	   ...
	   interceptDojoXhr: function(isUrlSecure) {
	      ...
	      xhr = lang.hitch(dojo, this.xhrIntercept, this, xhr);
	      if (lang.getObject("dojo/io/iframe"))
	         iframe.send = lang.hitch(iframe, this.ioIntercept, this, iframe.send);
	   },
	   ...
	   prepareSecure: function( /* object */ args, /* function(response,ioArgs) optional */ isAuthRequired, /* function() optional */ onauthenticated, /* boolean optional */ bypassXhrAuthHeader ) {
	      ...
	      if (args["com.ibm.social.incontext.util.auth.prepareSecureCalled"])
	         return args;
	      else
	         args["com.ibm.social.incontext.util.auth.prepareSecureCalled"] = true;
	      ...
	      args.handle = lang.partial(this.testAuthenticationHandler, this, isAuthRequired, onauthenticated);
	   ...
	   setAuthenticationTest: function( /* function(auth, response, ioArgs) */handler) {
	      this.isAuthenticationRequired = lang.partial(handler, this);
	   },
	   ...
	   isPossibleLoginRedirect: function(response, ioArgs) {
	      ...
	      if (ioArgs.xhr && (!response || array.indexOf(this.ignoredDojoErrors, response.dojoType) == -1)) {
	         
	         // attempting to retrieve the status code may throw an exception in IE when redirected to a different security domain
	         var statusCode;
	         try { statusCode = ioArgs.xhr.status; } catch (e) {}
	         ...
	         var expectedHandleAs = ioArgs.args.expectedContentType || ioArgs.args.handleAs;
	         var isNonHtmlTypeExpected = array.indexOf(this.nonHtmlTypes, expectedHandleAs) != -1;         
	         ...
	         if (this.checkByStatusCode && (isNonHtmlTypeExpected || this.checkAllStatusCodes)) {
	            ...
	            if (has("ie") && (statusCode == 0 || statusCode == "unknown")) {
	               ioArgs._authOutcome = "Status code was 0 or \"unknown\"";
	               return true;
	            }
	         }
	         if (this.checkByContentType && isNonHtmlTypeExpected && this.contentTypeRegex.exec(ioArgs.xhr.getResponseHeader("Content-Type")) && statusCode >= 200 && statusCode < 300 && statusCode != 204) {
	   ...
	   isUrlSecure: function(url) {
	    ...
	    hasCurrentViewer: function(){
	        return window.currentViewer && window.currentViewer.id && window.currentViewer.id != -1;
	    },
	
	    setCurrentViewer: function(user) {
	        if(user) {
	            window.currentViewer = user;
	        }
	    },
	
	    parseUserFromOpensocialResponse: function(resp) {
	
	        var user = this.parseUser(resp && resp.entry);
	    ...
	    parseUser: function(user){
	        if ( user && user.id && user.displayName ) {
	            ...
	            var orgId = lang.getObject("appData.connections.organizationId", false, user);
	            if ( orgId && orgId.indexOf('urn:lsid:lconn.ibm.com:') != -1 ) {
	                var endPrefix = orgId.lastIndexOf(':');
	                if ( endPrefix != -1 ) {
	                    orgId = orgId.substr(endPrefix+1);
	                }
	            }
	
	            var isExternal = lang.getObject("appData.connections.isExternal", false, user);
	...
	return com.ibm.social.incontext.util.auth;
});
