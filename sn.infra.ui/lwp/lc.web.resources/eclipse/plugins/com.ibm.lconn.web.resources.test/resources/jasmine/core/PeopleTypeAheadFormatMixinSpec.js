...
dojo.provide("lconn.test.jasmine.core.PeopleTypeAheadFormatMixinSpec");

dojo.require("lconn.core.PeopleTypeAheadFormatMixin");
dojo.require("lconn.core.globalization.bidiUtil");

// Declare the object(s) under test as arguments, AMD callback style
(function(PeopleTypeAheadFormatMixin, bidiUtil) {
   ...
   describe("the interface of lconn.core.PeopleTypeAheadFormatMixin", function() {
   ...
   describe("the method lconn.core.PeopleTypeAheadFormatMixin.formatItem()", function() {
      ...
      it("returns a string value when item doesnt inlcude quotes or commas, and html is true or false", function() {
         ...
         expect(typeof result == "string").toBeTruthy();
         expect(result).toBe(dojo._isBodyLtr() ? "Notreal Guy" : bidiUtil.enforceTextDirection("Notreal Guy", 'rtl'));
         html = false;
      ...
      it("returns a string value even if item includes quotes, and html is true or false", function() {
         ...
         expect(result).not.toBeNull();
         expect(typeof result == "string").toBeTruthy();
         expect(result).toBe(dojo._isBodyLtr() ? "Not&quot;real&quot; Guy" : bidiUtil.enforceTextDirection("Not&quot;real&quot; Guy", 'rtl'));
   ...
   describe("the method lconn.core.PeopleTypeAheadFormatMixin._getLabelFromItem()", function() {
      it("returns the item's name, email and business owner when existing", function() {
         ...
         })).toBe(dojo._isBodyLtr() ? 'doge : doge inc. <doge@example.com>\u200E' : bidiUtil
      ...
      it("returns the item's name and email when existing", function() {
         expect(typeahead._getLabelFromItem({
            name : 'doge',
            email : 'doge@example.com'
         })).toBe(dojo._isBodyLtr() ? 'doge <doge@example.com>\u200E' : bidiUtil.enforceTextDirection('doge \u200E<doge@example.com>\u200E', 'rtl'));
         expect(typeahead._getLabelFromItem({
            name : 'doge',
            member : 'doge@example.com'
         })).toBe(dojo._isBodyLtr() ? 'doge <doge@example.com>\u200E' : bidiUtil.enforceTextDirection('doge \u200E<doge@example.com>\u200E', 'rtl'));
      ...
      it("returns the item's name when existing", function() {
         expect(typeahead._getLabelFromItem({
            name : 'doge'
         })).toBe(dojo._isBodyLtr() ? 'doge' : bidiUtil.enforceTextDirection('doge', 'rtl'));
      });
      it("returns an empty string when the item doesn't have a name property", function() {
         expect(typeahead._getLabelFromItem({
            type : 'doge'
         })).toBe(dojo._isBodyLtr() ? '' : bidiUtil.enforceTextDirection('', 'rtl'));
      });
      ...
      it("returns an empty string when the item is null or undefined", function() {
         expect(typeahead._getLabelFromItem(null)).toBe(dojo._isBodyLtr() ? '' : bidiUtil.enforceTextDirection('', 'rtl'));
         expect(typeahead._getLabelFromItem()).toBe(dojo._isBodyLtr() ? '' : bidiUtil.enforceTextDirection('', 'rtl'));
      });
   });

   describe("the method lconn.core.PeopleTypeAheadFormatMixin._htmlify()", function() {
...
}(lconn.core.PeopleTypeAheadFormatMixin, lconn.core.globalization.bidiUtil));
