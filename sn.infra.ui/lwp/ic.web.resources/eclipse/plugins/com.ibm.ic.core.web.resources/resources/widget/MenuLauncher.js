...
], function(dom, Deferred, declare, windowModule, on, domStyle, lang, domGeometry, has, domClass, domConstruct, domAttr, query, topic, _Widget, registry, popup, focusUtil, ContentPane, Promise) {

   /**
    * Loosely based on dijit.form.DropDownButton. Enable a link that will launch
    * a menu. This class also provides the capability to instantiate the menu on
    * demand. You must pass an existing link as the source ref.
    * 
    * @class ic-core.widget.MenuLauncher
    * @extends dijit._Widget
    */
   var MenuLauncher = declare("lconn.core.widget.MenuLauncher", _Widget, /** @lends ic-core.widget.MenuLauncher.prototype */
   {
      hideDelay : 1000,
      openDelay : 0,
      preloadDelay : 0, // if greater than zero, load in the background during
      // the open delay
      ...
      // optMenu: {}, //arguments that should be passed to the menu when
      // initialized

      buildRendering : function() {
         this.domNode = this.popupStateNode = this.srcNodeRef;
         /*
          * This check allows customers to disable mega menus on hover by
          * removing the onmouseover attribute from customized header.jsp
          */
         ...
         if (this.domNode.tagName === "LI") {
            var anchorMenu = query('>a', this.domNode)[0];
            // Disable if the customer removed the onmouseover attribute from
            // the anchor
         ...
         this.menuId = this.menuId || domAttr.get(this.domNode, "aria-owns") || domAttr.get(this.domNode, "aria-describedby")
               || (this.id ? (this.id + "_popup") : registry.getUniqueId("ml"));
         this.externalContent = domAttr.get(this.domNode, "external-content");

         // We're done with the srcNode at this point
         // Removing it fixes an IE focus-loss problem replacing a DOM node with
         // itself
         delete this.srcNodeRef;
      },

      decorateAria : function() {
         domAttr.set(this.domNode, "aria-owns", this.menuId);
      },

      whenInit : function() {
         // decorate aria only after menu is initialized to avoid invalid id
         // reference
         this.decorateAria();
         var dfd = this._initDfd;
         if (!dfd) {
            var ret = this._initMenu();
            if (!(ret instanceof Deferred || ret instanceof Promise)) {
               if (!this.menu)
                  throw "_initMenu must initialize this.menu or return a deferred";
               ret = new Deferred();
               ret.resolve(this.menu);
            }
            dfd = this._initDfd = ret;
            dfd.then(lang.hitch(this, this.initFinal), lang.hitch(this, function() {
               this._initDfd = null;
            }));
         }
         return dfd;
       ...
       */
      initFinal : function() {
         // attach a hidden widget reference node to ensure that the widget gets
         // destroyed
         var menu = this.menu;
         menu._launcher = this;
         popup.moveOffScreen(menu.domNode);
         var span = domConstruct.create("span");
         span.style.display = "none";
         domAttr.set(span, "widgetId", menu.id);
         windowModule.body().appendChild(span);
         ...
         if (this.externalContent) {
            var content = registry.byId(this.externalContent);
            ...
            on(this, "Open", lang.hitch(this, function() {
               domStyle.set(this.menu.domNode, {
                  padding : 0
               });

               var pos = domGeometry.position(dom.byId(content.bindto), true);
               domStyle.set(content.domNode, {
                  top : pos.y + 'px',
                  left : pos.x + 'px',
                  height : pos.h + 'px',
                  width : pos.w + 'px',
      ...
      _preloadMenu : function() {
         this.whenInit().then(lang.hitch(this, function() {
            var menu = this.menu;
            var dlcp = ContentPane;
            if (menu._preload)
               menu._preload();
      ...
      _openMenu : function(eventType) {
         ...
         this.whenInit().then(lang.hitch(this, function() {
            if ((!this._opened && this._closing) || !this._shouldBeOpen)
               // _delayClose was called before init completed, and item has not
               // yet been opened
               return;
            ...
            if (!this._opened) {
               // If there's an href, then load that first, so we don't get a
               // flicker
               if (menu.href && !(menu.isLoaded || menu.isFailed)) {
                  var self = this;
                  var onLoadHandler = on(menu, "Load", function() {
                     onLoadHandler.remove();
                     self._openFinal(eventType);
                  });
                  var onDownloadErrorHandler = on(menu, "DownloadError", function() {
                     onDownloadErrorHandler.remove();
                     self._openFinal(eventType);
                  });
                  this._preloadMenu();
                  return;
      ...
      _openFinal : function(eventType) {
         ...
         if (lconn.core.widget.MenuLauncher._menu)
            popup.close(lconn.core.widget.MenuLauncher._menu);

         if (this.onBeforeOpen(eventType) === true)
            return;

         domClass.add(this.activeNode, this.classActive);
         ...
         popup.open({
            ...
            onClose : function() {
               ...
               if (self.domNode)
                  domClass.remove(self.activeNode, self.classActive);
               self._opened = false;
               self.onClose();
            }
         });
      ...
      _closeMenu : function(/* Boolean */focus) {
         ...
         if (this._opened) {
            if (has("ie"))
               setTimeout(lang.hitch(this, "_closeMenuFinal", focus), 0);
            else
      ...
      _delayOpen : function() {
         if (!this._opening) {
            this._delayEnd();
            this._opening = setTimeout(lang.hitch(this, "_openMenu", "mouseover"), this.openDelay);
            if (this.preloadDelay > 0 && (!this.menu || !this.menu.isLoaded))
               this._preload = setTimeout(lang.hitch(this, "_preloadMenu"), this.preloadDelay);
         }
      },
      _delayClose : function() {
         if (!this._closing) {
            this._delayEnd();
            this._closing = setTimeout(lang.hitch(this, "_closeMenu", false), this.hideDelay);
         }
      },
      _delayEnd : function() {
         try {
      ...
      /**
       * Type will be null if the event that launched this menu was a hover
       * 
       * @param {String}
       *           [eventType] Type of the event
       */
      ...
      focusMenu : function() {
         ...
         if (menu.focus)
            ...
            focusUtil.focus(menu._firstFocusItem);
         }
      }
   });
