...
], /** @lends lconn.core.TypeAhead.prototype */
{
   ...
   postCreate : function() {
      ...
      // Undo setting fontFamily, since we don't have an drop-down arrow to
      // worry about
      ...
      if (!this.downArrowNode) {
         this.downArrowNode = this.domNode.parentNode.appendChild(dojo.create("div", {
            style : {
               display : "none"
            }
         }));
      ...
      this.dummyPopupNode = this.domNode.parentNode.appendChild(dojo.create("div", {
   ...
   _onFocus : function( /* Event */evt) {
      ...
      if (dojo.isIE) {
         ...
         // Texbox got focus when it was clicked on, but IE was not sending any
         // keydown events
   ...
   _trimKeys : function() {
      // if maxChars is set, make sure no words in the typeahead exceed it.
      if (this.maxChars > 0) {
         ...
         for (var i = 0; i < inputKeys.length; i++) {
            // need to check the actual byte length to make sure we don't exceed
            // the storage
            if (lconn.core.util.text.getByteLength(inputKeys[i]) > this.maxChars) {
               var oldKey = inputKeys[i];
               inputKeys[i] = lconn.core.util.text.trimToByteLength(inputKeys[i], this.maxChars);
               changed = true;
               // TODO - better logging?
               if (window.console)
                  console.log("lconn.core.TypeAhead key too long. Trimming '" + oldKey + "' to '" + inputKeys[i] + "'.");
            }
         }
   ...
   _hasAttr : function(domNode, attr) {
      return dojo.isIE ? domNode.getAttribute(attr) !== null : domNode.hasAttribute(attr);
   },
      ...
      if (this.hintText) {
         if (!this._focused && (focusNode.value == "" || !focusNode.hasInput)) {
            ...
            if (focusNode.value != "") {
               ...
               // Make sure the input keeps focus when clicking in the field in
               // FF
               if (focus && dojo.isFF) {
                  setTimeout(function() {
            ...
            // Make sure the cursor is visible when tabbing into the main search
            // box
            if (focus && dojo.isIE)
               try {
   ...
   // If maxChars is defined, then we need to restrict EACH individual tag to
   // this length.
   // Which means we need to determine the size of the current tag we're typing,
   // not just
   // the overall length of the field. So we'll need to inspect the keypress
   // event and
   // check the input and make sure it's allows so that no individual tag will
   // exceed this
   // maximum.
   _checkKeyLength : function( /* Event */evt) {
      ...
      try {
         ...
         if (this.maxChars > 0) {
            ...
            // loop through the list of special keys and allow them regardless
            var isKeyAlwaysAllowed_ = function(event) {
               ...
               // dojo.keys has a list of special key definitions. We'll use
               // them
            ...
            // get the current word where the caret position is at in the input
            // field.
            var getCurrentWord_ = function() {
               var fn = this_.focusNode;
               var tk = this_.token;

               var cpos = this_._getCaretPos(this_.focusNode); // current cursor
                                                               // position
               var cword = this_.getTextBoxValue(); // current overall text
                                                      // box value
               if (this_.multipleValues && tk.length > 0) { // if multivalue,
                                                            // find the
                                                            // individual tag.
                  cword = tk + cword + tk;
            ...
            // check for the special characters and length of the current word.
            if (isKeyAlwaysAllowed_(evt) || lconn.core.util.text.getByteLength(getCurrentWord_()) < this.maxChars) {
               return true;
            }

            // if we got here, then the key pressed isn't a special key or token
            // AND the length is too long
   ...
   _onKey : function( /* Event */evt) {
      ...
      if (!this._checkKeyLength(evt))
         return; // first check to make sure we're under maxchars for this word
      this.inherited(arguments); // inherits from dijit.form._AutoCompleterMixin
      if (evt.keyCode === dojo.keys.ENTER) {
         ...
         if (this.submitFormOnNonSelectingEnter) { // flag used by
            ...
            if (this._trimKeys) {
               this._trimKeys(); // trim any keys before submitting if maxChars
                                 // is set
            }
         }
   ...
   _numTokensPreceding : function(str, pos) {
      // figure out how many tokens (if any) are in the given string, before pos
      var checkStr = str.substring(0, pos);
      // this should use _splitByToken, but the rest of the code is dumb and
      // uses a plain split, so just do that
   ...
   _autoCompleteText : function( /* String */text) {
      ...
      // Find which key the caret is currently in by counting the number of
      // tokens preceding cpos
      var count = 0;
      if (this.multipleValues)
   ...
   _openResultList : function( /* Object */results, /* Object */dataObject) {
      ...
      if (!this.domNode || // lconn.core: don't reveal results list if node has
                           // been destroyed
   ...
   _showResultList : function() {
      ...
      // Override to set activedescendant when the popup shows if it doesn't
      // already have one
      var fn = this.focusNode;
      ...
      if(fn.offsetWidth > pw.domNode.offsetWidth) {
         // CNXSERV-16672- Reduce dropdown width to align input field
         if (window.ui && typeof window.ui._check_ui_enabled === 'function' && window.ui._check_ui_enabled()) {
            dojo.style(pw.domNode, {"width" : ((fn.offsetWidth -5) +"px")} );
         } else {
      ...
      // RTC: 83154 - In IE there is an overflow issue that causes unnecessary
      // horizontal scrollbars on initial render
      if (dojo.isIE) {
   ...
   _fixPosition : function() {
      ...
      if (node) {
         ...
         var listPos = dojo.position(listNode, true);
   ...
   _announceOption : function( /* Node */node) {
      ...
      // lconn.core: exclude case when user clicks on the search from directory
      // item
      node == this._popupWidget.searchButton || node == this._popupWidget.resultsNode) {
         ...
         // lconn.core: we don't call labelFunc here - surprise, it gets called
         // in dojo 1.4 anyhow!
         newValue = this.formatItem(item);
         // currently this method causes the selected value to be written to the
         // textbox.
      ...
      // autocompleted text)
      if (this.multipleValues) {
         // IE workaround: fall back to original position if we can't get the
         // current position
   ...
   _startSearchFromInput : function() {
      ...
      if (this.multipleValues) {
         ...
         // The index associated with the name in keyArr which is currently
         // being typed
   ...
   shouldStartSearch : function( /* String */query) {
      ...
      /**
       * Ideographic written languages (Japanese Kanji, Chinese) have single
       * characters that represent more information than a Latin (English)
       * character. UTF-8 representation is a fair estimate of complexity, so
       * minChars is treated as "minBytes". encodeURIComponent is the simplest
       * way to convert to UTF-8 in Javascript.
       */
      if (typeof shouldStart == "undefined") {
         ...
         var length = 0;
         var encoded = encodeURIComponent(query); // converts strings to UTF-8
   ...
   _startSearch : function( /* String */key) {
      ...
      if (!this._popupWidget) {
         ...
         dijit.setWaiState(this.textbox, "owns", popupId); // associate popup
                                                            // with textbox
      ...
      // otherwise, if the user types and the last query returns before the
      // timeout,
