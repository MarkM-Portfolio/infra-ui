...
define([
	...
	"ic-as/lconn/widgets/communities/CommunitiesActivityStreamEdit",
	"ic-as/util/xhr/XhrHandler",
	"ic-core/theme",
	"ic-sharebox/ContextualSharebox"
], function (lang, dom, registry, ActivityStream, CommunitiesActivityStreamEdit, XhrHandler, theme, ContextualSharebox) {
	...
	lang.extend(com.ibm.social.as.lconn.widgets.CommunitiesActivityStreamClass, {	
		...
		onLoad: function(){
			...
			if(mode === "search"){
				...
				var resultContainerDomNode = dom.byId(this.getWidgetNodeId("activityStreamSearchNode"));
			...
			setTimeout(lang.hitch(this, function() {
				...
				if(dom.byId("activityStream")){
					// Attach the ActivityStream dijit for Communities onto the page
					this.activityStreamWidget = new ActivityStream({
		...
		onUnload: function() {
			...
			var editAccessDijit = registry.byId(this.getWidgetNodeId("activityStreamEditNode"));
		...
		onEdit: function() {
			...
			if(!this.editAccessDijit) {
				this.editAccessDijit = 	new CommunitiesActivityStreamEdit({iContext: this.iContext}, 
						dom.byId(this.getWidgetNodeId("activityStreamEditNode")));
			}
			else {
		...
		createShareBoxNode: function() {
			this.shareBoxWidget = new ContextualSharebox({
			...
			}, dom.byId(this.getWidgetNodeId("shareboxContainer")));	
		},
		...
		removeShareBoxNode: function() {
			...
			var shareboxDijit = registry.byId(this.getWidgetNodeId("shareboxContainer"));
			if (shareboxDijit) {
	...
	return com.ibm.social.as.lconn.widgets.CommunitiesActivityStreamClassImpl;
});
