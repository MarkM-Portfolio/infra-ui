...
/**
 * Common tag cloud widget
 * FIXME: this widget must be simplified and made more modular
 * @class lconn.core.CommonTags.TagWidget
...
(function() {
   ...
   function _v(el) { if (el) return dojo.hasClass(el, "lotusHidden"); return false; }
...
dojo.declare("lconn.core.CommonTags.TagWidget", [dijit._Widget, dijit._Templated], /** @lends lconn.core.CommonTags.TagWidget.prototype */ {
   ...
   templatePath: dojo.moduleUrl("lconn.core", "CommonTags/templates/tagWidget.html"),
   ...
   reload: function(isFirstLoad, urlParameters) {
      ...
      if (this._isRelated == true)
         this.ajaxCall.getTags(dojo.hitch(this, this.ajaxCallBack), isFirstLoad, this.selectedTags);
      else
         this.ajaxCall.getTags(dojo.hitch(this, this.ajaxCallBack), isFirstLoad);
   },

   //used to update parameters based on activity outside the scope of the tag widget
   ...
   prepareData:function() {
      ...
      if (this.selectedTags != null && dojo.string.trim(this.selectedTags)!='') {
         ...
         for (var i=0; i<arr.length; i++) {
            var tagName = dojo.string.trim(arr[i]);
            if (tagName == '') continue;
   ...
   _genRelatedTagsHtml: function() {
      ...
      if (this.tags != null ) {
         ...
         for (var j=0; j<length; j++) {
            ...
            span.innerHTML = '+';
            dojo.place(span, a);
            a = lconn.core.util.html.breakString(lconn.core.globalization.bidiUtil.enforceTextDirection(tagDisplayName), document, a, 10);
            dojo.place(a, li);
   ...
   _genNormalTagsHtml: function() {
      ...
      if (this.tags != null ) {
         ...
         dojo.attr(tagTableUL, "aria-label", this.nls.rs_tagsLabel);
         ...
         for (var i=0; i<cloudTags.length; i++) {
            ...
            dojo.place(li, tagTableUL);
         }
         dojo.place(tagTableUL, this._tagCloudTable);

         this.cloudHelper = new lconn.core.aria.Listbox(tagTableUL);
         ...
         for (var j=0; j<length; j++) {
            ...
            var a = document.createElement('a');
               ...
               var tagDisplayName = (tag.displayName) ? tag.displayName : tag.name;
               this.renderTag(a, tag);
               lconn.core.util.html.breakString(lconn.core.globalization.bidiUtil.enforceTextDirection(tagDisplayName), document, a, 10);
            li.appendChild(a);
   ...
   _genSelectedTagsHtml : function() {
      ...
      for (var i=0; i<this._selectedTagsArr.length; i++) {
         ...
         //some widgets save the selected tag as ID, we need to convert to display name, if exists
         ...
         lconn.core.util.html.breakString(lconn.core.globalization.bidiUtil.enforceTextDirection(tagDisplayName), document, span, 10);
         li.appendChild(span);

         var a = document.createElement('a');
            a.href = "javascript:;";
            a.className = "lotusDelete lotusRight";
            a.title = dojo.string.substitute(this.nls.rs_removeTagTitle, [tagDisplayName]);
            this.connect(a, "click", dojo.hitch(this, "_removeSelectedTagFromEvent", selectedTag));
            dojo.attr(a, "role", "option");

            // Add a single character to make the line height match the tag text
   ...
   _addSelectedTagFromEvent : function(tag, event) {
      try { if(event) dojo.stopEvent(event); } catch(e) {}
   ...
   _show: function() {
      ...
      if ((this.tags == null || this.tags.length==0) && (this.selectedTags == null || dojo.string.trim(this.selectedTags)=='')) {
         ...
         // Hide the form by default unless the user clicked the "Find a tag" link
   ...
   _createTypeAhead: function() {
      ...
      var tagsTypeAhead = new lconn.core.CommonTags.CommonTagsTypeAhead(args, this._typeAheadDom);
   },
   ...
   _popupHelp: function(event) {
      ...
      var tagEvt = {tagWidget:this, event:event};
      dojo.publish("/tagWidget/help/show", [tagEvt]);
   },
   ...
   _loading:function() {
      // this._tagItems.innerHTML = '<img alt="" src="'+this._blankGif+'" class="lotusLoading" />';
   },
   ...
   _showAllTags: function(event) {
      ...
      if (this._dialog == null){
         var tagDialogArgs={ajaxCall: this.ajaxCall, feedConverter:this.feedConverter, tagWidget:this };
         this._dialog = new lconn.core.CommonTags.TagDialog(tagDialogArgs);
      }
      this._dialog.show();
   },
   ...
   _searchTag:function(event, item) {
      ...
      if(this.tagsTypeaheadEnabled) {
         ...
         // This lets us capture the user typing in a tag and hitting enter without selecting something from the typeahead
         if (!tag) {
   ...
   _removeSelectedTagFromEvent: function(tag, event) {
      ...
      // Request focus in the selected tags section at the same index as the item we removed
   ...
   _removeSelectedTag: function(tag) {
    ...
    _setFocus: function() {
       // redirectWhenClickTag:true - Clicking tag will redirect to a new page.
       // _focusHint - If null, focus hasn't been requested
       if (this.redirectWhenClickTag || !this._focusHint) return;
       ...
       if (hint.related && this._relatedTags) {
          focusNode = dojo.query('a', this._relatedTags)[0];
       }
       else if (hint.selected >= 0 && this._selectedTagsArr.length) {
          var links = dojo.query('a', this._selectedTags);
    ...
    /////////
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Accepts an array of tags and overwrites selectedTags and selectedTagsArr
...
/**
 * @class lconn.core.CommonTags._TagTransform
 * @private
 */
dojo.declare("lconn.core.CommonTags._TagTransform", null, /** @lends lconn.core.CommonTags._TagTransform.prototype */ {

   /*
    * Keep the order of the original array. Get the top frequency tags from the array.
    */
   getPopularTags:function(tags, limit) {
      ...
      frequencies = frequencies.sort(function(a,b){return a-b;}).slice(frequencies.length - limit);
   ...
   addIntensityBin: function(tags) {
      ...
      // No-op if we already have a calculated intensityBin
      ...
      for (var i=0; i < length; i++) {
         var tag = tags[i];
         // Special case... if all tags have the same weight, make them average-size
         if (range == 0)
   ...
   existsInArray:function(tag, tagArray) {
      return dojo.some(tagArray, function(item) { return tag && item && item.name === tag.name; });
   },

   existsInRelatedTags:function(tag,tagArray){
      return dojo.indexOf(tagArray, tag) !== -1;
   },
   ...
   sortByName:function(tags) {
       ...
       if (tags && tags.length) {
          ...
          for (var i=0; i<namesArray.length; i++) {
             ...
             returnTags[i] = {id:id, name:name, displayName:displayName, frequency:frequency};
             if(max==-1) max = frequency;
             if(min==-1) min = frequency;
   
             max = Math.max(max, frequency);
             min = Math.min(min, frequency);
          }
       }
       return {tags:returnTags, maxCount:max, minCount:min};
    },
