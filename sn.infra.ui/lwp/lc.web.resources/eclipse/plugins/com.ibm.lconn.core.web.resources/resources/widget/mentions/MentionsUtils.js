 ...
 *
 * @mixin lconn.core.widget.mentions.MentionsUtils
 * @author Piyush K Agarwal <pagarwal@us.ibm.com>
 */
dojo.declare("lconn.core.widget.mentions.MentionsUtils", null, /** @lends lconn.core.widget.mentions.MentionsUtils.prototype */{
   ...
   isMentionsDomNode : function(domNode) {
      var type = dojo.attr(domNode, 'type');
   ...
   isPersonMentionsDomNode : function(domNode) {
      return (dojo.attr(domNode, 'type') == "PersonMentionsNode");
   },
   ...
   connectEvents : function() {
      ...
      // defect 127178: Removed IE browser from composition event, as it does not work well with mentions. Using IE8 approach instead
   ...
   compositionEnd : function(e) {
      ...
      if (this._isTracking) {
         ...
         //if(!dojo.isChrome) //123999 chrome duplicate ime char
   ...
   /**
    * Function to enable the text on the typeahead header to be set after the MentionsHelper has been created.
    *
    * @param String -
    *           Text for header.
    */
   setTypeaheadHeader : function(s) {
      /* We need to find a PersonMentionsType typeahead from the registered types. We then need to set it's
       * header item. */
      if (this._registeredTypes) {
         var personTypeaheads = dojo.filter(this._registeredTypes,
               function(item) {
         ...
         if (personTypeaheads.length > 0) {
            if (personTypeaheads[0] && personTypeaheads[0]._typeahead) {
               typeahead = personTypeaheads[0]._typeahead;

               // If the typeahead has a function to set the header then use that e.g SC typeahead
   ...
   /**
    * Works around side effect of fix for issue #24 Control selection sometimes fails to clear when removeAllRanges() is called in IE http://code.google.com/p/rangy/issues/detail?id=24 See RTC #83239
    */
      ...
      sel.removeAllRangesEx = dojo.partial(function(me) {
         ...
         try {
            ...
            if (this.docSelection.type != "None") {
               // Instead of creating a text range on the body, create a dummy input element
               var tmp = dojo.create("input", {
                  type : "text",
                  style : "display: none"
         ...
         // Inline call to updateEmptySelection(this); as the function is unavailable outside its closure
   ...
   showBizCardHover : function(e, node) {
      if (e && node && !this.disableBizCard) {
         ...
         var temp = dojo.byId("bc_document_node");
         if (!temp) temp = dojo.byId("bc_document_node_comm");
         if (temp) {
            var oldid = temp.id;
   ...
   getNodeText : function(node, formatMention) {
      var plainText = "";
      if (node) {
         dojo.forEach(node.childNodes, function(entry, i) {
            if (entry.nodeName == "BR") {
               plainText += "\n";
            } else {
               // CCM libraries support returning the mentions @{displayName|uid}
   ...
   getPlainMention : function(node) {
      ...
      var uidNode = dojo.query('.x-lconn-userid', node)[0];
      if (uidNode) {
         userId = dojo.isFF ? uidNode.innerHTML : uidNode.innerText;
      }
      ...
      for ( var i = 0; i < nodes.length; i++) {
         ...
         if (node.id) {
            if (node.id.indexOf("mentionsNode_") != -1) {
               ...
               // If mentions is complete post as mention JSON format, otherwise post as plain text
               if (attr) {
                  if (attr.isComplete) {
   ...
   moveToPreviousNode : function(node, range, sel) {
      ...
      var nodeIdx = dojo.indexOf(this.textAreaNode.childNodes, node);
      if (nodeIdx > 0) {
         ...
         if (nodeBeforeCursor.nodeType == 3) {
            ...
            } else if (dojo.isIE > 8 && nodeBeforeCursor.length == 1 && nodeBeforeCursor.data.charAt(0) == '\u200B') {
               this.moveToPreviousNode(nodeBeforeCursor, range, sel);
            } else {
   ...
   removeSelection : function(e) {
      ...
      var sel = curSel.selection;
      var range = curSel.range;
      curSel.selection.collapseToStart(); // fix for loss of focus in IE when the mention is selected and removed (via DEL or BACKSPACE)
      if (dojo.isIE) {
         sel.refresh();
      ...
      if (!mentionNode) {
         ...
         dojo.forEach(extractedContents.childNodes, function(node) {
            if (node) {
      ...
      if (range.startContainer == this.textAreaNode) {
         // Old WebKit has a bug where if you have two adjacent TextNodes in the dom
         // then you cannot place the cursor at the 0th position of 2nd node...
   ...
   completeMention : function(data) {
      ...
      if (domNode) {
         ...
         if (!domNode.nextSibling || (dojo.isIE && domNode.nextSibling.length==0)) {
   ...
   clearImeNodeIE : function(mentionsNode) {
      if (dojo.isIE && mentionsNode && mentionsNode.linkNode) {
   ...
   removeMentionNode : function(mentionNode) {
      ...
      // check to make sure the node was not extracted from the document incase of a selection delete
      var domNode = mentionNode.domNode;
      if (domNode && domNode.parentNode && domNode.parentNode.nodeType != 11) {
         // We are on the first node and the previous one is empty so we replace it for one with \u200B
   ...
   // dontMoveCursor is optional, if true cursor is not moved after the new textnode
   ...
   handleResumeMention : function(sel, range) {
      ...
      if (node && this._currentNode && !this._currentNode.isComplete) {
         ...
         if (node && curMentionTextNode && (document.activeElement == this.textAreaNode && node != window.document) && (node.data != curMentionTextNode.data || (node.data == curMentionTextNode.data && node != curMentionTextNode && range.startOffset != 0))) {
            if(dojo.isIE > 8 && !range.collapsed && !(range.endContainer && range.endContainer == this._currentNode.domNode)){ 
   ...
   setAriaLabel : function(label, content) {
      if (!this.ariaLabel) {
         this.ariaLabel = dojo.create("div", {
         ...
         if (dojo.isIE) {
            dojo.style(this.ariaLabel, {
               position : "absolute",
               top : "-9999px"
            });
         }
      }
      this.ariaLabel[dojo.isIE ? "innerText" : "innerHTML"] = (content ? dojo.string.substitute(label, content) : label) || "";
   },
