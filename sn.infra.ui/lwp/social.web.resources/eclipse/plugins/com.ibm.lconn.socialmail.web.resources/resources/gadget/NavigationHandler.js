...
dojo.require("com.ibm.lconn.socialmail.gadget._NavigationHandler");
...
dojo.declare("com.ibm.lconn.socialmail.gadget.NavigationHandler", null, {
   
   constructor: function() {
      if (!NavigationHandler) {  // Initialized only once
      
         function Custom() {};
         Custom.prototype = new com.ibm.lconn.socialmail.gadget._NavigationHandler();
         
         var customData = {
               // fields required by the superclass
               idMailListItem: 'lotusBannerMail', // id mail list item of the header
               idCalendarListItem: 'lotusBannerCalendar', // id calendar list item of the header
               idMailLoadingSpinner: 'os-site-mail-hover-loading', // id mail dropdown of the container (with the loading spinner)
               idCalendarLoadingSpinner: 'os-site-calendar-hover-loading', // id mail dropdown of the container (with the loading spinner)   
               classBadge: 'os-site-mail-notify', // class of the badge, required as the id is generated dynamically (the badge is already on the page)
               classMailDropdown: 'os-site-mail-hover', // class of the mail dropdown, required as it's set during the creation of the gadget site (dropdown div)
               classCalendarDropdown: 'os-site-calendar-hover', // class of the calendar dropdown, required as it's set during the creation of the gadget site (dropdown div)
               classDisplayNone: 'dijitDisplayNone', // class to set display none
               
               // fields NOT required by the superclass (used internally)
               topics: [],
               classHidden: 'dijitHidden', // class to set hidden the title node of the lightbox
               classLightboxContainer: 'cm-preview-site-container', // class of the container div, which include the 2 divs: standard message and embedded experience
               classLightboxMessage: 'os-site os-site-mail-hover-preview-messages', // class of the standard message div
               classLightboxMessageEE: 'os-site os-site-mail-hover-preview-messages-ee', // class of the embedded experience div
               classLightboxDialog: 'mailMessageDialog', // class of the dijit.Dialog of the lightbox    
               classCloseIconLightboxRtl: 'dijitDialogRtl', // class of the close icon right-to-left (BiDi)
               ...
               loadContainerDropdown: function(menuItem, event) {
                  // menuItem is the anchor element placed in the related list item (mail or calendar)
                  if (event.type === 'mouseover') lconn.core.header.menuMouseover(menuItem);
                  else if (event.type === 'click') lconn.core.header.menuClick(menuItem);
                  else if (event.type === 'focus') lconn.core.header.menuFocus(menuItem);
               },
               
               getDropdownContainerId: function(menuItem) { // menuItem is the anchor element placed in the related list item (mail or calendar)
                  return dojo.attr(menuItem, "_lconn_menuid"); // returns 'lconnheadermenu-mail' or 'lconnheadermenu-calendar' 
               },
               
               getServiceUrl: function() {
                  var serviceUrl = null;
                  var mailSvc = lconn.core.config.services.connectionsmail;
                  if (mailSvc) {
                     serviceUrl = lconn.core.url.getServiceUrl(mailSvc);
                  }
                  return serviceUrl;
               },
               
               getUrlParameter: function(locationHref, urlParameter) { // "parse" method returns an object with queryParameters
                  return lconn.core.url.parse(locationHref).queryParameters[urlParameter];
               },
               
               initContainer: function() {
                  var iContainer2 = dojo.getObject('com.ibm.lconn.gadget.container.iContainer2');
                  if (iContainer2)
                     iContainer2.init();
                  return iContainer2;
               },
               ...
               addLoginLogoutHandlers: function() {
                  // Login handler is needed when session is expired and we are presented with Login window
                  lconn.core.auth.addLoginHandler(cre$.util.hitch(this, this.clearCache));
                  // Logout handler to be called when some Logout link is pressed
                  lconn.core.auth.addLogoutHandler(cre$.util.hitch(this, this.clearCache));
               },
               
               addAttribute: function(node, attribute, value) {
                  // set/update an attribute of a DOM element, required as we add the dynamically generated id to the badge gadget site
                  return dojo.attr(node, attribute, value);
               },  
               ...
               getUserPrefs: function() {
                   return {
                       calendar: lconn.core.globalization.api.getCalendar(), 
                       textDirection: lconn.core.globalization.api.getTextDirection(), 
                       bidiEnabled: lconn.core.globalization.api.isBidiEnabled()
                   };
                },   
               ...
               generateMessageSite: function() {
                  this.messageSite = new dijit.layout.ContentPane({
                     content:
                         '<div class="' + this.classLightboxContainer + '">'
                         +  '<div id="'+this.gadgetIDs.message+'" class="' + this.classLightboxMessage + '"></div>'
                         +  '<div id="'+this.gadgetIDs.messageEE+'" class="' + this.classLightboxMessageEE + '"></div>'
                         +  '</div>'
                 });
               }, 
               ...
               generateMessageDialog: function() {
                  ...
                  this.messageDialog = new dijit.Dialog({
                  ...
                  this.messageDialog._size = function(){
                     ...
                     if (bb.w >= viewport.w || bb.h >= viewport.h) {
                        ...
                        if (this._singleChild && this._singleChild.resize) {
                           ...
                           this.containerNode.style.position = 'relative'; // workaround IE bug moving scrollbar or dragging dialog                
                        }
                     } else {
                        if (this._singleChild && this._singleChild.resize) {
                  ...
                  this.messageDialog._position = function(){
                     if (!dojo.hasClass(dojo.body(), 'dojoMove')){ // don't do anything if called during auto-scroll
                        ...
                        // If the viewport height is smaller than the lightbox height, then don't reposition height.
                        if (t>viewport.t){
                           node.style.top = t + 'px';
                        }
                        // If the viewport width is smaller than the lightbox width, then don't reposition width.
                        if (l>viewport.l){
                           node.style.left = l + 'px';
                        }
                        //added code for IE in RTL direction start
                        if (!!dojo.isIE && !dojo._isBodyLtr() && !!dijit._underlay && !!dijit._underlay.domNode) {
                           node.style.left = Math.floor((viewport.w - bb.w) / 2) + 'px'; // dialog absolute position
                           dijit._underlay.domNode.style.left = '0px'; // be sure shaded area starts from the beginning (from right)
                        }               
               ...
               getBadgeSpan: function() {
                  var domElements = dojo.query('.' + this.classBadge);                
               ...
               navigateGadgetHandlerCustomProperties: function() {
                  // TODO: could be an issue in Portal and other OneUI containers
                  this.addClass(document.getElementsByTagName('body')[0], 'lotusui30dojo');
               },
               
               clearCache: function () {
                  // summary:
                  //    Adds a callback function which should be invoked either when session has expired or during logout
                  //    to cleanup any specific information presented in cache.
                  //    Each gadget instance could react to such call accordingly.
                  ...
                  this.commonContainer.then(cre$.util.hitch(this, function(cc) {
                     ...
                     var areas = ["badge", "inbox", "calendar", "message"];
                     
                     cre$.util.forEach(areas, cre$.util.hitch(this, function(area, index){
                        if (index == 0 && this.notifySite ||
                           index == 1 && this.inboxSite ||
                           index == 2 && this.calendarSite ||
                           index == 3 && this.messageSite) {
                  ...
                  setTimeout(resolve, resolveTimeout); // resolve after 5sec in case of problem, so logout navigation completes
                  return deferred;
               },               

               startupSettings: function() { // used once the CommonContainer (cc) is loaded
                  // In Connections container we use Topics to set the related callbacks
                  // https://w3-connections.ibm.com/wikis/home?lang=en#/wiki/Lotus%20Connections%202.5/page/gadget%20Lifecycle%20Events
                  
                  // setting callback function when "gadgets.window.setTitle" is called in the lightbox. See "socmail/bootstrap/document.js"
                  var setTitleDocumentTopic = com.ibm.lconn.gadget.container.Topics.getSiteTopic(this.gadgetIDs.message, com.ibm.lconn.gadget.container.Topics.GadgetWindow.SITE_TOPIC_SET_TITLE);
                  this.topics.push(dojo.subscribe(setTitleDocumentTopic, cre$.util.hitch(this, function(){
                     this.updateDialogTitle(arguments[0] ? arguments[0] : arguments);
                  })));
                  // setting callback function when "gadgets.window.adjustHeight" is called in the mail dropdown. See "socmail/bootstrap/MailViewStandalone.js".
                  var adjustHeightInboxTopic = com.ibm.lconn.gadget.container.Topics.getSiteTopic(this.gadgetIDs.inbox, com.ibm.lconn.gadget.container.Topics.GadgetWindow.AFTER_ADJUST_HEIGHT);
                  this.topics.push(dojo.subscribe(adjustHeightInboxTopic, cre$.util.hitch(this, function(){
                     this.setDropDownHeight(this.getPane('inbox'), arguments, "inbox");
                  })));
                  // setting callback function when "gadgets.window.adjustHeight" is called in the calendar dropdown. See "socmail/bootstrap/CalendarViewStandalone.js".
                  var adjustHeightCalendarTopic = com.ibm.lconn.gadget.container.Topics.getSiteTopic(this.gadgetIDs.calendar, com.ibm.lconn.gadget.container.Topics.GadgetWindow.AFTER_ADJUST_HEIGHT);
                  this.topics.push(dojo.subscribe(adjustHeightCalendarTopic, cre$.util.hitch(this, function(){
                     this.setDropDownHeight(this.getPane("calendar"), arguments, "calendar");
                  })));
               },
               
               onMessageOpen: function (id, selections) {
                  this.closeCurrentSite(); // close the dropdown
                  // SM Defect 28065: Set "closeConfirmed" to false so that we can always clean-up the conainer when closing the light box.
               ...
               generateGadgetSite: function (options) {
                  // create the gadget site for the dropdown (mail or calendar)
               ...
               hideCalendarWhileLoading: function () {
                  ...
                  if (iframe) {
                     iframe.style.visibility = "hidden";
                     // SM Defect 29053: [IE] Display extra white area in calendar dropdown
               ...
               hideLightboxWhileLoading: function () {
                  ...
                  if (iframe) {
                     iframe.style.visibility = "hidden";
                     dojo.place('<table id="' + (this.messageSite.progressId = 'messageSiteProgress') + '" border="0" cellspacing="0" cellpadding="0"><tr><td>&nbsp;</td></tr></table>', iframe, "before");
                  }            
               },
               
               setBadgeAccessibilityAttributes: function () {
                  // Add accessibility attributes to the iframe of the badge gadget site
               ...
               showDialog: function () {
                  ...
                  // Defect 39277: CSS change for fixing blank page issue in IE
                  if (this.isIE()) {
                  ...
                  if (dijit._underlay && dijit._underlay.bgIframe
                        && dijit._underlay.bgIframe.iframe) {
                     var iframe = dijit._underlay.bgIframe.iframe;
                     iframe.setAttribute("role", "presentation");
                     iframe.setAttribute("title", "Background");
                     iframe.contentWindow.document
                           .write("<html><head><title>Background</title></head><body></body></html>");
                  }
               ...
               updateDialogTitle: function (/*String*/ pageTitle) {
                  // updates the dialog (message/event) title and its accessibility attributes (lightbox)
                  ...
                  if (!!this.messageDialog && !!this.messageSite && !!pageTitle) {
                     ...
                     // The message dialog domNode (i.e. div with id="dijit_Dialog_{id})
                     ...
                     // The message site domNode (i.e. div with id="dijit_layout_ContentPane_{id}").
               ...
               updateWarning: function (site) {
                  // a11y - warning dropdown - JAWS require a removal+addition of the DOM element to inform the user of the alert
               ...
               extraRpcCalls: function(rpcData) {
                  ...
                  if (rpcData.type == "com.ibm.socpim.MailDropDown") {
                     if (rpcData.dataObject == "hidden" && this.hoverLoaded.inbox) {
                        this.inboxSite.onClose(true); // close and set the focus back on header's Mail when ESC is pressed
                     }
                  }
                  else if (rpcData.type == "com.ibm.socpim.CalendarDropDown") {
                     if (rpcData.dataObject == "hidden" && this.hoverLoaded.calendar) {
                        this.calendarSite.onClose(true); // close and set the focus back on header's Calendar when ESC is pressed
                     }
                  }                  
         ...
         /*
          * Dojo now (1.9+) adds scroll bars to the popup container if it thinks that the popup height is 
          * greater than the viewport.  We don't want this behavior in ICM because we manage the scroll bars
          * ourselves within the popup. This behavior can be disabled by adding maxHeight = Infinity in the
          * popup.open args object, but unfortunately, MenuLauncher provides no mechanism that we can use
          * do this so we need to resort to the hackery below.
          * 
          * Would prefer to use dojo/aspect for this, but it's not available when using old style, 
          * non-AMD, dojo.require.
          */
