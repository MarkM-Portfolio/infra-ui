...
define([
        ...
        "dojo/topic",
        "dijit/_Widget"
], function(dojo, array, declare, lang, domClass, domConstruct, has, keys, topic, _Widget) {
    ...
    * Also used in concert with AutocompleteDataStore, which provides helper
    * methods for chaining requests and providing intelligent delays (so that
    * cached content can be displayed immediately, but remote content requires
    * more of a pause).
    * <p>
    ...
    * <p>
    * Subclasses of this class may choose to provide a default implementation of
    * initRenderer that instantiates a default rendering to provide a
    ...
    * to access selections made from the renderer. The data store object and the
    * data store are passed to this handler. The event information is not
    * available from this call - instead clients may choose to invoke methods on
    * the renderer.
    * <p>
    * TODO: Needs test case of multivalue with comma as delimeter TODO: fix
    * firefox position setting scrolling issue (set cursor to end, input doesn't
    * scroll)
    * 
    * @class ic-ui.AutocompleteInput
    * @author Clayton Coleman <claycole@us.ibm.com>
    */
   var AutocompleteInput = declare("com.ibm.oneui.AutocompleteInput", _Widget, /** @lends ic-ui.AutocompleteInput.prototype */
   {
      ...
      // The attribute name on the item to set the input to on selection. If
      // null,
      ...
      postMixInProperties : function() {
         this._search = lang.hitch(this, "search", null);
         this._searchImmediate = lang.hitch(this, "search", {
            immediate : true
         });
      },
      ...
      buildRendering : function() {
         ...
         if (!node || node.nodeName.toUpperCase() != "INPUT") {
            node = domConstruct.create("input", {
               autocomplete : "off"
            }, node);
         }
         this.domNode = node;
      ...
      /**
       * Invoked the first time the renderer widget is needed. Calls
       * initRenderer() once and only once per widget, and hooks the fetch
       * method on the renderer widget to this object.
       */
      _initRenderer : function() {
         ...
         if (renderer) {
            renderer.fetch = lang.hitch(this, "fetch");
         }
      },

      /**
       * Optional method that will be invoked the first time the renderer is
       * needed. This method may lazily instantiate the renderer.
       * 
      ...
      /**
       * Process incoming key events and dispatch accordingly. The renderer
       * widget is allowed to suppress or intercept all keystrokes. Some
       * keystrokes will immediately invoke a search, others will queue that
       * search for the next wait interval to elapse. All searches are done
       * after a timeout of at least 1ms, because the INPUT value element is not
       * updated until after the event handler is invoked.
       */
      onKeyPress : function(event) {
         ...
         /*
          * TODO: evaluate var autoReplace = this.autoReplace; var keyChar =
          * event.keyChar; // If we have an autoreplace pattern, convert
          * anything that matches it into multi-value-separating tokens if
          * (this.multipleValues && autoReplace && keyChar) {
          * autoReplace.lastIndex = 0; if (autoReplace.exec(keyChar)) {
          * dojo.stopEvent(event); this._normalize(autoReplace, this.token);
          *  // Behave as if we typed a separating character key = keyChar =
          * this.token; } }
          */
         ...
         if (!NOT_SEARCHABLE[key]) {
            switch (key) {
               case KEY_ENTER:
                  // When using a renderer widget that captures ENTER (to select
                  // a highlighted item),
                  // onSelect will have fired from the renderer widget prior to
                  // this line of code.
               ...
               case KEY_DELETE:
               case KEY_BACKSPACE:
                  // Both of these keys change the value of the input, so they
                  // should always search
                  search = true;
                  break;

               default:
                  // Non char keys (F1-F12 etc..) shouldn't open list.
                  // Ascii characters and IME input (Chinese, Japanese etc.)
                  // should.
                  // On IE and safari, IME input produces keycode == 229, and we
                  // simulate
                  // it on firefox by attaching to compositionend event (see
                  // compositionend method)
                  search = typeof key == 'string' || key == 229;
            }
         }

         // if the character isn't one that we would add to the search we want
         // to terminate
         // the currently executing request to the datastore. Otherwise, wait
         // for the next
         // search event timer to fire.
         if (!search) {
            this.ignoreFetch();
         }
         else if (this[WAIT_FOR_VALUE] === undefined) {
      ...
      /**
       * The blur event on the input should cancel any current timers, abort the
       * current datastore request, and notify the renderer that the blur has
       * occurred.
       */
      onBlur : function() {
         clearTimeout(this[WAIT_FOR_VALUE]);
      ...
      /**
       * Only do a search on click if we have focus, if no other search is in
       * flight, and if the renderer approves. When the input is in multiple
       * value mode, we disable click handling because the user is much more
       * likely to use the mouse to set specific locations.
       */
      onClick : function() {
         ...
         if (valueOnSelect) {
            ...
            if (this.multipleValues) {
               ...
               while (res = r.exec(value)) {
                  ...
                  if (last >= position) {
                     itemValue = value.substring(0, last - res[0].length + (res[1].length > 0 ? 1 : 0)) + itemValue + value.substring(last) + token
                           + (token != ' ' ? ' ' : '');
                     break;
                  }
               }
      ...
      /**
       * A search *should* happen, but can wait a short interval to see if other
       * keystrokes occur. Remember, the datastore may not immediately invoke
       * the query, so this interval is shorter than in past autocomplete
       * implementations.
       */
      suggestSearch : function() {
         this._shouldSearch = true;
         if (this[WAIT_FOR_VALUE] === undefined) {
            this[WAIT_FOR_VALUE] = setTimeout(this._search, DELAY_FOR_MORE_EVENTS);
         }
      },
       ...
       * Invoke a search.
       * 
       * @precondition This method requires that the caller has waited for any
       *               event which updates the value of the INPUT to complete
       */
      search : function(opts) {
         ...
         if (this._shouldSearch) {
            ...
            this.fetch(lang.trim(value), opts);
         }
      },
      ...
      isSearching : function() {
         return (this[WAIT_FOR_VALUE] !== undefined && this._shouldSearch) || this.inflight;
      },

      /**
       * Invoke the data store to retrieve the results of a search. This
       * implementation assumes the data store may wish to delay requests, so we
       * aggressively request content from the data store and abort inflight
       * requests frequently.
       */
      fetch : function(value, opts) {
         ...
         if (inflight) {
            /*
             * If we have an inflight request with the same query and we don't
             * want to upgrade it to an immediate query, let it run. Otherwise,
             * instruct the data store to abort the request if possible.
             */
         ...
         domClass.add(this.domNode, "lotusLoading");
      ...
      _finishFetch : function() {
         domClass.remove(this.domNode, "lotusLoading");
         delete this.inflight;
      },

      /**
       * Ignore the results of the current fetch, which will abort any remote
       * calls and prevent the event handler methods on the renderer from being
       * invoked.
       */
