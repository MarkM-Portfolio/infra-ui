...
define([
...
], function (array, runner, iEEGadgetRegistry) {
	...
	(function(){	
		...
		doh.register("com.ibm.lconn.gadget.test.registry.ee._EEGadgetMapperTests", [
			...
			function test_gadget_defs_extraction() {
				array.forEach(testDefs, function(testDef) {
					var mapping = new com.ibm.lconn.gadget.registry.ee._EEGadgetMapper(baseEEUrl, testDef);
					var result = [];
					
					array.forEach(mapping.gadgetDefs, function(mapping) {
			...
			function test_preload_resolution() {
				...
				array.forEach(testResolve.tests, function(test) {
					var url = test[0],
			...
			function test_ee_pattern_matcher() {
				...
				array.forEach(success, function(test) {
					doh.t(matcher.test(test));
				});
				
				array.forEach(fail, function(test) {
	...
	return com.ibm.lconn.gadget.test.registry.ee._EEGadgetMapperTests;
});
