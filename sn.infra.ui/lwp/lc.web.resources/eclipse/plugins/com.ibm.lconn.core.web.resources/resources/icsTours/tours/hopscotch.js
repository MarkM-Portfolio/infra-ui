...
this["templates"]["bubble_ess_default"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
with (obj) {
...
__p += '\n';
  ...
  function getNumFromI18n(strNum){
    if (typeof(strNum)!="string" ||(strNum.charCodeAt(0)<0x0660) || (strNum.charCodeAt(0)>0x0669)){
      return strNum;
    }
    var resultStr="";
    for (var i=0; i<strNum.length; i++){
      resultStr += strNum.charCodeAt(i) - 0x0660;
    }    
...
__p += '\n\n';
    ...
    /**
     * customI18N strings for ICS tours
     *
     * Defines the set of global strings for button text, alt, title, and aria-label text.
     *
     * These apply globally to all ICS tours and get pulled into this bubble-ess-default template.
     *
    */
 ...
 HopscotchI18N = {
  ...
  skipBtn: 'Skip',
  closeTooltip: 'Close',
  prevBtnDisabled: 'Previous button disabled',
  nextBtnDisabled: 'Next button disabled',
  tourAriaLabel: 'Tour: {numSteps} total steps',
  tourThisStepAriaLabel: 'Step {thisStep} of {numSteps}.'
  } 
;
__p += '\n';
 ...
 * ics_tours_utils
 *
 * Defines utility functions for use with the bubble_ess_default tour template.
 *
 * Currently one function - setFocusOnButton sets keyboard focus on the CTA, Next, or 
 * buttons or arrows, for keyboard accessibility support.
 * This can be included in the step of a tour with an onShow callback function
 * for example 
     onShow: function() {
       ics_tour_utils.setFocusOnButton();
     },
*/
    
ics_tour_utils = {
  /* Sets keyboard focus on next arrow, cta, done, or close buttons. CTA gets preference if it appears with next or done, then close. Next and done will never appear at the same time. Close is fall back when Next is disabled on last step and there is no done in the template. */
  setFocusOnButton: function() {
    if (document.getElementsByClassName("hopscotch-nav-button cta")[0]) {
      document.getElementsByClassName("hopscotch-nav-button cta")[0].focus();
      } else if (document.getElementsByClassName("hopscotch-nav-arrow next hopscotch-next")[0]) {
        document.getElementsByClassName("hopscotch-nav-arrow next hopscotch-next")[0].focus();
        } else if (document.getElementsByClassName("hopscotch-nav-button done")[0]) {
          document.getElementsByClassName("hopscotch-nav-button done")[0].focus();
          } else if (document.getElementsByClassName("hopscotch-bubble-close")[0]) {
            document.getElementsByClassName("hopscotch-bubble-close")[0].focus();
            } 
  }
  }
;
...
/* Display Tour step-by-steps
 * Note: This IBM ICS tour template supports tours with one or more steps.
 * Step 1: Action bar at top with Prev arrow greyed, Next arrow active, 
   plus current step number and number of totals steps.
 * Step 2 thru n-1: Regular content steps, with active 
   Prev arrow, step numbers, Next arrow.
 * Step n (last step):  Prev arrow, step number info, disabled next arrow.  
 * Note: For a one-step tour, the step is treated as the last step and default
   display is action bar with disabled Prev, 1 / 2, and disabled Next, plus close X.

   This template also includes these accessibility supports:
   * Sets role="alertdialog" for the bubble container.
   * An aria-label for the bubble content providing the total number of steps in the tour.
   * An aria-label and title for the previous and next navigation nav.
   * Appropriate aria-label, title, and alt attributes for images.
   * tabindex attributes to support keyboard navigation to the action buttons available, with Next or Done 
     first.
*/ ;
__p += '  \n';

/* In addition to the ics_tour_utils, the template also supports additional utilities by 
   including custom Data in a tour step.

   customData: {showArrow: "hide"}
     Hides display of the arrow pointer, so the bubble displays as a box with no arrow. 
     Useful for displaying a beginning or closing tour bubble.

   customData: {showNavActions: "hide"}
     Hides display of the actions bar, which includes the prev arrow, progress pips, 
     and next arrow or done button.

   customData: {showProgress: "hide"}
     Hides display of the progress numbers. Note that progress numbers are also hidden with showNavActions: "hide".

   customData: {showPrevBtn: "hide"}
     Hides display of the previous button. 

   customData: {loopFocus: "off"}
     Turns off auto-loop of tab key focus. Default is tabbing within the tour bubble loops within
     the bubble. Turn off to enable tab to focus outside the bubble, for an interactive tour.

   customData: {guidedLinks: [['tour-id','tour-title'],['tour-id','tour-title']...]}
     Provides a list of tour IDs and tour titles for display in a guided tour topic as a list of links. 
     TourManager adds additional support for rendering the links in the hopscotch bubble.
     
*/
;
__p += '     \n<div class="hopscotch-bubble-container" tabindex="1" role="alertdialog" title="' +
((__t = ( (utils.getI18NString('tourAriaLabel')).replace('{numSteps}',(optEscape(i18n.numSteps, tour.unsafe))) )) == null ? '' : __t) +
'" aria-labelledby="tour-label" aria-label="' +
((__t = ( (utils.getI18NString('tourAriaLabel')).replace('{numSteps}',(optEscape(i18n.numSteps, tour.unsafe))) )) == null ? '' : __t) +
'" style="width: ' +
((__t = ( step.width )) == null ? '' : __t) +
'px;">\n<div id="tour-label">' +
((__t = ( (utils.getI18NString('tourAriaLabel')).replace('{numSteps}',(optEscape(i18n.numSteps, tour.unsafe))) )) == null ? '' : __t) +
'</div>\n  ';
 /* checks for customData: {showNavActions: "hide"} and hides the hopscotch-actions if found */ ;
__p += '\n  ';
 if ((step.customData.showNavActions === 'hide') || ((getNumFromI18n(i18n.stepNum) === 1) && (tour.numSteps === 1))) { ;
__p += '\n    <div class="hopscotch-actions" style="display: none">\n  ';
 } else { ;
__p += '\n    <div class="hopscotch-actions">\n  ';
 } ;
__p += '\n    ';
 // check showPrevBtn ;
__p += '\n    ';
 if (step.customData.showPrevBtn !== "hide") 
      { ;
__p += '\n      ';
 // Displays disabled prev button for step 1 ;
__p += '\n      ';
 if  ((getNumFromI18n(i18n.stepNum) === 1) && (tour.numSteps > 1))
        { ;
__p += '\n          <label style="display: none" for="prev-disabled">' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'</label>\n          <span id="prev-disabled" class="hopscotch-nav-arrow prev is-disabled" title="' +
((__t = ( (utils.getI18NString('prevBtnDisabled')))) == null ? '' : __t) +
'" aria-label="' +
((__t = ( (utils.getI18NString('prevBtnDisabled')))) == null ? '' : __t) +
'"><svg class="hopscotch-svg-nav" role="img" aria-label="' +
((__t = ( (utils.getI18NString('prevBtnDisabled')))) == null ? '' : __t) +
'" x="0px" y="0px" viewBox="0 0 10 14"><polygon points="9.1,1 8,-0.1 2,5.9 0.9,7 2,8.1 8,14.1 9.1,13 3.1,7 "/><title>' +
((__t = ( (utils.getI18NString('prevBtnDisabled')))) == null ? '' : __t) +
'</title><desc>' +
((__t = ( (utils.getI18NString('prevBtnDisabled')))) == null ? '' : __t) +
'</desc></svg></span> \n      ';
...
__p += '      \n      ';
 // Displays enabled prev button for steps 2 through last ;
__p += '\n      ';
 if  (getNumFromI18n(i18n.stepNum) > 1) 
        { ;
__p += ' \n          <label style="display: none" for="prev-enabled">' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'</label>\n          <button id="prev-enabled" class="hopscotch-nav-arrow prev hopscotch-prev" title="' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'" aria-label="' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'" tabindex="7"><svg class="hopscotch-svg-nav" role="img" aria-label="' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'" x="0px" y="0px" viewBox="0 0 10 14"><polygon points="9.1,1 8,-0.1 2,5.9 0.9,7 2,8.1 8,14.1 9.1,13 3.1,7 "/><title>' +
((__t = ( (i18n.prevBtn))) == null ? '' : __t) +
'</title><desc>' +
...
__p += '\n    ';
 // when more than one step, show progress numbers ;
__p += '\n    ';
 if (tour.numSteps > 1)
    { ;
__p += '\n    ';
 // checks for customData: {showProgress: "hide"} and hides progress numbers if found ;
__p += '\n    ';
 if (step.customData.showProgress !== "hide") { ;
__p += '\n      <span class="progress-nums"> \n        ';
 // If not last step, use hopscotch info for current step number and total steps in the tour ;
__p += '\n        ';
 if (!step.isLast)
          { ;
__p += '\n            <span class="pip-num" title="' +
((__t = ( ((utils.getI18NString('tourThisStepAriaLabel')).replace('{numSteps}',(optEscape(i18n.numSteps, tour.unsafe)))).replace('{thisStep}',(i18n.stepNum)) )) == null ? '' : __t) +
'" aria-label="' +
((__t = ( ((utils.getI18NString('tourThisStepAriaLabel')).replace('{numSteps}',(optEscape(i18n.numSteps, tour.unsafe)))).replace('{thisStep}',(i18n.stepNum)) )) == null ? '' : __t) +
'">' +
((__t = ((i18n.stepNum))) == null ? '' : __t) +
' / ' +
((__t = ( (optEscape(i18n.numSteps, tour.unsafe)))) == null ? '' : __t) +
'</span>\n        ';
 // If this is the last step, make the total steps same as current step, in case a step got skipped for no target ;
__p += '\n        ';
 } else { ;
__p += '\n            <span class="pip-num" title="' +
((__t = ( ((utils.getI18NString('tourThisStepAriaLabel')).replace('{numSteps}',(i18n.stepNum))).replace('{thisStep}',(i18n.stepNum)) )) == null ? '' : __t) +
'" aria-label="' +
((__t = ( (utils.getI18NString('tourThisStepAriaLabel')).replace('{numSteps}',(i18n.stepNum)))) == null ? '' : __t) +
' ' +
((__t = ( (utils.getI18NString('tourThisStepAriaLabel')).replace('{thisStep}',(i18n.stepNum)))) == null ? '' : __t) +
'">' +
((__t = ((i18n.stepNum))) == null ? '' : __t) +
' / ' +
((__t = ((i18n.stepNum))) == null ? '' : __t) +
...
__p += '\n     ';
 // If this is not the last step, show enabled Next chevron else show disabled Next chevron ;
__p += '\n     ';
 if (!step.isLast)
       { ;
__p += '\n         <label style="display: none" for="next-enabled">' +
((__t = ( (i18n.nextBtn))) == null ? '' : __t) +
'</label>\n         <button id="next-enabled" class="hopscotch-nav-arrow next hopscotch-next" title="' +
((__t = ( (i18n.nextBtn) )) == null ? '' : __t) +
'" aria-label="' +
((__t = ( (i18n.nextBtn) )) == null ? '' : __t) +
'" tabindex="4"><svg class="hopscotch-svg-nav" role="img" aria-label="' +
((__t = ( (i18n.nextBtn) )) == null ? '' : __t) +
'" x="0px" y="0px" viewBox="0 0 10 14"><polygon points="9.1,1 8,-0.1 2,5.9 0.9,7 2,8.1 8,14.1 9.1,13 3.1,7 "/><title>' +
((__t = ( (i18n.nextBtn) )) == null ? '' : __t) +
'</title><desc>' +
...
__p += '\n         <label style="display: none" for="next-disabled">' +
((__t = ( (i18n.nextBtn))) == null ? '' : __t) +
'</label>\n         <span id="next-disabled" class="hopscotch-nav-arrow next is-disabled" title="' +
((__t = ( (utils.getI18NString('nextBtnDisabled')))) == null ? '' : __t) +
'" aria-label="' +
((__t = ( (utils.getI18NString('nextBtnDisabled')))) == null ? '' : __t) +
'"><svg class="hopscotch-svg-nav" role="img" aria-label="' +
((__t = ( (utils.getI18NString('nextBtnDisabled')))) == null ? '' : __t) +
...
__p += '\n  </div>\n  <div class="hopscotch-bubble-content">\n    ';
 if(step.title !== ''){
      if ((step.customData.showNavActions === 'hide') || ((getNumFromI18n(i18n.stepNum) === 1) && (tour.numSteps === 1))) { ;
__p += '\n        <h3 class="hopscotch-title no-actions">' +
((__t = ( optEscape(step.title, tour.unsafe) )) == null ? '' : __t) +
'</h3>\n      ';
 } else { ;
...
__p += '\n    ';
 if ((step.customData.showNavActions === 'hide') || ((step.title === '') && ((getNumFromI18n(i18n.stepNum) === 1) && (tour.numSteps === 1)))) { ;
__p += '\n        <div class="hopscotch-content no-actions">\n      ';
 } else { ;
...
'\n    ';
 if(buttons.showCTA){ ;
__p += '\n      <div class="hopscotch-cta-actions">\n        <a class="hopscotch-nav-button cta hopscotch-cta" tabindex="6" role="link" aria-label="' +
((__t = ( buttons.ctaLabel )) == null ? '' : __t) +
'" title="' +
((__t = ( buttons.ctaLabel )) == null ? '' : __t) +
'">' +
...
__p += '\n      ';
 // Inserts links provided by customData.guidedLinks ;
__p += '\n      ';
 if (step.customData.guidedLinks) { ;
__p += '\n        <div>\n        ';
 for ( var i=0; i<step.customData.guidedLinks.length; i++) {
          thisLinkId = step.customData.guidedLinks[i][1];
          thisLinkTitle = step.customData.guidedLinks[i][0]; ;
__p += '\n            <p><a class="ics-tours guided-links" role="link" id="' +
((__t = ( thisLinkId )) == null ? '' : __t) +
'" tabindex="6" aria-label="' +
((__t = ( thisLinkTitle )) == null ? '' : __t) +
'" title="' +
((__t = ( thisLinkTitle )) == null ? '' : __t) +
'">' +
((__t = ( thisLinkTitle )) == null ? '' : __t) +
...
__p += '\n      ';
 if ((step.customData.showNavActions === 'hide') || ((getNumFromI18n(i18n.stepNum) === 1) && (tour.numSteps === 1))) { ;
__p += '\n        <label style="display: none" for="close-noactions">' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'</label>\n        <button id="close-noactions" title="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" aria-label="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" class="hopscotch-bubble-close hopscotch-close no-actions" tabindex="5"><svg class="hopscotch-svg-close" role="img" aria-label="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" x="0px" y="0px" viewBox="0 0 14 14"><polygon points="13.1,2 12,0.9 7,5.9 2,0.9 0.9,2 5.9,7 0.9,12 2,13.1 7,8.1 12,13.1 13.1,12 8.1,7 "/><title>' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'</title><desc>' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'</desc></button> \n      ';
 } else { ;
__p += '\n        <label style="display: none" for="close-tooltip">' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'</label>\n        <button id="close-tooltip" title="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" aria-label="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" class="hopscotch-bubble-close hopscotch-close" tabindex="5"><svg class="hopscotch-svg-close"  role="img" aria-label="' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'" x="0px" y="0px" viewBox="0 0 14 14"><polygon points="13.1,2 12,0.9 7,5.9 2,0.9 0.9,2 5.9,7 0.9,12 2,13.1 7,8.1 12,13.1 13.1,12 8.1,7 "/><title>' +
((__t = (i18n.closeTooltip )) == null ? '' : __t) +
'</title><desc>' +
...
__p += '\n  ';
 /* loop tabs within bubble, unless customData: {loopFocus === "off"} */ ;
__p += '\n  ';
 if (step.customData.loopFocus !== 'off') { ;
__p += '\n  <div id="loop-tab-8" tabindex="8" role="presentation"></div>\n  <div id="loop-tab-2" tabindex="2" role="presentation"></div>\n  <div id="loop-tab-3" tabindex="3" role="presentation"></div>\n  ';
 } ;
__p += '\n</div>\n';
 /* checks for customData: {showArrow: "hide"} and hides pointer arrow */ ;
__p += '\n';
 if (step.customData.showArrow === 'hide') { ;
__p += '\n  <div class="hopscotch-bubble-arrow-container hopscotch-arrow" style="display: none">\n';
 } else { ;
__p += '\n  <div class="hopscotch-bubble-arrow-container hopscotch-arrow">\n';
 } ;
__p += '\n  <svg class="hopscotch-img-arrow" x="0px" y="0px" viewBox="0 0 17 22"><polygon points="11,11 11,11 11,11 0,0 0,22 "/></svg>\n</div>\n';
