/* Copyright IBM Corp. 2014, 2015  All Rights Reserved.              */
	...
	define([
		...
		"dojo/topic",
		"dojo/ready",
		"ic-as/gadget/ASComponentUtil",
		"ic-as/util/xhr/XhrHandler"
	], function (dojo, array, lang, windowModule, domGeometry, ioQuery, topic, ready, ASComponentUtil, XhrHandler) {
		...
		(function(scope){
			...
			function setupPrefs(){
				...
				if(urlParams){
					lang.mixin(prefs, ioQuery.queryToObject(urlParams));	
				}
				asc = {};
			...
			function noOp() {}
			 ...
			 function attachDynamicHeight(){
				 topic.subscribe("com/ibm/social/as/event/resizeTextHeight", handleResize);
			...
			bootAS = function() {
				...
				asc.util = new ASComponentUtil();
				asc.util.initLogger(); // must be first.. things will be starting to call as_console_debug
		
				XhrHandler.init({isGadget:false, useOauth: asc.useOAuth}); // we need xhr to know its context
				asc.util.initAS();
		...
		return com.ibm.social.as.gadget.component.ActivityStreamInit;
	});
