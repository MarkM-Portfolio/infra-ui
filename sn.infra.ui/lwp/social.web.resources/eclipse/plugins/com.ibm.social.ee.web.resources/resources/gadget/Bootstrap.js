...
dojo.require("com.ibm.social.ee.Module");
...
(function () {
    ...
    dojo.mixin(_bootstrap, {
        ...
        initializeNetwork: function (context) {
            ...
            var isMultiTenant = lconn.core.config.properties.DeploymentModel === "MultiTenant";

            //read property from open social config
            var isAnonymousAllowedConfig = dojo.getObject("com.ibm.social.eeconfig.config.isAnonymousAllowed");
            ...
            var clzStr = com.ibm.social.ee.EventRegistry.getClass(event);
            this.clz = dojo.getObject(clzStr);
            ...
            var networkClazz = isSmartCloud ? com.ibm.social.incontext.NetworkDojo : com.ibm.social.incontext.NetworkOS;
            ...
            this.network = new networkClazz({
                ...
                onAuthApproval: function (oauthApprovalUrl, retryFunction, retryOpts) {
                    ...
                    dojo.forEach(dojo.query(".eeOAuthImg"), function (imgEl) {
            ...
            if(!hasCurrentViewer){
                ...
                this.network.getJson({
                    ...
                    handle: function (response) {
                        var user = com.ibm.social.incontext.util.auth.parseUserFromOpensocialResponse(response);

                        if(user){
                            com.ibm.social.incontext.util.auth.setCurrentViewer(user);
                        }
        ...
        initializeKeyHandlers: function () {
            if (gadgets.util.hasFeature("com.ibm.connections.ee")) {
                ...
                dojo.subscribe("lconn/core/mentions/focus", function (tracking) {
                    if (tracking) {
                        com.ibm.connections.ee.preventCloseOnEscape();
                    }
                });
        ...
        onSizeChange: function (height) {
            ...
            else if (dojo.isIE && this.isResizingEnabled()) {
                // Get the max height of all children of body
                maxHeight = this.calculateHeight();

                var margin = dojo.isIE <= 8 ? 20 : 5;
        ...
        onLoadGadgetComplete: function (context) {
            ...
            onLoadFunc = function () {
                ...
                if (context.portletUrlSettings) {
                    ...
                    pocUtil.updateEEUrls(dojo.byId(ID_GADGETBODY_DIV), context);
                    dojo.subscribe("com/ibm/social/ee/data/loaded", dojo.hitch(pocUtil, pocUtil.updateEEUrls, ID_GADGETBODY_DIV, context));
                    dojo.subscribe("com/ibm/social/ee/comment/afterCreated", dojo.hitch(pocUtil, pocUtil.updateEEUrls, ID_GADGETBODY_DIV, context));
                    dojo.subscribe("com/ibm/social/ee/comment/afterUpdated", dojo.hitch(pocUtil, pocUtil.updateEEUrls, ID_GADGETBODY_DIV, context));
                }
            ...
            var gadgetOpts = {
                ...
                context: context,
                onLoaded: dojo.hitch(this, this.onGadgetLoaded, onLoadFunc),
        ...
        setTitle: function (context) {
            if (gadgets.util.hasFeature("settitle")) {
                ...
                if (context.eventTitle) {
                    var node = dojo.create("span", { innerHTML: context.eventTitle });
        ...
        getClassesToTrace: function () {
            var classesToTrace = com.ibm.social.eeconfig.config.jsTrace;
            if (!classesToTrace) {
...
//Registering dojo listeners
    (function () {
        if (gadgets.util.hasFeature("ibm.connections.ee")) {
            ...
            dojo.subscribe("com/ibm/social/ee/event/modified", function (entry) {
                ee.setDirty(entry.modified);
            });
            dojo.subscribe("com/ibm/social/ee/event/addentry", function (id, timestamp) {
