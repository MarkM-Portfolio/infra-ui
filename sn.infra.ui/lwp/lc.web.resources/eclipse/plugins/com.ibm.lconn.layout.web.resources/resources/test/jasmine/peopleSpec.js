...
dojo.provide("com.ibm.lconn.layout.test.jasmine.peopleSpec");
...
(function(peopleLayout) {
   ...
   describe("the interface of com.ibm.lconn.layout.people", function() {
   ...
   describe("the com.ibm.lconn.layout.people.createLink() method", function() {
      ...
      it("returns a bizcard microformat linking to Profiles when passing the user as first argument, a text node as second argument, and a hyperlink node as third argument", function() {
         var parent = dojo.create('a');
         var child = dojo.doc.createTextNode('Susan Adams44');
      ...
      it("sets the expected class when the user is active", function() {
         ...
         expect(dojo.hasClass(link, "lotusPerson")).toBeTruthy();
         expect(dojo.hasClass(link, "lotusPersonInactive")).toBeFalsy();
      });

      it("sets the expected class when the user is inactive", function() {
         var link = peopleLayout.createLink(INACTIVE_USER);

         expect(dojo.hasClass(link, "lotusPerson")).toBeFalsy();
         expect(dojo.hasClass(link, "lotusPersonInactive")).toBeTruthy();
      });
   });
