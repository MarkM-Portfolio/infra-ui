...
public class CustomResources implements ResourceOverrideService
{
  ...
  private static final String HIKARI_PATH_PREFIX = "com.ibm.social.hikari.theme/css/defaultTheme/"; //$NON-NLS-1$

  private static final String HIKARI_PATH_OVERRIDE = "hikariTheme/"; //$NON-NLS-1$

  private static final String HIKARI_APPLICATION_PATH_PREFIX = "com.ibm.social.hikari.theme/css/base/applications/"; //$NON-NLS-1$

  private static final String HIKARI_APPLICATION_PATH_OVERRIDE = "hikariTheme/applications/"; //$NON-NLS-1$
  ...
  /**
   * Returns a path that is suitable for the customization directory. This method handles a few packaged bundles like gen4 and hikari as a
   * special case, and look for custom files in <code>&lt;CONNECTIONS_CUSTOMIZATION_PATH&gt;/themes/&lt;theme_id&gt;Theme/</code>. In the
   * future we may want to declare a customization path in the bundle.
   */
  private static String customCssPath(String path)
  {
    ...
    if (path.startsWith(HIKARI_PATH_PREFIX))
      return HIKARI_PATH_OVERRIDE + path.substring(HIKARI_PATH_PREFIX.length());
    if (path.startsWith(HIKARI_APPLICATION_PATH_PREFIX))
      return HIKARI_APPLICATION_PATH_OVERRIDE + path.substring(HIKARI_APPLICATION_PATH_PREFIX.length());
    return path;
  }
}
