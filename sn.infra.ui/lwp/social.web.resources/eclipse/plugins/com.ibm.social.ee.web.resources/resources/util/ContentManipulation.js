...
(function () {
   function startsWith(str, txt){
		...
		while(stack.length > 0 && ((textSummary.size < threshold) || (threshold == -1))) {
			...
			if ((!node._discovered || node.nodeName.toLowerCase()=="a") && node.nodeType != 3 && node.nodeName) {
				...
				if (brCount < 2 && (type =="br" || type == "div" || type == "p" || type == "h1" || type == "h2" || type == "h3" || type == "h4" || type == "h5" || type == "blockquote" || type == "tr" || type == "li")) { 
				   ...
				   if (hasContent || dojo.trim(textSummary.content).length > 0) { 
				   	hasContent = true;
	...
	cm.generate_images = function(HTMLString, imgHeight, imgWidth, numImages, imageContainer, url, callback) {
		...
		var images = dojo.query("img", contentDiv);
		if(images.length === 0) {
			if (callback)
				   callback(0);
			return;
			...
			for (var i = 0; i < numImages; i++) { 
				if (state.images.length > 0) {
					...
					if(img.complete || img.height > 0 || img.width > 0) {		
						...
						dojo.connect(img, "onload", dojo.hitch(cm, cm._image_loaded, minHeight, minWidth, numImages, el, state, {target: img}, url, callback, false));
						dojo.connect(img, "onerror", dojo.hitch(cm, cm._image_loaded, minHeight, minWidth, numImages, el, state, {target: img},url, callback, true));
						document.body.appendChild(img);
					}
				}
				else
	...
	cm._getImageUrl = function (url) {
		...
		var filesCfg = dojo.getObject("lconn.core.config.services.files"), blogsCfg = dojo.getObject("lconn.core.config.services.blogs");
	...
	cm._add_images = function(images, minHeight, minWidth, numImages, el, url, callback) {
	   var isLtr = dojo._isBodyLtr();
		...
		for (var i = 0; i < images.length && i < numImages; i++) {
			...
			var a = dojo.create("a", {href: url, target: "_blank", className: "eeImgPrev", style: {height: minHeight + "px", width: minWidth + "px" }}, el); 
			a.appendChild(img);
		}
		if (callback)
