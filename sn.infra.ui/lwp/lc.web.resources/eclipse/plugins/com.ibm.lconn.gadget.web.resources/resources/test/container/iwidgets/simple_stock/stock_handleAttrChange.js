...
dojo.declare("stock_handleAttrChange",null,{
	...
	handleAttributeChange:function(iEvent){
		...
		var changes = iEvent.payload.changes;
		var len = changes.length;
		var i = 0;
		for ( i = 0; i<len; i++ ){
			if ( changes[i].id === "symbol" ) {
				this._updateChart(changes[i].newVal);
				this._updateChart(changes[i].newVal);
				break;
			}
		}
	...
	onview:function(){
		...
		dojo.connect(changeButton, "onclick", this, function() {
			...
			this.iContext.getiWidgetAttributes().setItemValue("symbol",symbol);
...
//		var locale = this.iContext.getElementById("titleLocale").options[this.iContext.getElementById("titleLocale").options.selectedIndex].value;
		var title = iDescriptor.getItemValue("title");
	...
	_updateCompanyName : function(symbol) {		
		...
		var callback = cre$.util.hitch(this, function(xhr){
			...
			data = data.replace(/\/\//,"");
			data = this.iContext._iRuntime.parseToJson(data);
			console.log(data);
			if(data) {
				data = data[0];
	...
	save : function(){
      ...
      var iDescriptor = this.iContext.getiDescriptor();
      iDescriptor.setLocalizedItemValue("title", "stock_new", "en");
      iDescriptor.commit();
