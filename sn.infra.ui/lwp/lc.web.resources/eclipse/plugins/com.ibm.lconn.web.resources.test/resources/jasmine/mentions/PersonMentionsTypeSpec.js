...
/**
 * Jasmine spec for {@link lconn.core.widget.mentions.PersonMentionsType}
 * 
 * @module lconn.test.jasmine.mentions.PersonMentionsTypeSpec
 */
dojo.provide("lconn.test.jasmine.mentions.PersonMentionsTypeSpec");

dojo.require("lconn.core.widget.mentions.PersonMentionsType");

(function(PersonMentionsType) {
   var args = {
      parentNode : dojo.create('span', {id: "testSpanNode"}),
      network: {}
   }

   describe("the lconn.core.widget.mentions.PersonMentionsType class", function() {
      var node;
      beforeEach(function() {
   ...
   describe("the lconn.core.widget.mentions.PersonMentionsType.initDefaultDataStore() method", function() {
      var node, _search;
      beforeEach(function() {
         _search = lconn.core.widget.mentions.PersonMentionsType.SEARCH_URL;
      });
      
      afterEach(function() {
         node.cleanUp();
         lconn.core.widget.mentions.PersonMentionsType.SEARCH_URL = _search;
         _search = undefined;
      });
      ...
      it("uses the anonymous service as the default TA feed", function() {
         ...
         expect(node._typeaheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.search) + '/anonymous/people/typeahead');
         expect(node._extendedTypeAheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial) + '/anonymous/rest/people/@public/@all');
      });

      it("uses oauth service when network oauth defined as the TA feed", function() {
         node.network = {oauth : true};
         node.initDefaultDataStore();
         expect(node._typeaheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.search) + '/oauth/people/typeahead');
         expect(node._extendedTypeAheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial) + '/oauth/rest/people/@public/@all');
      });

      it("uses PeopleDataStoreOpenSocial as data store when lconn.core.config.services.search is NOT defined", function() {
         delete lconn.core.widget.mentions.PersonMentionsType.SEARCH_URL;
         node = new PersonMentionsType(args);
         expect(node.typeaheadDataStore.declaredClass).toBe('lconn.core.PeopleDataStoreOpenSocial');
      });

      it("uses anonymous opensocial service when lconn.core.config.services.search is NOT defined", function() {
         delete lconn.core.widget.mentions.PersonMentionsType.SEARCH_URL;
         node = new PersonMentionsType(args);
         expect(node._typeaheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial) + '/anonymous/rest/people/@public/@all');
      });

      it("uses oauth opensocial service when lconn.core.config.services.search is NOT defined", function() {
         delete lconn.core.widget.mentions.PersonMentionsType.SEARCH_URL;
         args.network = {oauth : true};
         node = new PersonMentionsType(args);
         expect(node._typeaheadFeed).toBe(lconn.core.url.getServiceUrl(lconn.core.config.services.opensocial) + '/oauth/rest/people/@public/@all');
      });
...
}(lconn.core.widget.mentions.PersonMentionsType));
