...
dojo.provide("lconn.core.PeopleFilterList");
...
(function() {
    ...
    * 
    * @class lconn.core.PeopleFilterList
   ...
   w.addPerson = function addPerson(personObj) {
      ...
      this.connect(link, 'onclick', dojo.hitch(this, '_onNameClick', personObj));
      this.connect(link, 'onkeydown', dojo.hitch(this, '_onNameKeyDown', link, personObj));
      // actual link contents
      var span = document.createElement('span');
      if (this.showBizCard && !personObj.noCard) {
         span.innerHTML = lconn.core.NameUtil.getHTML(personObj.name, personObj.email, personObj.uuid);
      }
      else {
         span.innerHTML = lconn.core.HTMLUtil.escapeText(personObj.name);
         dojo.addClass(span, "bidiAware");
      }
      link.appendChild(span);
      lconn.core.globalization.bidiUtil.enforceTextDirectionOnPage(link);
      ...
      if (this.showBizCard && window.SemTagSvc && SemTagSvc.parseDom && dojo.config.isLotusLiveEnabled) {
         SemTagSvc.parseDom(0, link);

         dojo.query("a", link).forEach(function(node) {
            // If user have bizcard, set it can't be tabed, set link's bizCard
            // and
            ...
            if (!link.bizCard) {
               ...
               var linkLabel = dojo.attr(link, "aria-label");
               var bizLabel = dojo.attr(node, "aria-label") || dojo.attr(node, "title");
               linkLabel = linkLabel + ", press enter to popup " + bizLabel;
               dojo.attr(link, "aria-label", linkLabel);
            }
         });
      }
   ...
   w.removePerson = function removePerson(personObj) {
      ...
      if (optionId) {
         ...
         if (person) {
            ...
            dojo.query('[role$=\'option\']', this.domNode).forEach(function(node) {
   ...
   w._resetKeys = function _resetKeys(selIdx) {
      ...
      if (this.count > 0) {
         ...
         // keyboard
         this._keyHelper = new lconn.core.aria._Helper(this.domNode, {
   ...
   dojo.declare("lconn.core.PeopleFilterList", [
         dijit._Widget,
         dijit._Templated,
         lconn.core.Res
   ], w);
