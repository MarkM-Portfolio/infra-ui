...
define([
	...
	"dojo/dom-class",
	"dojo/i18n",
	"dojo/i18n!./nls/PeopleFinderService",
	"ic-personcard/widget/PersonWidget",
	"../../auth",
	"../../url",
	"../../config/services",
	"../DataReader",
	"../../typeahead/Service"
], function (dojo, declare, lang, domClass, i18n, i18nPeopleFinderService, PersonWidget, auth, coreUrl, coreServices, DataReader, Service) {

	/**
	 * ic-core.typeahead.Service implementation for
	 * People Finder service
	 * 
	 * @class ic-core.peopleFinder.directory.PeopleFinderService
	 * @author Andrea Paolucci <andreapa@ie.ibm.com>
	 */
	var PeopleFinderService = declare(
		"lconn.core.peopleFinder.directory.PeopleFinderService",
		Service, /** @lends ic-core.peopleFinder.directory.PeopleFinderService.prototype */
	{
		//ic-core.peopleFinder.DataReader vars
		/**
		...
		//End of ic-core.peopleFinder.DataReader vars
		...
		postMixInProperties: function(){
			...
			this.strings = i18nPeopleFinderService;
			lang.mixin(this, this.strings);
		},
		
		postCreate: function() {
			if(!this.isDisabled) {
				this.isDisabled = (auth.getUser() && auth.getUser().isExternal);
			}
			...
			if(this.additionalFields) {
				lang.mixin(this._dataReader.additionalFields, this.additionalFields);
			}
		...
		executeQuery: function(query) {
			...
			} else if(this.isLoginNeeded && !auth.isAuthenticated()) {
				this._handleLogin(promise);
			} else {
				this._dataReader.executeQuery(query,
						lang.hitch(this, "_handleResults", promise, query),
						lang.hitch(this, "_handleError", promise));
			}
		...
		initResults: function(elements) {
			...
			for(var i=0; i<elements.length; i++) {
				...
				var entryWidget = new PersonWidget({
				...
				if(this.isFocusable) {
					entryWidget.connect(entryWidget, "onEntryFocused", lang.hitch(this, "focusCallback", entryWidget.domNode));
				}
		...
		getDirectoryLink: function() {
			var url = coreUrl.getServiceUrl(coreServices.profiles) || coreUrl.getServiceUrl(coreServices.scprofiles).toString().replace(/\/contacts/, "");
			url += "/html/searchProfiles.do";
			return url;
		}
	});
	return PeopleFinderService;
});
