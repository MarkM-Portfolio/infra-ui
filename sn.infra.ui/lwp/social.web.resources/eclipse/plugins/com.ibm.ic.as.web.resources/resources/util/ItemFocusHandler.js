/* Copyright IBM Corp. 2012, 2015  All Rights Reserved.           */
	...
	define([
	...
	], function (declare, lang, domClass, topic, events, html) {
		...
		var ItemFocusHandler = declare("com.ibm.social.as.util.ItemFocusHandler", null,
		{
			currentItem: null,
			...
			constructor: function() {
				...
				this.gotFocusHandle = topic.subscribe(events.ITEMGOTFOCUS, lang.hitch(this, function(item,e,focus) { this._changeItemFocus(item, e, focus);}));
				
				// Subscribe to event published when a news item loses focus.
				this.lostFocusHandle = topic.subscribe(events.ITEMLOSTFOCUS, lang.hitch(this, function(item, e) { this._itemLostFocus(item, e);}));
				
				// Subscribe to event published when a news item loses focus.
				this.selectHandle = topic.subscribe(events.ITEMSELECTED, lang.hitch(this, function(item, e) { this._itemSelected(item, e);}));
				
				// Subscribe to event published when a news item loses focus.
				this.deselectHandle = topic.subscribe(events.ITEMDESELECTED, lang.hitch(this, function(item, e) { this._itemDeselected(item, e);}));
				
				// Listen out for when the feed is destroyed and remove references to news items
				this.feedDestroyHandle = topic.subscribe(events.FEEDDESTROYED, lang.hitch(this, function() {
			...
			_itemSelected: function(item, e) {
				if (!this.currentSelectedItem || item != this.currentSelectedItem) {
		            ...
		            var dirty = lang.getObject("currentSelectedItem.commentInput.dirty", false, this);
		
		            var confirmCloseCallback = lang.hitch(this, function(confirmClose){
		                if(confirmClose){
			...
			_selectCurrentItem: function(item) {
				if (item.newsItemNode) {
					domClass.add(item.newsItemNode, "lotusPostSelected");
			...
			_deselectNewsItem: function(item, selectInProgress) {
				...
				if(item && item.newsItemNode && item !== this.currentSelectedItem) {
					domClass.remove(item.newsItemNode, "lotusPostSelected");	
					if(selectInProgress){
						domClass.remove(this.currentSelectedItem.newsItemNode, "lotusPostSelected");				
					}
				}
			...
			_itemLostFocus: function(item, e) {
				if (typeof e=="undefined") {
					...
					setTimeout(lang.hitch(this, "_unsetActiveNewsItem"),20);
				} else {
			...
			_disableCurrentNewsItem: function() {
				...
				if (this.currentItem && this.currentItem.newsItemNode) {
					domClass.remove(this.currentItem.newsItemNode, "lotusPostHover");
				}
			...
			_setCurrentItem: function(item) {
				if (item.newsItemNode) {
					domClass.add(item.newsItemNode, "lotusPostHover");
			...
			_getParentNewsItem: function(node) {
			    ...
			    while (node) {
			    	...
			    	if (domClass.contains(node, "activityStreamNewsItemContainer")) {
			    		break;
			    	}
			 ...
			 */
			destroy: function() {
				this.gotFocusHandle.remove();
				this.lostFocusHandle.remove();
				this.selectHandle.remove();
				this.deselectHandle.remove();
				this.feedDestroyHandle.remove();
			}
		});
		return ItemFocusHandler;
	});
