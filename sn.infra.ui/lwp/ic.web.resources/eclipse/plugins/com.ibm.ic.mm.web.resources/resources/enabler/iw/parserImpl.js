...
define([
	...
	"ic-mm/enabler/iw/utils",
	"ic-mm/enabler/xpath",
	"ic-mm/enabler/xslt"
], function (declare, debug, parser, utils, xpath, xslt) {

	declare("com.ibm.mm.enabler.iw.parser.WidgetParser",null,  {
	     parseWidgetDefinition: function(){   
	         return null;
	     }
	});
	declare("com.ibm.mm.enabler.iw.parser.legacyXMLParser",parser.WidgetParser,  {
	    ...
	    namespaces: {
	     ...
	     parseWidgetDefinition: function(){  
	         var xmlData = xslt.loadXmlString(this.xmlStr);
	     ...
	     readMarkup: function ( /*XMLDocument*/xmlData ) {
	                   debug.entry( "legacyXMLParser.readMarkup", xmlData.text );
	                   var contentsXPath = "/iw:iwidget/iw:content";
	                   //we support html fragment only in iw:content
	                   var nodes = xpath.evaluateXPath(contentsXPath, xmlData,this.namespaces);
	                   var defaultContent = "";					
	                   ...
	                   debug.exit("legacyXMLParser.readMarkup",defaultContent); 
	               ...
	               readItemSets: function ( /*XMLDocument*/xmlData ) {
	                    debug.entry("legacyXMLParser.readItemSets");
	                        ...
	                        for (var j=0; j<child.length; j++) {
	                            var aItemNode = child[j];
	                             if ( aItemNode.nodeType == 1) {
	                                 ...
	                                 debug.entry("legacyXMLParser.readItemSets found attribute name:"+aItemNode.getAttribute("name")+" value:"+aItemNode.getAttribute("value")); 
	                    ...
	                    debug.exit("legacyXMLParser.readItemSets");
	                    return itemSetsArr;
	               },
	               readPayloadDefs: function ( /*XMLDocument*/xmlData ) {
	                    debug.entry("legacyXMLParser.readPayloadDefs");
	                    var payloadDefsArr = {};
	                    var contentsXPath = "/iw:iwidget/iw:payloadDef";
	                    var nodes = xpath.evaluateXPath(contentsXPath, xmlData,this.namespaces);
	                    for (var i=0; i< nodes.length ;i++) {
	                        var aNode =  nodes[i];
	                        var payloadDef = utils.getPayloadDef(aNode);                                                                                 
	                        payloadDefsArr[payloadDef.name]=payloadDef;
	                    }
	                    debug.exit("legacyXMLParser.readPayloadDef ",nodes.length+ " payloadDefs are saved "); 
	               ...
	               readPublicEvents:function(/*XMLDocument*/xmlData,/*String*/eventType){
	                ...
	                readResources:function(/*XMLDocument*/xmlData){
	                    ...
	                    var resources = [];
	                    var nodes = xpath.evaluateXPath(resourcePath, xmlData,this.namespaces);
	                    if (typeof nodes != "undefined" && nodes != null && nodes.length !=0 ){
	                        for (var i=0; i<nodes.length;i++){
	                            var node = nodes[i];
	                            var resource = {};
	...
	declare("com.ibm.mm.enabler.iw.parser.standardXMLParser",parser.WidgetParser,  {
	    constructor:function(responseText){
	        this.xmlStr = responseText;
	    },
	    namespaces: {
	    ...
	    reservedAttributes:{
	     ...
	     readMarkup: function ( /*XMLDocument*/xmlData ) {
	                   debug.entry( "standardXMLParser.readMarkup", xmlData.text );
	                   var contentsXPath = "/iw:iwidget/iw:content";
	                   //we support html fragment only in iw:content
	                   var nodes = xpath.evaluateXPath(contentsXPath, xmlData,this.namespaces);
	                   var contents = {};
	                   ...
	                   debug.exit("standardXMLParser.readMarkup",contents); 
	                   return contents;
	               },
	               readItemSets: function ( /*XMLDocument*/xmlData ) {
	                    debug.entry("standardXMLParser.readItemSets");
	                    ...
	                    for (var i=0; i< nodes.length ;i++) {
	                        ...
	                        for (var j=0; j<child.length; j++) {
	                            var aItemNode = child[j];
	                             if ( aItemNode.nodeType == 1) {
	                                 var isReadOnly = aItemNode.getAttribute("readOnly");
	                                 debug.entry("standardXMLParser.readItemSets found attribute name:"+aItemNode.getAttribute("name")+" value:"+aItemNode.getAttribute("value")); 
	                    ...
	                    debug.exit("standardXMLParser.readItemSets");
	                    return itemSetsArr;
	               },
	               readPayloadDefs: function ( /*XMLDocument*/xmlData ) {
	                    debug.entry("standardXMLParser.readPayloadDefs");
	                    var payloadDefsArr = {};
	                    var contentsXPath = "/iw:iwidget/iw:payloadDef";
	                    var nodes = xpath.evaluateXPath(contentsXPath, xmlData,this.namespaces);
	                    for (var i=0; i< nodes.length ;i++) {
	                    ...
	                    debug.exit("standardXMLParser.readPayloadDef ",nodes.length+ " payloadDefs are saved "); 
	                    return payloadDefsArr;
	               },
	               readPublicEvents:function(/*XMLDocument*/xmlData,/*String*/eventType){
	                ...
	                readEventDescriptions:function(/*XMLDocument*/xmlData){
	                     ...
	                     var nodes = xpath.evaluateXPath(eventDescriptionpath, xmlData,this.namespaces);
	                     if (typeof nodes != "undefined" && nodes != null && nodes.length !=0 ){
	                       for (var i=0; i<nodes.length;i++){
	                           var node = nodes[i];
	...
	return com.ibm.mm.enabler.iw.parserImpl;
});
