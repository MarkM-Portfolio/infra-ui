...
dojo.provide("com.ibm.social.as.gadget.ActivityStreamConfigUtil");
...
dojo.require("com.ibm.social.incontext.util.auth");
dojo.requireLocalization("com.ibm.social.as", "activitystream");
dojo.requireLocalization("lconn.homepage", "activitystreamconfig");

/**
 * Utility class for generating AS config for AS gadgets
 * @author BrianOG
 */
dojo.declare("com.ibm.social.as.gadget.ActivityStreamConfigUtil", null,
{
	strings: null,
	stringsHp: null,
	coreFilters: null,
	...
	constructor: function(){
		...
		this.stringsHp = dojo.i18n.getLocalization("lconn.homepage", "activitystreamconfig");
		this.svcs = dojo.mixin({}, lconn.core.config.services);
		this.xhrHandler = com.ibm.social.as.util.xhr.XhrHandler;
	},
	...
	getASConfig: function(opts) {
		...
		var cfgPromise = this.attachUserInfo(cfg)
			.then(dojo.hitch(this, "getThirdPartyApps"))
			.then(dojo.hitch(this, function() {
				dojo.mixin(cfg, this.buildConfig(opts, cfg.userInfo));
				return cfg;
			}));
	...
	buildConfig: function(opts, userInfo) {
		...
		if ( !lconn.core.config.services["microblogging"] ) {
	...
	_buildFollowingView: function(userInfo){
		...
		return dojo.mixin(following, sharedViews);
	},
	...
	getUserInfo: function() {
		...
		var osSvc = lconn.core.url.getServiceUrl(lconn.core.config.services["opensocial"]).uri;
		var url = osSvc + ( asc.useOAuth ? "/oauth" : "" ) + "/rest/people/@me/@self";
		
		// allow errors to bubble through, do not intercept.
		var promise = lconn.core.util.promises.deferredToPromise(this.xhrHandler.xhrGet({
			url: url,
			handleAs: "json"
		}))
        .then(function(resp){
            var user = com.ibm.social.incontext.util.auth.parseUserFromOpensocialResponse(resp);
            return user;
        });
    ...
    attachUserInfo: function(cfg) {
        return this.getUserInfo().then(function(ui) {
            ...
            var hasCurrentViewer = com.ibm.social.incontext.util.auth.hasCurrentViewer();
            if(!hasCurrentViewer){
                com.ibm.social.incontext.util.auth.setCurrentViewer(ui);
                lconn.core.auth.setAuthCheck(function () {
                    return com.ibm.social.incontext.util.auth.hasCurrentViewer();
                });
            }
            return cfg;
        });
	...
	getUserFromEmail: function(email) {
		...
		.then(dojo.hitch(this, "resolveUserFromEmail"));
	...
	resolveUserFromEmail: function(resp) {
        ...
        var user = com.ibm.social.incontext.util.auth.parseUser(resp.totalResults == 1 && resp && resp.list && resp.list[0]);
	...
	getThirdPartyApps: function() {
		...
		.then(dojo.hitch(this, "resolveThirdPartyApps"));
	...
	resolveThirdPartyApps: function(resp) {
		...
		for ( var i = list.length-1; i>=0; i-- ) {
			...
			if ( a && a.generator ) {
				...
				if ( !lconn.core.config.services[svc] ) {
	...
	addThirdPartyAppsToFilter: function(options, mixins) {
		...
		for ( var i=this.thirdPartyApps.length-1; i>=0; i-- ) {
			...
			if ( mixins ) {
				dojo.mixin(options[app.id], mixins);
			}
		}
	},	
...
//			extensions : [ "lconn.homepage.as.extension.SavedActionExtension", "lconn.homepage.as.extension.UnfollowExtension", "com.ibm.social.as.lconn.extension.MicroblogDeletionExtension" ],
    ...
    if ( s.communities ) {
		...
		if ( lconn.core.config.properties["com.ibm.social.as.noNetwork"] === "true" || userInfo.isExternal ) {
	...
	augmentConfig: function(cfg) {
		...
		if ( dojo.indexOf(cfg.extensions, "com.ibm.social.as.extension.CommentExtension") == -1 ) {
			cfg.extensions.push("com.ibm.social.as.extension.CommentExtension");
		}
		if ( dojo.indexOf(cfg.extensions, "com.ibm.social.as.gadget.refresh.RefreshButtonExtension") == -1 ) {
			cfg.extensions.push("com.ibm.social.as.gadget.refresh.RefreshButtonExtension");
		}
		if ( dojo.indexOf(cfg.extensions, "com.ibm.social.as.extension.RepostExtension") == -1 ) {
	...
	buildConfigFromEmail: function(email) {
		...
		.then(dojo.hitch(this, function(ui) {
