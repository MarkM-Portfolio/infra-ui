 ...
 *
 */
public class CommunitiesFeedService extends ConnectionsService {
	private final static String COPYRIGHT = com.ibm.lconn.bookmarklet.Copyright.SHORT;
	private String COMMUNITIES_URL = ServiceConfiguration.getString("CommunitiesFeedService.url"); //$NON-NLS-1$
	
	private Log _logger = LogFactory.getLog(CommunitiesFeedService.class);
	...
	public Entry buildEntry(Entry entry, ServiceData data) throws IRISyntaxException {
		...
		BookmarkServiceData fsd = (BookmarkServiceData)data;
		String title = fsd.getTitle();
		if (title == null) title = "";
		...
		String comments = fsd.getComments();
		entry.setContent(comments);
		Element content = entry.getFirstChild(new QName("content"));
		if (content!=null) {
			content.setAttributeValue("type", "html");
		}
		entry.addCategory("http://www.ibm.com/xmlns/prod/sn/type", "feed", "feed"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		
		List tags = fsd.getTags();
		if (tags!=null && tags.size()>0) {
			for(int i=0;i<tags.size();i++) {
				entry.addCategory((String)tags.get(i));
			}
		}
		entry.addLink(fsd.getUrl());
		if(_logger.isDebugEnabled()) {
