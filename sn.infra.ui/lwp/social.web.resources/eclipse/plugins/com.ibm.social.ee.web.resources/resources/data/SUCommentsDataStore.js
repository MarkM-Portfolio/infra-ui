...
dojo.require("com.ibm.social.ee.bean.SUPermission");

dojo.declare("com.ibm.social.ee.data.SUCommentsDataStore", com.ibm.social.ee.data.DataStore, {
   itemsFetched: false,
   dateAttrs: ["published", "updated"],
   permissionsUrl: null,
   attrMap: {
      "id" : "id",
      "author": function(item) { return { "id": com.ibm.social.ee.util.misc.getItemId(item.author.id), "name": item.author.displayName }; },
      "contents": "content",
      ...
      "updated" : function (item) { return dojo.date.stamp.fromISOString(item.published); },
      "published" : function(item) { return dojo.date.stamp.fromISOString(item.published); },
      "urlEntry" : function(item) { 
         var urlEntry = dojo.getObject("ds.attributes.urlEntry", false, item);
         if (urlEntry) 
            return urlEntry;
         else
   ...
   _fetch: function (request) {
      if (!this.itemsFetched)
         ...
         request.url = com.ibm.social.incontext.util.uri.rewriteUri(this.url, params);
   ...
   dataLoaded: function (request, response) {
	   ...
	   if (this.permissionsUrl && items.length) {
	      ...
	      for (i = 0; i < items.length; i++) {
	         if (i > 0)
	           requestUrl += "&";
	         var shortId = com.ibm.social.ee.util.misc.getItemId(items[i].id);
            ...
            handle: function(response, ioArgs) {
               if (!(response instanceof Error) && response.documentElement) {
                  ...
                  for (i = 0; i < entries.length; i++) {
                     var permission = new com.ibm.social.ee.bean.SUPermission(entries[i]);
                     var entityId = permission.getEntityId();
                     for (j = 0; j < items.length; j++) {
                        var shortId = com.ibm.social.ee.util.misc.getItemId(items[j].id);
                        if (shortId === entityId) {
   ...
   getValue: function (item, attribute, defaultValue) {
      if (item.preloaded) {
         ...
         if (attribute in this.attrMap) {
            ...
            if (dojo.isFunction(valueMap))
               return valueMap.call(this, item);
            else
               return dojo.getObject(valueMap, false, item);
         }
         return defaultValue;
      }
   },
   _newItem: function(item, keywordArgs) {
      var postBody = dojo.toJson({"content": item.ds.attributes.contents});
      this.net.postJson({
         noStatus: true,
         url: this.url,
         postData: postBody,
   ...
   _saveItemComplete: function(item, keywordArgs, response, ioArgs) {
      ...
      if ((!keywordArgs.requests || keywordArgs.requests === 0) && keywordArgs.onComplete) {         
         if (this.permissionsUrl && response.entry) {
            ...
            // Retrieve permissions for the fetched items
            var requestUrl = this.permissionsUrl + "?";
            var shortId = com.ibm.social.ee.util.misc.getItemId(item.id);
            requestUrl += ("action=DELETE_COMMENT&entityId="+shortId);
