...
define([
   ...
   var i, BLACKLIST = {};
   for (i=0;i<BLACKLIST_ELEMS.length;i++) {
   ...
   var _handleElement = function(/* Element */ element, /* boolean */ modify) {
      ...
      var i, attr, name, value, attrsToRemove = [];
      ...
      var node, hasScript, children = element.childNodes;
      for (i=0;i<children.length;i++) {
         node = children[i];
         if (node.nodeType == 1) {
            hasScript = _handleElement(node, modify);
            if (!modify && hasScript) {
               return true;
            }
         }
