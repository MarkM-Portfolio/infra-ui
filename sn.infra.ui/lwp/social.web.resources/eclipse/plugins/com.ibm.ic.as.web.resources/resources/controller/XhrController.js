/* Copyright IBM Corp. 2012, 2015  All Rights Reserved.              */
	define([
		...
		var XhrController = declare("com.ibm.social.as.controller.XhrController", null,
		{
			...
			populateFeedEventName: events.POPULATE,
			...
			fetchFeed: function(url, dontRefreshFilters, destroyNewsFeed, dynamicAdd, clickEvent, scrollTop){
				...
				var xhr = this.model.getStream({
					newsUrl: url,
					onLoad: lang.partial(lang.hitch(this, "onFeedLoad"), destroyNewsFeed, dynamicAdd, url, clickEvent, scrollTop),
					onError: lang.partial(lang.hitch(this, "onFeedError"), destroyNewsFeed, dynamicAdd, url, clickEvent, scrollTop)
				});
			...
			onFeedLoad: function(destroyNewsFeed, dynamicAdd, url, clickAction, scrollTop, response){
				topic.publish(events.FEEDFETCHED, url, destroyNewsFeed);
				...
				topic.publish(this.populateFeedEventName, response, destroyNewsFeed, dynamicAdd, clickAction, scrollTop, url);
			},
			...
			onFeedError: function(destroyNewsFeed, dynamicAdd, url, clickAction, scrollTop, error){
				...
				if ( error.unauthenticated !== true ) {
					topic.publish(this.populateFeedErrorEventName, error, dynamicAdd);
				}
			},
