...
define([
   ...
   "dojo/_base/lang",
   "dojox/html/entities",
   "ic-core/lcTextArea/mixins/Mentions"
   //"ic-ui/layout/people"
], function (declare, domConstruct, JSON, has, array, lang, entities, mentionsMixin) {
    ...
    *
    * @mixin ic-core.lcTextArea.mixins.IMentionsSupportRTE
    */
   ...
   function addNodeToArray(node, nodeArray) {
      if (node.nodeType === 1 && node.className == 'vcard') {
         var dataMentions = node.getAttribute('data-mentions');
         if (dataMentions) {
            nodeArray.push(JSON.parse(dataMentions));
         }
         else {
            nodeArray.push(mentionsMixin.getDataMentions(node));
         }
      } 
      else {
   ...
   var IMentionsSupportRTE = declare("lconn.core.lcTextArea.mixins.IMentionsSupportRTE", null, /** @lends ic-core.lcTextArea.mixins.IMentionsSupportRTE.prototype */
   {
      /**
       * Get the mentions that are contained in the text input.
       * <p>
       * This method mimics the behavior of {@link lconn.core.widget.mentions.MentionsUtils#getTextAsJson}.
       *
      ...
      getTrackedMentions : function() {
         ...
         var dummy = domConstruct.toDom(this._editor.getData());
         var nodesArray = [];

         array.forEach(dummy.childNodes, function(line) {
            if (line.nodeName == 'P' || line.nodeName == 'BR') {
               array.forEach(line.childNodes, function(node) {
      ...
      /**
       * gets returns the editor value.
       * the value is formatted if plainTextOnly is true.
       */
      getValue : function(plainTextOnly) {
         ...
         if (plainTextOnly) {
            value = mentionsMixin.formatGetValue(value);
         }
         else {
      ...
      /**
       * takes in a value, which based on plainText boolean,
       * is formatted properly, then set into the editor.
       */
      setValue : function(value, plainTextOnly) {
         ...
         value = value.replace(/data-cke-saved-href=+"(?:[^\\"]+|\\.)*"/gi, '');
         value = mentionsMixin.formatSetValue(value, plainTextOnly);
         if (this._editor && this._editor.document) {
            this._editor.document.getBody().setHtml(value);
            // Grab any mentions <span> elements and re-attach the bizCard
