...
/**
 * @class lconn.core.upload.data.FileList
 */
dojo.declare("lconn.core.upload.data.FileList", null, /** @lends lconn.core.upload.data.FileList.prototype */ {
   ...
   _addFile: function(fileObject) {
      var file = dojo.mixin(dojo.mixin({}, fileObject), {_fileList: this});
   ...
   addFiles: function(fileArray) {
      if (!dojo.isArray(fileArray)) {
         fileArray = [ fileArray ];
      }

      fileArray = dojo.map(fileArray, this._addFile, this);
   ...
   validateDuplicates: function(name) {
      ...
      var nameList = dojo.filter(this.getUploadableFiles(true), function(file) {
         return name == file.getName();
      }, this);

      var localsExist = nameList.length > 1;
      dojo.forEach(nameList, function(file) {
   ...
   getUploadableFiles: function(skipValidation) {
      var states = lconn.core.upload.data.File.prototype.UploadStates;
      return dojo.filter(this._files, function(file) {
   ...
   getInvalidFiles: function() {
      return dojo.filter(this._files, function(file) { return !file.isValid(); });
   },
   ...
   _processExtensions: function(ext) {
      ...
      if (ext && dojo.isArray(ext.allowed)) {
         normalized.allowed = dojo.map(ext.allowed, function(extension) {
      ...
      } else if (ext && dojo.isArray(ext.prohibited)) {
         normalized.prohibited = dojo.map(ext.prohibited, function(extension) {
            if (extension.charAt(0) == '.') {
               extension = extension.substring(1);
            }
