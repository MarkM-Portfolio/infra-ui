...
define([
	...
	"./TooltipDialog"         // 248295: Enable "dijit-legacy-requires" feature for dijit backward compatibility
			...
			if(this.domNode.style.display != "none"){
				...
				if(!dim){
					if(this._shrunk){
						...
						array.forEach([this.domNode, this.containerNode, this.titleBar, this.actionBarNode, this.footer_AP], function(node){
				...
				if(dim){
					...
					if(this.actionBarNode){
					   ...
					   if (this.footer_AP){
					      layoutNodes.push({domNode: this.footer_AP, region: "bottom"});
					   }
					}
	...
	focus.watch("curNode", function(attr, oldNode, node){
		...
		if(node && topDialog && !topDialog._fadeOutDeferred && node.ownerDocument == topDialog.ownerDocument){
		   ...
		   //typeAhead skipped in case of typeahead popup being loaded into dialog and attempting to change focus.
			do{
				if(node == topDialog.domNode || domClass.contains(node, "dijitPopup") || domClass.contains(node, "typeAhead")){ return; }
