...
(function () {
	...
	SWFUpload.prototype.loadFlash = (function(oldLoadFlash) {
		return function() {
			if (dojo.isFunction(oldLoadFlash)) {
				oldLoadFlash.call(this);
			}
			
			var movie = this.movieElement;
			if (movie) {			
				dojo.addClass(movie, 'lconnInvisibleMovie');
				...
				this.a11y_input.appendChild(dojo.doc.createTextNode(this.customSettings.htmlButtonText || (dojo.isWebKit ? nls.BUTTON_TEXT_WEBKIT : nls.BUTTON_TEXT)));
	...
	SWFUpload.a11y.handleKeyPress = function (event) {		
		if (event.keyCode == dojo.keys.TAB) {
			var containerNode = dojo.byId(this.customSettings.containerNode) || dojo.body();
			// get all focusable nodes
			var nodes = dojo.query("*", containerNode).filter(dijit.isTabNavigable);
	...
	var _simulateKeyEvent = function(targetNode, event) {
		...
		if (dojo.isIE) {
			fauxEvent = dojo.doc.createEventObject();
			...
			fauxEvent = dojo.doc.createEvent("KeyboardEvent");
			
			var initFn = dojo.isMozilla ? fauxEvent.initKeyEvent : fauxEvent.initKeyboardEvent;
			initFn.call(fauxEvent, "keypress", true, true, dojo.global, event.ctrlKey, event.altKey, event.shiftKey, event.metaKey, event.keyCode, event.charCode);
