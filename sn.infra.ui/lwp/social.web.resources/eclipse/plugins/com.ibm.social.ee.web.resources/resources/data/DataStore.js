...
dojo.declare("com.ibm.social.ee.data.DataStore", null, {
   ...
   containsValue: function (item, attribute, value) {
      return dojo.indexOf(item.getValues(item,attribute), value) != -1;
   },
   ...
   fetch: function (kw){
      ...
      if (!request.abort)//have to do a mixin in here so that request maintains the proper scope
         request = dojo.mixin(request, {abort: dojo.hitch(this, this.abortRequest, request)});
   ...
   _fetch: function(request) {
      ...
      var opts = {
         ...
         handle: dojo.hitch(this, this.handleGet, dojo.hitch(this, this.dataLoaded, request), dojo.hitch(this, this.dataError, request)),
         preventCache: true
      };        
      ...
      if ((typeof response == "Error") || (response.name == "Error"))
      {
         ...
         if (response && response.dojoType == "timeout") {
            if (ioArgs && ioArgs.xhr && ioArgs.xhr.status == 200) {
               response = dojo._contentHandlers[ioArgs.handleAs](ioArgs.xhr);
